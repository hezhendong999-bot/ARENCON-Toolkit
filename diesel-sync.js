...
const CloudSync = (function () {
  ...
  function _noteLocalWrite(ok) {
    ...
    try { console.error('[DieselSync] local save FAILED — device storage full or unavailable; will retry'); } catch (_) {}
  ...
  function _persistAtStamp() {
    ...
    if (stateJson === _lastSavedJson && !_localWriteFailed) return;  // nothing new to make durable (S732: unless the last write failed)
    _lastSavedJson = stateJson;
    _cachePut(_cacheKey(), {
      state: stateJson, projectId: _projectId, toolKey: _toolKey,
      instanceId: engine.instanceId || _instanceId, instanceNumber: engine.instanceNumber || _instanceNumber,

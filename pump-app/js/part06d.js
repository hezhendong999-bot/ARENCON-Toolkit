...
/* One quiet, permanent line saying this is the fork and nothing leaves the
   device. Drawn once, never a toast — a background fact, not an event. */
function _pfNoCloudBanner(){
  try{
    if(document.getElementById('pf-lane')) return;
    var b=document.createElement('div');
    b.id='pf-lane';
    b.textContent='Parallel build \u2014 saved on this device only, never to the cloud.';
    b.style.cssText='position:fixed;left:0;right:0;bottom:0;z-index:9998;padding:6px 14px;'
      +'font:600 12px Calibri,sans-serif;color:#C98A4A;background:rgba(201,138,74,.12);'
      +'border-top:1px solid rgba(201,138,74,.45);text-align:center;';
...
window.addEventListener('load', () => {
  ...
  function _cloudSyncInit(){
    /* ═══ PARALLEL LANE — THE CLOUD IS OFF (S730j) ═══════════════════════
       This tool is a FORK of the live Diesel tool, built so the multi-pump
       work cannot touch what the field uses today. Cloud sync is therefore
       DISABLED outright: a fork writing to tool_data would put experimental
       rows next to real reports under the same project id, and a merge on
       the other side could pull them into a live report. Local records are
       already namespaced (see getProjectSaveKey). The cloud comes back only
       when the Owner is present at a tablet and rules on how this lane's
       records are to be stored — never as a quiet re-enable. */
    _pfNoCloudBanner();
    /* S732 — the fork never restored its own records: the S730 early return
       sat ABOVE the standalone loadAutosave(), so every reload began blank
       (found by the boot proof, not by reading). Local-only was always the
       lane's design; this is the load half of it. Key is pumpfork_, cloud
       stays off, nothing live is touched. */
    loadAutosave();
    updateProgress();
    return;

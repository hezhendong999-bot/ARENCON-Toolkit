...
function getProjectSaveKey(){
  ...
  /* PARALLEL LANE (S730j). This tool is a FORK. Its local records live under
     their own prefix so that a project number typed here can never land on
     top of the same project's live Diesel report on a shared tablet. The
     prefix is the whole safety story for local storage — do not "tidy" it
     back to 'diesel_'. */
  return 'pumpfork_'+pno;
}

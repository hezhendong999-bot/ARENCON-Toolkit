...
// ══════════════════════════════════════════════════
const S1 = [
  { num:"1.1", scope:"visit", hint:"Same answer on the sister report.", text:"Coordinate with Building owner/authorized personnel, AHJ, contractor for test date and time. Coordinate alternate parking arrangement with Owner if required." },
  { num:"1.2", scope:"machine", hint:"Shared suction header: flush the header once; this tick is “this pump connected after that flush.”", text:"Confirm with the Contractor that the suction pipe has been flushed prior to connecting to the fire pump. Written confirmation required." },
  { num:"1.3", scope:"visit", hint:"One attendance list for the room. TSSA is the diesel attendee, not a second meeting.", text:"The following personnel shall attend the fire pump commissioning test: The sprinkler/fire pump installation contractor; The fire pump & controller test agent; TSSA inspector. Note: Sometimes the fire pump & controller testing agent is the TSSA inspector." },
  { num:"1.4", scope:"room", hint:"One tick if the whole installation was one pressurized envelope. If an isolation valve split the test, say so in the note and treat each envelope separately.", text:"Confirm hydrostatic test of the fire pump installation at 200 psi, or 50 psi in excess of maximum pressure, whichever is greater, for a minimum of 2 hours. Record what piping was in the test." },
  { num:"1.5", scope:"visit", hint:"Contractor test-kit gauges for this visit. Installed pump gauges are 2.4.", text:"Contractors shall bring calibrated gauges (not older than 12 months) to replace the installed fire pump suction and discharge gauges during the pump test." },
  { num:"1.6", scope:"visit", hint:"Same kit can serve both pumps the same day.", text:"Ask the Contractors to bring calibrated Hose Monsters (and paired flow chart) if possible, to minimize water damage to landscape. If not, play pipes are acceptable." },
  { num:"1.7", scope:"visit", hint:"Photo the NIST cert for the gauges used today.", text:"The calibration standard is NIST. Ensure to take a photo of the calibration certificate." },
  { num:"1.8", scope:"machine", hint:"The ESA certificate covers the room electrical work — note it rather than treating it as per controller. Control wiring and fire alarm points on this controller are this tick.", text:"Confirm fire pump control wiring, fire alarm monitoring, ESA inspection and verification is complete." },
  { num:"1.9", scope:"visit", hint:"Same answer on the sister report.", text:"Ensure to bring ear protection and PPE." },
];

const S2 = [
  { num:"2.1", scope:"machine", hint:"This pump and its driver / controller.", text:"Confirm fire pump components have been installed properly as per design drawings and are secure." },
  ...
  { num:"2.5", scope:"diesel-only", text:"Conduct diesel tank, concrete containment and dike inspection. Confirm no cracks on the fire pump pad. Confirm net capacity of the concrete dike exceeds 10% of diesel tank capacity. Unit: 1 US gal = 0.161 ft³. FM requires additional 2\" freeboard in addition to the required dike height (normally 6\" to 8\")." },
  { num:"2.6", scope:"diesel-only", text:"If a floor drain is located within the containment, it shall be plugged or provided with curb to prevent fuel entering the drain." },
  { num:"2.7", scope:"diesel-only", text:"Any concrete surface (e.g. pads, tank support, floor etc.) within the containment footprint shall be treated with an impermeable coating (e.g. Epoxy) / fuel oil sealant as per CSA B139-19." },
  { num:"2.8", scope:"diesel-only", text:"Confirm tank diesel level at the level indicator on top of the diesel tank. Confirm the capacity of tank and tank type (double-wall) on the tank placard. The fuel tank shall be kept as full and maintained as practical at all times but never below 66% of tank capacity (pull the rod out and measure the length)." },
  { num:"2.9", scope:"diesel-only", text:"Confirm the diesel fuel tank supports (2\" sch 40 pipes) are enclosed in concrete (sona-tube or concrete footing)." },
  { num:"2.10", scope:"room", hint:"One walk of the room. Same answer on the sister report.", text:"Confirm all valve tags have been provided within the fire pump room." },
  { num:"2.11", scope:"room", hint:"OBC room construction. Same answer on the sister report.", text:"Confirm firestopping provided at each pump room penetration, except exterior wall. Any exposed pump room structural steel (not full height wall pump rooms) shall be treated with min. 1-hr F.R.R. fire spray. Interior door and frames shall be equipped with automatic door closure and rated for min. 45 minutes." },
  { num:"2.12", scope:"diesel-only", hint:"Engine starting batteries. Not the electric controller.", text:"Confirm if batteries and battery racks are provided." },
  { num:"2.13", scope:"diesel-only", text:"Confirm pump engine exhaust is equipped with a muffler to discharge fumes to exterior. The discharge point shall be minimum of 12 ft above any accessible level, and not closer than 5 ft from any building openings." },
  { num:"2.14", scope:"diesel-only", text:"The engine exhaust flex connection, exhaust pipe, long elbow and muffler shall be wrapped in high temperature insulation wrap and preferably c/w aluminum jacket within the pump room, regardless the height of the exhaust pipe." },
  { num:"2.15", scope:"diesel-only", text:"The exhaust flex connection shall be stainless steel, seamless or welded corrugated (not interlocked), not less than 12\" in length." },
  { num:"2.16", scope:"diesel-only", text:"The flex connection shall be mechanically guarded only, and shall not be wrapped." },
  { num:"2.17", scope:"diesel-only", text:"After the muffler, the exhaust pipe shall be a stainless-steel pressure chimney that complies with CSA B139. Black steel pipe as exhaust pipe discharge to exterior is commonly done but is not acceptable." },
  { num:"2.18", scope:"diesel-only", hint:"OFC clearance is still this engine’s chimney, not a room tick.", text:"The chimney outlet shall be minimum of 2 ft above roof line and maintain sufficient clearance to building air intake per OFC." },
  { num:"2.19", scope:"diesel-only", text:"Any ductwork, including intake and exhaust ducts, engine exhaust chimney that needs to travel within the building to roof after exiting the ceiling of pump room, shall be wrapped in min. 1-hr F.R.R. fire wrap. This is not required if the chimney discharges directly through the exterior wall of fire pump room." },
  { num:"2.20", scope:"diesel-only", hint:"Certificate lives on this tank.", text:"Confirm if TSSA certificate \"Fuel Oil Distributor Inspections Above Ground Tanks\" has been provided at the diesel fuel tank. The TSSA certificate shall state which CSA B139 yearly edition is used (2019)." },
  { num:"2.21", scope:"machine", hint:"This pump and its jockey.", text:"Confirm no shut-off valves installed on the fire pump & jockey ½\" pressure sensing lines. The pressure sensing line shall be installed between the fire pump/jockey discharge check valve and control valve. It is not acceptable to install the pressure sensing lines on the upstream side of the discharge check valve." },
  { num:"2.22", scope:"machine", hint:"This pump’s suction. A shared header does not move the air relief to the room.", text:"If any inverted U shape overhead piping is installed on the upstream side of the fire pump suction outlet, a ½\" automatic air relief valve shall be provided at the top of the suction pipe." },
  { num:"2.23", scope:"machine", hint:"Last length into this casing.", text:"Confirm min. 10× pipe diameter is provided on the suction side of the fire pump, if the suction pipe is running perpendicular to the fire pump." },
];

# ARENCON Fire Protection Toolkit — Project Knowledge (CANON)

**Last reconciled:** Sunday, May 31, 2026 — full PK merge pass (folds the recovered S25→S154 history + the S205 lean canon + the S216 forward regen into one comprehensive file, aggressively pruning everything dead). This document is the single source of truth.

**Live FRT after S216:** SW `arencon-frt-v579` / CSS `frt.css?v=438`. HEAD `b9ef0c94ac`.

This file folds every FRT delta through S216 and supersedes all `PK_DELTA_S2*.md` files for the items they cover. Diesel-specific deltas (`DIESEL_PK_DELTA_*`) belong to the pump-tool workstream and are folded here only where they state a cross-cutting rule.

---

## ⛔ DO NOT RESURFACE — authoritative ledger

Never propose, re-investigate, or list any of these as a next step, priority, or fallback. If older notes contradict this, this wins.

| Item | Status |
|---|---|
| **Staging environment** | **PERMANENTLY ABANDONED.** Do not mention, propose, or treat as a priority. There is one environment: PROD. |
| **Written business case / principals presentation doc** | **PERMANENTLY EXCLUDED.** The principals deliverable is the PDF output of the live tool — never a written case. `FRT_REWRITE_BUSINESS_CASE.md` refresh and any "delivered vs promised" deliverable are excluded. |
| **Fly.io tile renderer** | **DECOMMISSIONED.** Azure Function is the only renderer. |
| **iOS native app / iPhone field use** | Abandoned (S112). Android TWA only. Field tablets are Android. |
| **Tiled-renderer-disabled era (S28)** | Historical only. Tiled rendering is LIVE via Azure (5 levels L0–L4). Do not cite "drawings load as plain `<img>`" as current. |
| **IAR feature** | **FULLY RETIRED** (UI gone S135; S143 actively clears legacy `iar:true`→false on load; PDF IAR column removed). Do not re-add IAR UI, handlers, `toggleIAR`, or the pink `#E91E8C` IAR badge. |
...
| **S206 feature build** (Move/Copy ⤴ reach, per-obs scoping, board touch-drag, M3 thumb targets, orphan deletions) | **NEVER PUSHED — that build was LOST (container reset).** Pieces were rebuilt later where still wanted (per-obs scoping shipped S209; gallery ⤴ defic shipped S216). Do not cite S206 as live history. |
| **Mobile backlog M1 / M2** | **COMPLETE (S166/S188).** Drawings grid `minmax(280px,1fr)`, pin-editor vertical stack, trade-pill flex-wrap. Do not re-open. |
| **Mobile backlog M3** (touch-target audit) | **SUBSTANTIALLY COMPLETE (S178/S188).** Photo-thumb corner buttons have ≥44px hit areas. |
| **`hubBridge.js` / `ui/pins.js`** | **DELETED (S206)** — proven orphans (abandoned normalized-rows experiment; old DOM pin system retired S135). Do NOT recreate or "restore." |
| **`markupEngine.js`** | **⚠️ LIVE — NEVER DELETE.** Looks orphaned (not ES-imported, uses `self.`) but it sets `window.MarkupEngine`, consumed by `ui/lightbox.js` for the photo-markup toolbar. Deleted-then-restored in S206. Any dead-code pass must treat it as live. |
...
| **Obs photo-tile button overlap** | **FIXED S207** (CSS). One control per corner: ⤴ pin top-left, ✕ del top-right, ✨ AI bottom-right, ✓ sync badge bottom-left. |
| **Auto-refresh force-reload disruption** | **FIXED S207.** Forced `reload()` on `sw-updated` replaced with a non-disruptive top-center "Update ready" banner (Refresh / Not now); `boot()` restores tab + scroll via sessionStorage. Do NOT restore the force-reload — it once cost a field day; the deferred banner preserves the safety intent without interrupting work. |
| **S146/S147 trade fan-out** (untagged pin on a multi-trade contractor listed under EVERY trade) | **RETIRED + SHIPPED S208 (HEAD 6eda93c6).** Detailed view uses `Model.derivePinTradeSingle` → each pin under ONE trade. `derivePinTrades` (plural) kept but uncalled by Detailed view; `derivePinTrade` (singular, Trade-Board) UNCHANGED. PDF was already single-trade (`pdf.js _aByT`) — unaffected. |
| **Trade tint / trade stripe on deficiency rows or cards** | **Do NOT re-propose.** Locked S208: trade = the navy band (`#2A3A5C`); contractor = the colour. One colour language per element. Both stripe + tint were explored and rejected with Mark. |
| **Deficiency List redesign re-demo** | **DONE — DO NOT RE-DEMO (S209).** Mark: "we've locked it in already." The mockup `FRT_list_options_demo.html` is final. |
| **"Mark as recommendation" text button** | **REMOVED PERMANENTLY (S207–S211).** The ★ star is the only recommendation control in every editor. Never re-add the text button. Preserve the list-row star's `_recHoldUntilNav` mis-tap-undo behavior. |
| **"General" priority** | **SLATED FOR REMOVAL — locked S216 with the Board redesign (below).** Already gone from the Detailed combined status control and the new editor. The Board redesign removes the General column entirely and migrates existing General pins to Site Records. |
| **Table view (Deficiencies tab)** | **RETIRED S216.** Toggle is now Detailed + Board only. `_renderTableView` left defined-but-inert (S137); remove with `.dfx-tbl*` table-only CSS at a later canon pass. |
| **B2f persistent roster / S134 kanban Trade Board / S136 picker** | **RETIRED.** The contractor surface is the S142 §2 ClickAssign Roster, since absorbed into the current Detailed + Board model. Do not resurface the kanban Trade Board as a UI generation. |
| **`_collectFullState` empty-push / `instance_number.desc` load / Wi-Fi-only upload** | **NEVER reintroduce** — each is a root cause of the S25 data-loss incident; see Data Integrity. |

**Cold-start rule:** read this ledger first; honor it over any older source.

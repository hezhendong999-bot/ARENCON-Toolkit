...
t('module version bumped', () => eq(window.MarkupSelection.VERSION, '2.8.2'));
...
t('LOCKED (Mark): picks = glow BOX + check badge; grouped members = contour glow', () => {
  ...
  const mem = src.slice(src.indexOf('if(this.selIds && this.selIds.length){'));   // S461u: glow in BOTH modes
...
t('LOCKED: group-box chrome math is byte-identical to the approved v2.2', () => {
  const src = readFileSync('/home/claude/markupSelection.js','utf8');
  // The chrome scales from this.nw — NOT from a helper that changes the value.
  // (S461p: an _selNW() rewrite made the box + handles thin and small.)
  if(/_selNW\(\)\/Math\.max/.test(src))
    throw new Error('chrome scale math was rewritten — locked chrome must read this.nw');
  if(!/_fixNW:function/.test(src))
    throw new Error('the nw=0 guard must repair the VALUE, not the math');
});

...
console.log('\n── S461 contour halo ──');
t('module version bumped', () => eq(window.MarkupSelection.VERSION, '2.1.0'));
...
t('BLUE group box untouched — its geometry/pad code is unchanged', () => {
  const src = readFileSync('/home/claude/markupSelection.js','utf8');
  if(!/var pad=\(coarse\?30:22\)\*k, hs=11\*k;/.test(src)) throw new Error('S459e blue grab pad changed!');
  if(!/ctx\.strokeStyle='#3F6E9C'; ctx\.lineWidth=lw;\n      ctx\.strokeRect\(g\.b\.x1,g\.b\.y1/.test(src)) throw new Error('blue group box draw changed!');
});

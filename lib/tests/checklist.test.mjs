...
// ═══ signaturePad (S461 leg 3) ═══
console.log('\n── SigPad (vector-stroke signature engine) ──');
const SigPad = (() => {
  // minimal DOM shim for the module load + repaint/printSrc paths
  const mkCtx = () => ({ ops: [], lineWidth: 0, lineCap: '', lineJoin: '', strokeStyle: '',
    clearRect(){ this.ops.push('clear'); }, beginPath(){ this.ops.push('begin'); },
    moveTo(){ this.ops.push('move'); }, lineTo(){ this.ops.push('line'); }, stroke(){ this.ops.push('stroke'); } });
  globalThis.window = globalThis;
  globalThis._sigTestCanvases = {};
  globalThis.document = {
    body: { classList: { contains: () => globalThis._darkMode === true } },
    getElementById: id => globalThis._sigTestCanvases[id] || null,
    createElement: () => { const c = { width: 0, height: 0, _ctx: mkCtx() }; c.getContext = () => c._ctx; c.toDataURL = () => 'data:print'; return c; }
  };
  return require('../ui/signaturePad.js');
})();
t('module surface', () => {
  for (const k of ['init','ink','repaint','repaintAll','printSrc']) eq(typeof SigPad[k], 'function', k);
  eq(SigPad.VERSION, '1.0.0');
});
t('ink follows theme (light #1C2333 / dark #ffffff)', () => {
  globalThis._darkMode = false; eq(SigPad.ink(), '#1C2333');
  globalThis._darkMode = true;  eq(SigPad.ink(), '#ffffff');
  globalThis._darkMode = false;
});
t('repaint draws stored vector strokes in current ink', () => {
  const ctx = { lineWidth:0,lineCap:'',lineJoin:'',strokeStyle:'', calls:[], clearRect(){this.calls.push('clear');}, beginPath(){this.calls.push('begin');}, moveTo(){this.calls.push('move');}, lineTo(){this.calls.push('line');}, stroke(){this.calls.push('stroke');} };
  globalThis._sigTestCanvases['sig-canvas'] = { width: 900, height: 130, getContext: () => ctx };
  globalThis._sigStrokes = { 'sig-canvas': [ { pts: [{x:1,y:1},{x:2,y:2},{x:3,y:3}] } ] };
  SigPad.repaint('sig-canvas');
  eq(ctx.calls[0], 'clear');
  eq(ctx.calls.filter(c=>c==='line').length, 2, 'two lineTo for 3 pts');
  eq(ctx.strokeStyle, '#1C2333', 'current theme ink');
});
t('printSrc forces dark ink regardless of theme (PDF rule) and uses an offscreen canvas', () => {
  globalThis._darkMode = true;   // dark mode active…
  const out = SigPad.printSrc('sig-canvas');
  eq(out, 'data:print', 'rendered via offscreen createElement canvas');
  globalThis._darkMode = false;
});
t('printSrc falls back to live bitmap when no strokes (legacy/uploaded)', () => {
  globalThis._sigStrokes = {};
  globalThis._sigTestCanvases['sig-canvas'].toDataURL = () => 'data:livebitmap';
  eq(SigPad.printSrc('sig-canvas'), 'data:livebitmap');
});
t('repaintAll is safe with no state', () => {
  delete globalThis._sigStrokes;
  SigPad.repaintAll();   // must not throw
});

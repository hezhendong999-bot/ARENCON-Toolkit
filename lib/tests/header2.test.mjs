...
/* S670: this pinned '2.7.0' and had therefore failed on every push since the
   engine passed that version — a red harness that everyone learns to ignore is
   worse than no harness. The version exists to move; what must never happen is
   it going BACKWARDS or stopping being a real version, so that is what is
   tested. Raise FLOOR deliberately when an old version must be locked out. */
const VERSION_FLOOR = [2, 7, 0];
t('B01 version is semver and never below the 2.7.0 floor', () => {
  const m = /^(\d+)\.(\d+)\.(\d+)$/.exec(ENGINE2_VERSION);
  ok(m, 'ENGINE2_VERSION must be x.y.z, got ' + ENGINE2_VERSION);
  const v = [ +m[1], +m[2], +m[3] ];
  const cmp = v[0] - VERSION_FLOOR[0] || v[1] - VERSION_FLOOR[1] || v[2] - VERSION_FLOOR[2];
  ok(cmp >= 0, ENGINE2_VERSION + ' is below the ' + VERSION_FLOOR.join('.') + ' floor');
});
...
t('C15 electric config: day/night artwork engine-owned (S460 bug root-caused)', () => {
  ...
  ok(elec.includes('_dayNight('),
     'electric uses the SHARED day/night factory (engine-owned artwork + foldGroup)');
...
t('D13 FRT config: 4 adminOnly gates (section + 3 repair rows)', () => {
  /* S670 — this expected 5 and listed rows that no longer exist. Both changes
     were Mark's, recorded in headerConfigs.js itself:
       S497e — Diagnostics removed from the FRT More menu (admin clutter), which
               is the fifth adminOnly gate this count used to include;
       S497h — 'Fix Blurry' and 'Repair R2 Links' removed because neither ever
               had an implementation — permanently inert rows in a recovery menu
               are worse than absent ones.
     So the surviving gates are the section itself plus its three real rows. The
     count is still asserted exactly, so a gate silently dropping still fails. */
  eq((frt.match(/adminOnly:true/g)||[]).length, 4);
  ok(frt.includes('Re-upload All') && frt.includes('Repair Photos') && frt.includes('R2 Cleanup'),
     'the three real repair rows are present');
  ok(!frt.includes('Fix Blurry') && !frt.includes('Repair R2 Links'),
     'S497h: an inert row must not come back without an implementation');
});
t('D14 AI Review is PURPLE #7B2D8E (S488 Mark — S455 burgundy reversed), no burgundy anywhere', () =>
  ok(frt.includes('#7B2D8E') && !frt.includes('#7d1f35') && !cfgSrc.replace(/\/\*[\s\S]*?\*\//g,'').replace(/\/\/.*$/gm,'').includes('#7d1f35')));
...
t('D21 Repair section admin-gated (S284), and every row inside it too', () =>
  ok(frt.includes('repairSection:true') && (frt.match(/adminOnly:true/g)||[]).length >= 4));
...
t('E03 both self-referential avail measures are GONE (grown-box and squeezed-edge)', () => {
  const rf = src.slice(src.indexOf('function refit'), src.indexOf('try{ new ResizeObserver'));
  ok(!rf.includes('left.offsetWidth') && !rf.includes('leftEdge'),
     'avail must come from the reserved-minimum math, not from measuring .left');
});
...
t('E07 fold avail RESERVES the left minimum (no self-referential measure)', () =>
  /* S670 — this matched the literal `const TITLE_MIN = 60`, which S551 turned
     into a conditional: below 480px the title is not drawn, so reserving 60px
     for it would reserve room for something absent — the same mistake as the
     hamburger's right-edge gap. Match the declaration, not one of its values. */
  ok(/const TITLE_MIN\s*=/.test(src) && src.includes('reserved += c.offsetWidth')));
t('E08 title floor is structural CSS matching TITLE_MIN, and no min-width:0 survives in the title rule', () => {
  ...
  ok(tr.includes('min-width:60px') && !tr.replace(/\/\*[\s\S]*?\*\//g,'').includes('min-width:0'));
});
t('E10 drawer children are the v1-verbatim port (icons full-width 46, :host-armored vs skin specificity)', () => {
  ok(src.includes(':host .drawer > .hicon{ width:100%') &&
     src.includes('min-height:46px') &&
     src.includes(':host .drawer > *{ width:100%'));
});
t('E12 drawer icon row: QR/day-night/text-size share one 3-across row', () =>
  ok(src.includes(".iconrow{ display:flex; gap:8px") && src.includes("iconFold.push(b)")));
t('E13 drawer order (Mark): buttons, then icon row, then IDB below all buttons', () => {
  const rb = src.slice(src.indexOf('function _rebuildDrawer'), src.indexOf('function refit'));
  ok(rb.indexOf('rest.forEach') < rb.indexOf("'iconrow'") &&
     rb.indexOf("'iconrow'") < rb.indexOf('meter.node'), 'rest -> iconrow -> meter');
});
t('E15 fold groups: trio folds together (never a stretched single)', () => {
  const r = computeFold(
    [I('qr', { foldGroup:'icons', exemptUntilLast:true }),
     I('dark', { foldGroup:'icons', exemptUntilLast:true }),
     I('ts', { foldGroup:'icons' }),
     I('big')],
    { qr:34, dark:34, ts:34, big:100 }, 150);
  const f = new Set(r.folded);
  ok((f.has('ts') && f.has('qr') && f.has('dark')) || (!f.has('ts') && !f.has('qr') && !f.has('dark')),
     'icon trio folds all-or-nothing, got: ' + r.folded.join(','));
});
t('E17 Diesel config is v2-native (no v1 light-DOM contract) and matches the new rules', () => {
  const d = cfgSrc.slice(cfgSrc.indexOf('export function dieselHeaderConfig'),
                         cfgSrc.indexOf('_signout(h.onSignout', cfgSrc.indexOf('export function dieselHeaderConfig')))
                  .replace(/\/\*[\s\S]*?\*\//g,'').replace(/\/\/.*$/gm,'');
  ok(!d.includes('signalSlots') && !d.includes('wrapId') && !d.includes("icon:''"), 'v1-isms removed');
  ok(d.includes("#7B2D8E") && !d.includes('#7d1f35'), 'AI purple');
  ok(d.includes("_dayNight(") && d.includes("_qr(") && d.includes('_textSize('), 'icon trio via shared factories');
  ok(d.includes("skin:'chrome'"), 'unified chrome skin');
});
t('E16 all three icon factories carry the shared fold group', () => {
  ['_qr','_dayNight','_textSize'].forEach(fn => {
    const s = cfgSrc.slice(cfgSrc.indexOf('function ' + fn), cfgSrc.indexOf('}', cfgSrc.indexOf('function ' + fn) + 40));
    ok(s.includes("foldGroup:'icons'"), fn + ' missing foldGroup');
  });
});
t('E14 Load lives in More, never as a standalone header button', () =>
  /* S670 — the old assertion also required 'Export All Projects' in the More
     menu. S497e removed it from FRT deliberately: exporting every project is a
     Hub action, not something a field tool should offer. The part that still
     matters — neither one may reappear as a bare button in the bar — is kept. */
  ok(!frt.includes("key:'load'") && !frt.includes("key:'exportall'") &&
     frt.includes('Load JSON') && !frt.includes('Export All Projects')));
t('E11 no drawer close button — v1 device-verified closure (backdrop / item tap)', () =>
  ok(!src.includes("dclose")));
...
t('E18 the account avatar is creatable AFTER build (S670 defect guard)', () => {
  /* The defect: the avatar element was built only inside `if (cfg.account)` at
     build time. Only the Hub can pass an account block up front — FRT, Diesel
     and Electric learn who is signed in a round trip later — so those three got
     no avatar, and because setAccount also hides the standalone Sign Out they
     were left with no way out of the app at all. Guard both halves: creation is
     a callable, and the late path calls it before painting. */
  ok(/function mkMe\s*\(/.test(src), 'avatar creation must be a function, not a build-time branch');
  const sa = src.slice(src.indexOf('setAccount(a){'), src.indexOf('setPresence(o){'))
                .replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');   /* code only */
  ok(sa.indexOf('mkMe()') !== -1 && sa.indexOf('mkMe()') < sa.indexOf('paintMe()'),
     'setAccount must create the element before painting it');
  ok(src.includes('actions.insertBefore(meWrap, moreBtn.nextSibling)'),
     'a late-created avatar must land in the same slot as a build-time one');
});
t('E06 no ts-dead font declarations anywhere in the sealed CSS', () => {
  ...
  /* S670 — two declarations have always failed this and the failure was carried
     rather than judged. Both are the round initials avatar (.pav, and the bigger
     .meav .pav in the account menu). Those circles are a FIXED pixel size, so
     initials that grew with the S/M/L text control would spill out of them —
     the letters must scale with the circle, not with body text. That is a real
     exemption, so it is declared by SELECTOR and the fixed geometry is proven
     below. Anything else with a dead size still fails, which is the point. */
  const EXEMPT = ['.pav', '.meav .pav'];
  const selectorOf = (i) => {
    const open = css.lastIndexOf('{', i);
    const prev = Math.max(css.lastIndexOf('}', open), css.lastIndexOf('{', open - 1));
    return css.slice(prev + 1, open).replace(/\/\*[\s\S]*?\*\//g, '').trim();
  };
  const dead = [...css.matchAll(/font(?:-size)?:[^;]*?\d+(?:\.\d+)?px[^;]*;/g)]
    .filter(m => !m[0].includes('var(--ts'))
    .filter(m => EXEMPT.indexOf(selectorOf(m.index)) === -1);
  eq(dead.length, 0, 'every size scales with S/M/L: ' + dead.map(d => d[0]).join(' | '));
  /* the exemption is only valid while the circle really is a fixed size */
  EXEMPT.forEach(sel => {
    const at = css.indexOf(sel + '{');
    ok(at !== -1, sel + ' rule missing — retire its exemption above');
    ok(/width:\d+px/.test(css.slice(at, css.indexOf('}', at))),
       sel + ' no longer has a fixed width, so its font must scale with --ts');
  });
});

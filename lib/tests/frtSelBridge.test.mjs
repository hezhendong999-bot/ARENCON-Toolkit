...
t('hollow rect: perimeter hits, empty interior misses', () => {
  const s = { id:'r', type:'rect', x1:100, y1:100, x2:300, y2:250, size:4 };
  if(!hit(s, 100, 175)) throw new Error('left edge should hit');
  if(!hit(s, 200, 250)) throw new Error('bottom edge should hit');
  if(hit(s, 200, 175)) throw new Error('hollow CENTER must miss');
});
...
t('hollow triangle: edges hit, center misses (the halo lesson, applied to hits)', () => {
  const s = { id:'t3', type:'triangle', x1:100, y1:100, x2:300, y2:280, size:5 };
  if(!hit(s, 200, 280)) throw new Error('base edge should hit');
  if(hit(s, 200, 220)) throw new Error('hollow triangle center must miss');
});

t('circle: ring hits, center misses; fillcircle center hits', () => {
  const o = { id:'c', type:'circle', x1:0, y1:0, x2:200, y2:200, size:4 };
  if(!hit(o, 200, 100)) throw new Error('ring right should hit');
  if(hit(o, 100, 100)) throw new Error('hollow circle center must miss');
  const f = { id:'fc', type:'fillcircle', x1:0, y1:0, x2:200, y2:200, size:4 };
  if(!hit(f, 100, 100)) throw new Error('filled circle center should hit');
});

...
var _ringColor = null;  // S653 — from profiles.ring_color (header avatar)
...
export var Auth = {
  ...
  request: function(path, opts, _isRetry) {
    ...
    return fetch(SUPABASE_URL + path, {
      ...
      if (res.status === 401 && !_isRetry && path.indexOf('/auth/v1/') !== 0) {
        /* S730 — FAIL ONCE, SURFACE ONCE. Once a refresh has been refused by
           the auth server, every 401 used to go back through the refresh path
           again — a tablet back from a week away would 401-loop on every save
           with nothing on screen. If the session is already known dead, say
           so and stop; the status line reads the same flag. */
        if (window._authSessionExpired) {
          var _dead = new Error('Session expired \u2014 sign in again');
          _dead.status = 401; _dead.sessionExpired = true;
          return Promise.reject(_dead);
        }
      ...
      if (!res.ok) {
        return res.json().catch(function() { return { message: res.statusText }; }).then(function(err) {
          /* S730 — carry the status so a caller can tell "the server said no"
             from "the network never answered". Message text is unchanged. */
          var _e = new Error(err.message || err.msg || res.statusText);
  ...
  _refreshToken: function() {
    ...
    return this.request('/auth/v1/token?grant_type=refresh_token', {
      ...
      /* S730 — A REFUSED REFRESH IS A DEAD SESSION; AN UNANSWERED ONE IS NOT.
         The auth server answering 4xx (token expired, revoked, or rotated
         away by another tablet signed in as the same person) means no retry
         will ever succeed: set the flag the status line and request() read.
         A network failure — basement, airplane mode — has no status and is
         left alone: the session may be fine and the flag must not lie. The
         stored refresh token is never removed here (S338): a transient
         failure must not clobber a token the next attempt may still need;
         a fresh sign-in overwrites it. Nothing in IndexedDB or the outbox is
         touched — the work survives to the re-login. */
      var _st = err && err.status;
      if (typeof _st === 'number' && _st >= 400 && _st < 500) {
        try { window._authSessionExpired = true; } catch (_) {}
        console.warn('[Auth] Refresh REFUSED by auth server (' + _st + ') — session expired, sign in again:', err.message);
      } else {
        console.warn('[Auth] Refresh failed (no server answer; session state unknown):', err.message);
      }
      return null;
    });
  },
  ...
  _loadRole: function(userId) {
    ...
    /* S653: ring_color joins the same fetch. The tool headers now draw the
    account avatar in the person's own colour, so a person is one colour
    everywhere — header, board, and the ring on their pins. One request, not
    a second round trip. */
    return this.request('/rest/v1/profiles?id=eq.' + userId + '&select=role,initials,full_name,ring_color').then(function(rows) {
      if (rows && rows.length > 0) {
        _role = rows[0].role;
        _initials = rows[0].initials || null;
        _fullName = rows[0].full_name || null;
        _ringColor = rows[0].ring_color || null;
      }
  ...
  /* S653 — identity for the shared header avatar. Every tool reads these
     rather than each one fetching its own profile row. */
  getFullName: function() { return _fullName || ''; },
  getRingColor: function() { return _ringColor || '#888888'; },

/* ══════════════════════════════════════════════════════════════════════════
   ARENCON Toolkit — NFPA 20 ACCEPTANCE      lib/calc/pumpAcceptance.js  v1.1.0
   ──────────────────────────────────────────────────────────────────────────
...
/* ── 3-POINT (standard) TEST — one flow point ──────────────────────────────
   ...
   Advisories are returned as CODES with the numbers behind them, never as
   sentences (S678). The judgement is shared; the WORDING is the host's, because
   report language is a client-facing decision and changing it must never mean
   editing the file that decides whether a pump passes. Each host maps a code to
   its own sentence — identical text today, free to diverge (a diesel phrase can
   read wrong on an electric report) without a "which tool is asking" branch
   appearing in here, which is the first crack that turns a shared engine back
   into two forks. An unknown code must be dropped by the host, not printed. */
function evalStdPoint(row, ctx) {
  ...
  if (curveFlag && adj != null && !isNaN(plac)) {
    advisories.push({ code: 'curveBand', adj: adj, placard: plac });
  }
  var npsh = parseFloat(ctx.npshPsi);
  if (!isNaN(suc) && !isNaN(npsh) && npsh > 0 && suc < npsh) {
    advisories.push({ code: 'suctionBelowNpsh', npsh: npsh });
  }
  if (!isNaN(bf) && bf < 20) advisories.push({ code: 'backflowLow', bfUp: bf });
  var sr = ctx.sysRating;
  if ((row.pct === '0%') && net != null && !isNaN(suc) && sr && (net + suc) > sr) {
    advisories.push({ code: 'churnOverPressure', total: (net + suc), sysRating: sr });
  }

  return { net: net, adj: adj, ratedNet: ratedNet, gate: gate, curveFlag: curveFlag,
           advisories: advisories, override: row.overStd || '', autoVerdict: autoVerdict, verdict: eff };
}

/* ── 7-POINT (PLD) TEST — one flow point ───────────────────────────────────
   ctx: { ratedRpm, ratedNet, pldSetting, skipNoPLD, npshPsi, sysRating }
   (npshPsi and sysRating feed the advisories only — S722; absent = no advisory.)
...
function evalPldPoint(row, ctx) {
  ...
  /* S722 — the SAME four advisory codes evalStdPoint emits, so the host words
     them through one function and the two tests can never drift again. Until
     this the 7-point host hand-rolled three of them and omitted churn
     over-pressure entirely: a pump that would be flagged for over-pressuring
     the system on a 3-point test drew no warning at all on a 7-point one.
     Readings follow the host's established convention — the SCORED w/o-PLD
     leg (what the 3-point test measures by definition), falling back to the
     w/PLD leg only where the host took no w/o-PLD reading. The PLD device
     flag/fail wording has no 3-point counterpart and stays with the host. */
  var advisories = [];
  if (curveFlag && adjNet != null && !isNaN(placard)) {
    advisories.push({ code: 'curveBand', adj: adjNet, placard: placard });
  }
  var sucCheck = parseFloat(skipNoPLD ? row.suc_w : row.suc_no);
  if (isNaN(sucCheck)) sucCheck = parseFloat(row.suc_w);
  var npsh = parseFloat(ctx.npshPsi);
  if (!isNaN(sucCheck) && !isNaN(npsh) && npsh > 0 && sucCheck < npsh) {
    advisories.push({ code: 'suctionBelowNpsh', npsh: npsh });
  }
  var bf = parseFloat(row.bfUp);
  if (!isNaN(bf) && bf < 20) advisories.push({ code: 'backflowLow', bfUp: bf });
  var sucNo = parseFloat(row.suc_no), disNo = parseFloat(row.dis_no);
  var churnNet = (!isNaN(sucNo) && !isNaN(disNo)) ? (disNo - sucNo) : null;
  var sr = ctx.sysRating;
  if ((row.pct === '0%') && !skipNoPLD && churnNet != null && sr && (churnNet + sucNo) > sr) {
    advisories.push({ code: 'churnOverPressure', total: (churnNet + sucNo), sysRating: sr });
  }

  return { netNo: netNo, netW: netW, adjNet: adjNet, ratedNet: ratedNet, gate: gate,
           pldDev: pldDev, curveFlag: curveFlag, advisories: advisories, override: row.overPld || '',

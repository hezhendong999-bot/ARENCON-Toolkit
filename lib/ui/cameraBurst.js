...
function _openUI(stream, done, _sessionId, _ctx, _prep) {
  ...
  function _addShot(blob) {
    ...
    var d = {
      ...
      /* ═══ S713 — THE FILE IS SET AT BIRTH, NOT ON FAILURE. ═════════════════
         Before: d.file stayed null when the disk write SUCCEEDED, so at Done
         the only copy of every shot was a read-back from IndexedDB. 99 stills
         later that read-back met a dying tab and one photo out of a hundred
         survived. Chrome backs large blobs with disk, not RAM, so holding the
         File costs almost nothing — and a shot now exists in two places until
         the report model has actually taken it. */
  ...
  btnDone.addEventListener('click', function () {
    ...
    settled.then(function () { return _materialize(); }).then(function (recs) {
      recs.forEach(function (r) { try { r.file._burstK = r.k; } catch (_) {} });

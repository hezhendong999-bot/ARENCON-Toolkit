...
function _openUI(stream, done, _sessionId, _ctx, _prep) {
  ...
  /* S693b — the verdict key is VERSIONED. The first shipped hunt only knew
     ...
     does the .6). */
  function _wideVerdict() { try { return localStorage.getItem('arc-cam-wide-v2') || ''; } catch (e) { return ''; } }
  function _wideRemember(v) { try { localStorage.setItem('arc-cam-wide-v2', v); } catch (e) {} }
    ...
    navigator.mediaDevices.enumerateDevices().then(function (ds) {
      ...
      function tryNext() {
        if (i >= backs.length) {
          ...
          if (_cand.length) {
            var pick = null, k;
            for (k = 0; k < _cand.length; k++) { if (!_cand[k].torch && _cand[k].id !== _torchDevId) { pick = _cand[k]; break; } }
            if (!pick) { for (k = 0; k < _cand.length; k++) { if (!_cand[k].torch) { pick = _cand[k]; break; } } }
            if (!pick && _torchDevId) { for (k = 0; k < _cand.length; k++) { if (_cand[k].id !== _torchDevId) { pick = _cand[k]; break; } } }
            if (!pick) pick = _cand[0];
            trace.push('tier2 pick: ' + pick.lbl + ' (torch=' + pick.torch + ')');
            _switchToWide(pick.id, pick.lbl, trace);
            return;
          }

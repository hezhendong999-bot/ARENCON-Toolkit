...
function _openUI(stream, done, _sessionId, _ctx, _prep) {
  ...
  var btnAF = _ib('<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 7-2.6"/></svg>', 'focus locked — tap for auto focus');
  btnAF.style.display = 'none';
  btnAF.style.color = '#FFB020';
  ...
  topbar.appendChild(btnPhone); topbar.appendChild(btnAF); topbar.appendChild(btnFlash); topbar.appendChild(btnNight); topbar.appendChild(btnGrid); topbar.appendChild(btnFloat); topbar.appendChild(btnFlip);
  ...
  retLock.style.cssText = 'position:absolute;top:-20px;left:50%;transform:translateX(-50%);display:none;';
  ...
  function _attachStream(s, newFacing) {
    ...
    /* S703 — a new track has its own focus state. Carrying the lock indicator
       across a lens change would show a padlock for a lock that does not exist
       on this sensor. Clear the badge WITHOUT applying constraints to the fresh
       track: the flip already opened it in its default mode. */
    try { _s703Locked = false; retLock.style.display = 'none'; btnAF.style.display = 'none'; reticle.style.display = 'none'; } catch (e) {}
    setTimeout(_recap, 800);
    ...
    if (flashMode === 'flash' && (hasTorch || _torchDevId)) {
      ...
      _setTorch(true, function (ok) {
        ...
        setTimeout(function () {
          ...
          _s702jCapture(function () { _setTorch(false); });
        }, 320);
      });
    } else {
  ...
  /* ═══ S703 — THE TAP FOCUSES AND STAYS. ═══════════════════════════════════
     What was wrong (Mark: "it doesn't do shit"): S488 drew a box ONLY where the
     track granted `pointsOfInterest`, and this fleet reports that false on every
     camera — so there was never a mark at the finger on the tablet he actually
     holds. Where single-shot existed it hunted and then handed the camera back
     to continuous 1500ms later, which walks straight off a close placard before
     the shutter. And where neither existed the function returned in silence and
     the tap was simply eaten.

     Now: every qualifying tap draws the box, hunts, and LOCKS. No words — the
     box and the padlock carry the whole state (Mark: "I don't want these popup
     words on camera, nobody cares"). A failure flashes the box red instead of
     printing a sentence. */
  var _s703Locked = false;
  function _s703Place(clientX, clientY) {
    try {
      var rp = vidWrap.getBoundingClientRect();
      reticle.style.left = (clientX - rp.left) + 'px';
      reticle.style.top = (clientY - rp.top) + 'px';
      reticle.style.borderColor = '#FFB020';
      retLock.style.display = 'none';
  ...
  function _s703Lock() {
    ...
    try {
      ...
      reticle.style.borderColor = '#FFB020';
      reticle.style.display = 'block';
      retLock.style.display = 'block';   // the box STAYS while the lock holds
      btnAF.style.display = 'flex';
    } catch (e) {}
  }
  function _s703Fail() {
    /* No sentence. The box goes red for a moment and leaves. */
    try {
      reticle.style.borderColor = '#E26076';
      retLock.style.display = 'none';
      clearTimeout(_fh);
      _fh = setTimeout(function () {
        reticle.style.display = 'none';
        reticle.style.borderColor = '#FFB020';
      }, 900);
    } catch (e) {}
  }
  function _s703Unlock() {
    _s703Locked = false;
    try {
      retLock.style.display = 'none';
      btnAF.style.display = 'none';
    ...
    /* Hand the sensor back to whatever automatic mode it has. Continuous where
       the camera offers it; otherwise single-shot is the only automatic there
       is and the camera simply re-hunts on its own terms. */
    try {
      var mode = (caps && caps.focusMode && caps.focusMode.indexOf && caps.focusMode.indexOf('continuous') >= 0) ? 'continuous' : null;
      if (mode && track) {
        var cp = track.applyConstraints({ advanced: [{ focusMode: mode }] });
        if (cp && cp.catch) cp.catch(function () {});
      }
    } catch (e) {}
  }
  btnAF.addEventListener('click', function (e) { e.stopPropagation(); _s703Unlock(); });
  function focusAt(clientX, clientY) {
    ...
    /* S703 — capabilities are re-read AT TAP TIME. _recap does not run until
       800ms after open, so a tap in the first second used to see an empty caps
       object and take the silent return below. That is not "this camera cannot
       focus", it is "we have not asked yet". */
    if (!caps || !Object.keys(caps).length) {
      try { caps = (track && track.getCapabilities) ? track.getCapabilities() : caps; } catch (e) {}
    }
    ...
    var canPOI = !!(track && caps && caps.pointsOfInterest);
    /* S703 — THE TAP IS NEVER SILENT. The box goes down at the finger before
       anything is asked of the sensor. It means "working this spot", which is
       true on every path including the one that then fails. A tap that produces
       nothing at all is the defect being fixed here. */
    _s703Place(clientX, clientY);
    if (!canPOI && !canHunt) { _s703Fail(); return; }
    if (canPOI) {
      /* THE ONLY CASE THAT EARNS A RETICLE AT THE FINGER: the track accepts a
         point of interest, so this really is "focus HERE". Normalise against
         the VIDEO'S VISIBLE BOX, not the overlay's CSS box — the feed is
         object-fit cover, so the two differ whenever the sensor's aspect ratio
         does not match the element, and using the wrong one aims the sensor at
         the wrong part of the scene. */
      var vr = video.getBoundingClientRect();
      var nx = Math.max(0, Math.min(1, (clientX - vr.left) / (vr.width || 1)));
      var ny = Math.max(0, Math.min(1, (clientY - vr.top) / (vr.height || 1)));
      try {
        var pp = track.applyConstraints({ advanced: [{ pointsOfInterest: [{ x: nx, y: ny }] }] });
        if (pp && pp.then) {
          /* S706 — same best-effort trap as focusMode. A resolve here proves
             nothing, so the hunt is run anyway where it is available: the point
             aims the sensor, single-shot makes it actually go and hold. */
          pp.then(function () { if (canHunt) _s703Hunt(canCont); else _s703Lock(); },
                  function () { if (canHunt) _s703Hunt(canCont); else _s703Fail(); });
        } else { if (canHunt) _s703Hunt(canCont); else _s703Lock(); }
      } catch (e) {
        if (canHunt) _s703Hunt(canCont); else _s703Fail();
      }
      return;
    }
    _s703Hunt(canCont);
  }
  /* ═══ S706 — WHY THE BOX APPEARED AND NOTHING FOCUSED. ═════════════════════
     Mark, on the tablet: "the tap to focus box shows up but it doesn't do any
     focus at all. It's only showing as visual but useless." Two mechanisms,
     both real, both invisible from the repository:

     1. `applyConstraints({ advanced: [...] })` is BEST-EFFORT by specification.
        A browser is allowed to ignore an entry it cannot satisfy and still
        RESOLVE the promise successfully. So the old success path fired on a
        camera that had done nothing — the padlock lit for a lock that did not
        exist. An apply is now only believed when `getSettings()` agrees.

     2. Re-applying the SAME mode is not a state change. After S703 the first
        tap left the track latched in 'single-shot'; every tap after that asked
        for 'single-shot' again, the driver saw no transition, and no new hunt
        ever started. So focus appeared to work once and then never again —
        which reads exactly like "it never works". The camera is now stepped
        OUT of single-shot before being put back into it, which is what forces
        a fresh hunt.

     Where `getSettings()` does not expose focusMode at all we cannot verify,
     and we say so by trusting the resolve — that is the ceiling of what the
     web can tell us, not a claim. */
  function _s703Mode() {
    try { var s = (track && track.getSettings) ? track.getSettings() : {}; return s.focusMode || ''; } catch (e) { return ''; }
  }
  function _s703CanRead() {
    try { var s = (track && track.getSettings) ? track.getSettings() : {}; return ('focusMode' in s); } catch (e) { return false; }
  }
  function _s703Apply(mode) {
    return new Promise(function (resolve) {
      if (!track) { resolve(false); return; }
      var verify = function (accepted) {
        if (!_s703CanRead()) { resolve(!!accepted); return; }   // unverifiable — trust the accept
        resolve(_s703Mode() === mode);
      };
      var basic = function () {
        /* A NON-advanced constraint must reject when it cannot be satisfied,
           so it is the honest second ask when the best-effort one did nothing. */
        try {
          var pb = track.applyConstraints({ focusMode: mode });
          if (pb && pb.then) pb.then(function () { verify(true); }, function () { resolve(false); });
          else verify(true);
        } catch (e) { resolve(false); }
      };
      try {
        var pa = track.applyConstraints({ advanced: [{ focusMode: mode }] });
        var after = function () { if (_s703CanRead() && _s703Mode() !== mode) basic(); else verify(true); };
        if (pa && pa.then) pa.then(after, basic); else after();
      } catch (e) { basic(); }
    });
  }
  function _s703Hunt(canCont) {
    var run = function () {
      _s703Apply('single-shot').then(function (ok) { if (ok) _s703Lock(); else _s703Fail(); });
    };
    if (_s703Mode() === 'single-shot') {
      var hasManual = !!(caps && caps.focusMode && caps.focusMode.indexOf && caps.focusMode.indexOf('manual') >= 0);
      var away = canCont ? 'continuous' : (hasManual ? 'manual' : '');
      if (away) { _s703Apply(away).then(function () { setTimeout(run, 60); }); return; }
    }
    run();
  }
  (function () {
    ...
    expo.addEventListener('touchend', function () { expDragging = false; if (_s703Locked) return; _fh = setTimeout(function () { reticle.style.display = 'none'; }, 2500); }, { passive: true });
  })();

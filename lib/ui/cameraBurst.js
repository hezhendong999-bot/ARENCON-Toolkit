...
function _openUI(stream, done, _sessionId, _ctx, _prep) {
  ...
  function focusAt(clientX, clientY) {
    ...
    if (canPOI) {
      ...
      try {
        ...
        if (pp && pp.then) {
          pp.then(function () { _s703Lock(); }, function () {
            /* Granted in capabilities but refused in practice. Fall to the hunt
               rather than give up — single-shot may still be available. */
  ...
  /* S703 — the hunt, and then the HOLD. The 1500ms hand-back to continuous is
     deliberately gone (owner override, S703): continuous AF walks off a close
     placard between the tap and the shutter, which is the whole reason the tap
     felt like it did nothing. The camera now stays where it was put until the
     next tap or until the AF button in the top bar is pressed. */
  function _s703Hunt(canCont) {
    try {
      var hp = track.applyConstraints({ advanced: [{ focusMode: 'single-shot' }] });
      if (hp && hp.then) hp.then(function () { _s703Lock(); }, function () { _s703Fail(); });
      else _s703Lock();
    } catch (e) {
      _s703Fail();
    }

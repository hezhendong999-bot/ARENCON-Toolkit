...
function _openUI(stream, done, _sessionId, _ctx, _prep) {
  ...
  function applyZoom() {
    ...
    if (hasHwZoom && track) {
      ...
      if (!_zoomPending) { _zoomPending = true; _zoomRaf = requestAnimationFrame(function () { _zoomPending = false; var hz = Math.min(caps.zoom.max, Math.max(caps.zoom.min, zoom)); try { var zp = track.applyConstraints({ advanced: [{ zoom: hz }] }); var landed = function () { _hwZoomNow = hz; _s702ClearInterimScale(); if (flashMode === 'torch') { try { var tp = track.applyConstraints({ advanced: [{ torch: true }] }); if (tp && tp.catch) tp.catch(function () {}); } catch (e) {} } }; if (zp && zp.then) zp.then(landed, landed); else landed(); } catch (e) { _hwZoomNow = hz; video.style.transform = 'none' + _mirror(); } }); }
  ...
  function focusAt(clientX, clientY) {
    ...
    /* S702 — the exposure slider no longer lives inside this box, so the box no
       longer has to exist for the slider's sake. If the tap cannot reach the
       sensor, nothing is drawn at the finger. */
    if (!canPOI && !canHunt) return;
    if (canPOI) {
      ...
      try {
        ...
        if (pp && pp.catch) pp.catch(function () {
          /* Granted in capabilities but refused in practice: withdraw the box
             rather than leave a mark implying a focus that did not happen. */
          reticle.style.display = 'none';
          _s702Say('Focus here is not available on this camera');
        });
      } catch (e) {
        reticle.style.display = 'none';
        _s702Say('Focus here is not available on this camera');
      }
      clearTimeout(_fh); _fh = setTimeout(function () { reticle.style.display = 'none'; }, 3500);
      return;
    }
    /* Re-hunt only. The camera refocuses on whatever it is pointed at — it does
       NOT focus where the finger landed, so no mark is drawn there. Saying so
       plainly is the whole point; a box at the finger would be a claim the
    ...
    try {
      ...
      if (hp && hp.then) {
        hp.then(back, function () {
          /* S702 — a rejected apply used to run the same path as success, so a
             failed refocus looked identical to a working one. */
          _s702Say('Refocus failed on this camera');
        });
      } else back();
    } catch (e) {
      _s702Say('Refocus failed on this camera');

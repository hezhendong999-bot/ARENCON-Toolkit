...
/* S735d — width/height from the JPEG SOF marker in the first 256 KB, no decode.
   Resolves null when the file is not a JPEG or the marker is not found. */
function _jpegDims(blob) {
  try {
    return blob.slice(0, 262144).arrayBuffer().then(function (buf) {
      ...
      while (o + 9 < n) {
        ...
        var mk = v.getUint8(o + 1);
        if (mk === 0xFF) { o++; continue; }
        if (mk === 0xD8 || (mk >= 0xD0 && mk <= 0xD7) || mk === 0x01) { o += 2; continue; }
        if (mk === 0xD9 || mk === 0xDA) return null;
        var len = v.getUint16(o + 2);
        if ((mk >= 0xC0 && mk <= 0xCF) && mk !== 0xC4 && mk !== 0xC8 && mk !== 0xCC) {
          return { h: v.getUint16(o + 5), w: v.getUint16(o + 7) };
        }
        o += 2 + len;
      }
      return null;
...
export function openCameraBurst(ctx) {
  ...
  return new Promise(function(resolve) {
    ...
    _open = true;
    /* S735 — TELL THE PAGE BEFORE THE HARDWARE OPENS. The drawing viewer
       underneath holds the tile cache and level canvases (hundreds of MB on a
       tablet at deep zoom). Android killed the whole app when the camera's
       buffers landed on top of that (Nasim, 7310.17, 22 Sep: Hub + FRT boot in
       the same second, zero client_error = process death). The viewer
       releases that memory on this event and rebuilds on 'arc-camera-closed'.
       Every exit below — success, refusal, a UI throw — announces the close. */
    try { window.dispatchEvent(new CustomEvent('arc-camera-opening')); } catch (eEvO) {}
    ...
    var _s702Req = _s702Back
      ? { video: { deviceId: { exact: _s702Back }, width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 } }, audio: false }
      : { video: { facingMode: { ideal: 'environment' }, width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 } }, audio: false };
    ...
    /* S735 — the cap stays, the landscape lock goes. Every live request now
       asks for width AND height { ideal: 2048, max: 2048 }. The old
       ideal 2048x1536 named a LANDSCAPE shape; turning the tablet to portrait
       asked the sensor for the opposite aspect and forced a reconfigure (the
       rotation freeze / crash family, Nasim 22 Sep). A square target is
       orientation-neutral and still lands on the same 2048x1536 mode in the
       usual case; max keeps the memory cap. NEVER drop the `ideal`: with only
       `max`, Chrome picks its own default of 640x480 and every photo shrinks
       to 0.3 MP without an error. */
    navigator.mediaDevices.getUserMedia(_s702Req).catch(function (e) {
      ...
      return navigator.mediaDevices.getUserMedia({
        video: { facingMode: { ideal: 'environment' }, width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 } },
        audio: false
      });
      ...
      try {
        ...
        _open = false;
        try { window.dispatchEvent(new CustomEvent('arc-camera-closed')); } catch (eEvC) {}   /* S735 */
        resolve(null);
      }
    }).catch(function() {
      _removeStartingOverlay();
      _open = false;
      try { window.dispatchEvent(new CustomEvent('arc-camera-closed')); } catch (eEvC2) {}   /* S735 */
      resolve(null);
    });
  });
}
...
function _openUI(stream, done, _sessionId, _ctx, _prep) {
  ...
  _ffMsg.querySelector('#cam-ff-retry').addEventListener('click', function (ev) {
    ...
    navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, facingMode: facing }, audio: false })
      .then(function (s) {
        ...
        try { if (track && !track.muted) _hideAdjusting(); } catch (eHm) {}   /* S735 — no auto-hide any more */
  ...
  function _s702EnsureFocusableBack() {
    ...
    /* ═══ S718b — THE HUNT BELOW MUST NEVER RUN ON iOS. ══════════════════════
       Field report 1 Sep 2026 (Mark, iPhone): the burst preview paints for
       about a second and then goes black, every open, while the camera
       indicator stays lit. Android is unaffected.

       Why: this hunt asks the open track whether it can autofocus. Safari does
       not report focusMode AT ALL, so on every iPhone the answer is "no" and
       the hunt always starts. It then opens a SECOND back lens to compare —
       and iOS permits only one capture at a time, so acquiring the second one
       stops the first. The second lens reports no focus capability either
       (nothing on iOS ever does), so the code discards it and keeps "the
       working stream", which by then is dead. Black, with no error.

       The hunt tests for a capability iOS never exposes: it cannot succeed
       there, so it can only do damage. Skipped, tried-flag set so nothing
       retries it. Android/Chrome — where focusMode IS reported, and where the
       two-back-camera problem this was built for actually exists — takes the
       identical path it takes today. This file is shared with Diesel and
       Electric; the guard is platform-scoped, so their behaviour is unchanged
       on every device that was working. */
    if (_isIOSDev) {
      _s702BackTried = true;
      try { console.info('[CamBurst S718b] iOS — skipping the autofocus lens hunt (focusMode is never reported on Safari; probing a second lens would stop the live stream).'); } catch (e) {}
      return;
    }
    if (_s702CanAF(caps)) {
    ...
    navigator.mediaDevices.enumerateDevices().then(function (ds) {
      ...
      navigator.mediaDevices.getUserMedia({
        video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, deviceId: { exact: backs[0].deviceId } },
        audio: false
      }).then(function (s2) {
  ...
  function _acquireTorchTrack(cb) {
    ...
    navigator.mediaDevices.enumerateDevices().then(function (ds) {
      ...
      function tryNext() {
        ...
        navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, deviceId: { exact: id } }, audio: false })
          .then(function (s) {
  ...
  function _leaveWideLens(cb) {
    ...
    var _req = _torchDevId
      ? navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, deviceId: { exact: _torchDevId } }, audio: false })
      : navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, facingMode: 'environment' }, audio: false });
  ...
  function _attachStream(s, newFacing) {
    ...
    /* S735 — the overlay no longer times out on its own, so a new track that
       arrives live must clear one the old track left up. */
    try { if (track && !track.muted) _hideAdjusting(); } catch (eHm) {}
  ...
  function _resGuard() {
    function check() {
      ...
      if (w && w < 2000 && track) {
        try {
          var p = track.applyConstraints({ width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 } });
          if (p && p.catch) p.catch(function () {});
        } catch (e) {}
      }
    }
  ...
  function flip() {
    ...
    var _req = (next === 'environment' && _torchDevId)
      ? navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, deviceId: { exact: _torchDevId } }, audio: false })
      : navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, facingMode: { exact: next } }, audio: false });
    _req
      .then(function (s) { _attachStream(s, next); })
      .catch(function () {
        navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 2048, max: 2048 }, height: { ideal: 2048, max: 2048 }, facingMode: next }, audio: false })
          .then(function (s) { _attachStream(s, next); })
  ...
  function _preview(blob) {
    /* ═══ S735d — THE COMMON SHOT NEEDS NO REVIEW COPY. ═══════════════════════
       A burst grab is 2048 wide; the review cap is 2560. So for every ordinary
       shot this function decoded the photograph (12 MB), drew it onto a
       full-size canvas (12 MB), re-encoded it at the SAME size, made a new
       object URL, and left both canvases for the garbage collector — which
       does not see canvas memory and so does not hurry. Three taps a second
       on a phone is how that becomes "Aw, Snap". Now: size from the JPEG
       header, no decode; if the shot already fits the cap, the browser
       decodes it scaled straight to 480 for the thumb and the original URL
       stays as the review copy. Only a larger file (the Phone button's 12 MP)
       takes the old path, and that path now frees its canvases explicitly. */
    if (typeof window.createImageBitmap === 'function') {
      return _jpegDims(blob).then(function (dims) {
        var mx = dims ? Math.max(dims.w, dims.h) : 0;
        if (!dims || mx > 2560) return _previewFull(blob);
        var rw = Math.max(1, Math.round(Math.min(dims.w, dims.h) * Math.min(1, 480 / mx)));
        return createImageBitmap(blob, { resizeWidth: rw, resizeQuality: 'medium' }).then(function (bmp) {
          var thumb = null;
          try {
          ...
          try { bmp.close(); } catch (_c) {}
          return { url: null, thumb: thumb };           // url:null = keep the original URL
        }).catch(function () { return _previewFull(blob); });
      });
    }
    return _previewFull(blob);
  }
  function _previewFull(blob) {
    return new Promise(function (resolve) {
      var url, img;
      ...
      img.onload = function () {
        try {
          ...
          try {
            ...
            thumb = tc.toDataURL('image/jpeg', 0.7);
            if (!thumb || thumb.length > 200000) thumb = null;
            tc.width = 0; tc.height = 0;                    // S735d — free it now, not at GC
          } catch (_t) { thumb = null; }
          try { URL.revokeObjectURL(url); } catch (e2) {}
          c.toBlob(function (b) {
            c.width = 0; c.height = 0;                        // S735d — free it now, not at GC
            var u = null;
    ...
    var _thisPut = _bPut({ k: d.k, blob: blob, name: name, type: type, at: Date.now(), session: _bsid, handedOff: false, ctx: _ctx })
      .then(function (ok) {
        ...
        return _preview(blob).then(function (pv) {
          ...
          if (!pv.url) return;                   // S735d — shot already fits the cap: the original IS the review copy
  ...
  /* S735 — "Adjusting…" means ONE thing: the camera track is muted (the
     hardware is not delivering frames). It used to appear on every rotation
     with a 1.2 s floor, covering a live feed that was fine; and every 'mute'
     re-armed that timer, so a real stall was hidden after 1.2 s. Now: shown
     while muted, hidden the instant 'unmute' fires, no timer at all. The
     S487n arrived-muted path below and the 3 s first-frame watchdog are
     unchanged — they are what escalates a stall that never recovers. */
  function _showAdjusting() { _adjust.style.display = 'flex'; if (_adjustTimer) { clearTimeout(_adjustTimer); _adjustTimer = null; } }
  ...
  function _onOrient() { _updateStageAspect(); }
  try {
  ...
  function _close(result) {
    ...
    overlay.remove();
    /* S733 — tell the page the camera is gone. The drawing viewer defers its
       rotation refit while #cam-burst-overlay is up (a full pin + markup +
       tile re-render under a live video froze the camera for seconds on
       every rotation); this is how it learns it is safe to catch up. */
    try { window.dispatchEvent(new CustomEvent('arc-camera-closed')); } catch (eEv) {}

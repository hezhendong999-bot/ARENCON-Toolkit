...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
var _BDB_NAME = 'ARENCON_BURST', _BDB_VER = 1, _BDB_STORE = 'shots';
...
function _bTx(mode, fn) {
  return _bOpen().then(function (db) {
    if (!db) return null;
    return new Promise(function (resolve) {
      var tx, out = null;
      try { tx = db.transaction(_BDB_STORE, mode); } catch (e) { resolve(null); return; }
      try { out = fn(tx.objectStore(_BDB_STORE)); } catch (e) { resolve(null); return; }
...
function _bPut(rec) { return _bTx('readwrite', function (st) { return st.put(rec); }).then(function (r) { return r != null; }); }
function _bGet(k)   { return _bTx('readonly',  function (st) { return st.get(k); }).then(function (r) { return r || null; }); }
function _bDel(k)   { return _bTx('readwrite', function (st) { return st['delete'](k); }); }
function _bAll()    { return _bTx('readonly',  function (st) { return st.getAll ? st.getAll() : null; }).then(function (r) { return r || []; }); }
...
function _bMarkHandedOff(keys) {
  ...
  keys.forEach(function (k) {
    chain = chain.then(function () {
      return _bGet(k).then(function (rec) {
        if (!rec) return;
...
function _bSweep(currentSession) {
  return _bAll().then(function (rows) {
    ...
    (rows || []).forEach(function (r) {
      ...
      if (age < 60000) return;   // another tab/window may still be shooting
      orphans.push(r);
    });
...
function _openUI(stream, done) {
  ...
  var _bsid = 'b' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
  var shots = [];
  ...
  function _addShot(blob) {
    ...
    _updateUI();
    _bPut({ k: d.k, blob: blob, name: name, type: type, at: Date.now(), session: _bsid, handedOff: false })
      .then(function (ok) {
        if (!ok) {
  ...
  function _confirmDiscard(n, onYes) {
    ...
    back.style.cssText = 'position:absolute;inset:0;z-index:12;background:rgba(11,10,13,.86);display:flex;align-items:center;justify-content:center;padding:24px;font-family:Calibri,sans-serif;';
    ...
    h.textContent = 'Discard ' + n + (n === 1 ? ' photo?' : ' photos?');
    var p = document.createElement('div');
    p.style.cssText = 'font-size:15px;line-height:1.45;color:#a09aa8;margin-bottom:20px;';
    p.textContent = (n === 1 ? 'This photo has' : 'These ' + n + ' photos have') +
    ...
    keep.textContent = 'Keep shooting';
    ...
    kill.textContent = 'Discard ' + n + (n === 1 ? ' photo' : ' photos');
    ...
    kill.addEventListener('click', function () { _dismiss(); onYes(); });
  }
  ...
  function _showRecoverBar(orphans) {
    ...
    txt.textContent = orphans.length + (orphans.length === 1
      ? ' photo from an interrupted session is still on this device'
      : ' photos from an interrupted session are still on this device');
    ...
    add.addEventListener('click', function () {
      ...
      var chain = Promise.resolve();
      orphans.forEach(function (rec) {
        chain = chain.then(function () {
          if (!rec || !rec.blob) return null;
          var d = { k: rec.k, name: rec.name || ('recovered_' + rec.k + '.jpg'),
                    type: rec.type || 'image/jpeg', size: rec.blob.size || 0, file: null };
          shots.push(d);
  ...
  _bSweep(_bsid).then(function (orphans) {

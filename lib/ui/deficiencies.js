...
// ════════════════════════════════════════════════════════════════════════════
// S502 (Mark-locked): FRT S263 status pill, ported verbatim (look + behaviour).
// ONE tappable pill; text is always "Outstanding" for open items, COLOUR carries
// priority (red=high / amber=low). Tap → coloured popover with two groups:
// Outstanding (High/Low) and Other (Recommendation · Site Record · Closed), plus
// ↩ Reopen only when closed. No separate priority control.
//
// DATA MAPPING (locked): the 5 choices write Diesel's flat record as —
//   high   → status:'open',     priority:'high'
//   low    → status:'open',     priority:'low'
//   rec    → isRecommendation:true   (status/priority preserved)  [classification]
//   site   → isSiteRecord:true       (status/priority preserved)  [classification]
//   closed → status:'resolved'
// rec/site are per-item CLASSIFICATION flags (Option 1, Mark S501): the item
// NEVER moves group; it stays under its contractor. New fields isRecommendation /
// isSiteRecord round-trip through cloud sync like any other deficiency field.
// ════════════════════════════════════════════════════════════════════════════
var _CV_CHOICES=[
...
// Which choice is this deficiency currently in? (FRT precedence)
function _deficChoice(d){
  if(d.status==='resolved') return 'closed';
  if(d.isRecommendation) return 'rec';
  if(d.isSiteRecord) return 'site';
  return (_deficPriNorm(d.priority||'high')==='low') ? 'low' : 'high';
}
// S510 (Mark): a Recommendation and a Site Record are not contractor work items —
// nobody is being asked to fix anything, so neither may collect a Contractor or an
// ARENCON response. Only a true deficiency can. Note this gates the ADD buttons
// only: any response already recorded against an item that was later reclassified
// stays visible, because silently hiding recorded field commentary would look
// exactly like data loss to the inspector who wrote it.
function _canAddResponse(d){ return !(d && (d.isRecommendation || d.isSiteRecord)); }
function _deficChoiceMeta(ch){ for(var k=0;k<_CV_CHOICES.length;k++){ if(_CV_CHOICES[k].choice===ch) return _CV_CHOICES[k]; } return _CV_CHOICES[0]; }
function _deficChoiceColorVar(ch){
  return ({high:'var(--b-high)',low:'var(--b-low)',rec:'var(--b-cat-rec-fg)',site:'var(--b-cat-site-fg)',closed:'var(--b-cat-closed-fg)'})[ch]||'var(--b-high)';
}
// The pill (collapsed row control). scope = 'c:<safe>:<i>' or 'g:<i>' so one
// handler serves both card renderers.
/* S730 — OWNER CHIP, host-provided. A multi-machine host (the multi-pump
   shell) needs each deficiency to say which machine it is about — a work
   order against neither pump in a two-pump room cannot be filled. The
   engine does not know what machines exist, so it asks the host: if the
   host defines _deficOwnerHook(d, scope) the returned markup is drawn next
   to the status pill; if not, nothing is drawn and the single-pump tools
   render exactly as before. Scoped by presence, never by tool name. */
function _deficOwnerChip(d, scope){
  return (typeof _deficOwnerHook==='function') ? (_deficOwnerHook(d, scope)||'') : '';
}
function _deficStatusPill(d, scope){
  var ch=_deficChoice(d), m=_deficChoiceMeta(ch);
  return '<span class="cv-pill-anchor">'
    +'<button type="button" class="dfx-or-chip cv-pill '+m.cls+'" data-defic-scope="'+scope+'" data-cur="'+ch+'" '
    +'onclick="_deficTogglePillMenu(this)">'+m.pill+' <span class="cv-pill-caret">▼</span></button>'
    +'</span>';
}
// The shared popover (one per document, appended lazily).
function _deficEnsureMenu(){
  var m=document.getElementById('cv-defic-menu');
  if(m) return m;
  var h='<div class="cv-statusmenu" id="cv-defic-menu" role="menu">';
  var last='';
  _CV_CHOICES.forEach(function(c){
    if(c.group!==last){ h+='<div class="cv-statusmenu-label">'+c.group+'</div>'; last=c.group; }
    h+='<button type="button" class="cv-statusmenu-item '+c.cls+'" data-choice="'+c.choice+'" onclick="_deficPickStatus(\''+c.choice+'\')"><span class="cv-statusmenu-dot"></span>'+c.menu+'</button>';
  });
  h+='<div class="cv-statusmenu-sep" id="cv-defic-reopen-sep" style="display:none;border-top:1px solid var(--border);margin:2px 4px;"></div>';
  h+='<button type="button" class="cv-statusmenu-item" id="cv-defic-reopen" data-choice="reopen" style="display:none;color:#2C7FB8;" onclick="_deficPickStatus(\'reopen\')"><span class="cv-statusmenu-dot"></span>↩ Reopen</button>';
  h+='</div>';
  var wrap=document.createElement('div'); wrap.innerHTML=h;
  document.body.appendChild(wrap.firstChild);
  return document.getElementById('cv-defic-menu');
}
var _deficMenuPill=null;
function _deficTogglePillMenu(pill){
  var menu=_deficEnsureMenu();
  if(_deficMenuPill===pill && menu.classList.contains('open')){ _deficCloseMenu(); return; }
  _deficMenuPill=pill;
  var cur=pill.dataset.cur;
  menu.querySelectorAll('.cv-statusmenu-item').forEach(function(it){ it.classList.toggle('sel', it.dataset.choice===cur); });
  var showReopen=(cur==='closed');
  document.getElementById('cv-defic-reopen-sep').style.display=showReopen?'block':'none';
  document.getElementById('cv-defic-reopen').style.display=showReopen?'flex':'none';
  menu.classList.add('open');
  var r=pill.getBoundingClientRect(), mw=menu.offsetWidth;
  var left=Math.max(8, Math.min(r.left, window.innerWidth-mw-10));
  menu.style.left=left+'px'; menu.style.top=(r.bottom+6)+'px';
  document.querySelectorAll('.cv-pill').forEach(function(x){ x.classList.remove('cv-pill-active'); });
  pill.classList.add('cv-pill-active');
}
function _deficCloseMenu(){
  var menu=document.getElementById('cv-defic-menu');
  if(menu) menu.classList.remove('open');
  document.querySelectorAll('.cv-pill').forEach(function(x){ x.classList.remove('cv-pill-active'); });
  _deficMenuPill=null;
}
document.addEventListener('click', function(e){
  if(!e.target.closest('#cv-defic-menu') && !e.target.closest('[data-defic-scope]')) _deficCloseMenu();
});
// Resolve a scope string back to its deficiency object.
function _deficByScope(scope){
  if(scope[0]==='g'){ var gi=parseInt(scope.slice(2),10); return {d:generalDeficiencies[gi], kind:'g'}; }
  var rest=scope.slice(2), p=rest.lastIndexOf(':');
  var name=rest.slice(0,p), i=parseInt(rest.slice(p+1),10);
  return {d:(deficiencies[name]&&deficiencies[name][i]), kind:'c', name:name, i:i};
}
// Apply a chosen status to the pill's deficiency, write the mapped data, re-render.
function _deficPickStatus(choice){
  if(!_deficMenuPill) return;
  var ref=_deficByScope(_deficMenuPill.getAttribute('data-defic-scope'));
  var d=ref.d; if(!d){ _deficCloseMenu(); return; }
  if(choice==='reopen') choice='high';
  // reset classification/status, then set the chosen one
  d.isRecommendation=false; d.isSiteRecord=false;
  if(choice==='high'){ d.status='open'; d.priority='high'; }
  else if(choice==='low'){ d.status='open'; d.priority='low'; }
  else if(choice==='rec'){ if(d.status==='resolved') d.status='open'; d.isRecommendation=true; }
  else if(choice==='site'){ if(d.status==='resolved') d.status='open'; d.isSiteRecord=true; }
  else if(choice==='closed'){ d.status='resolved'; }
  _deficCloseMenu();
...
function addDeficiencyForContractor(name) {
  ...
  deficiencies[name].push({ description:'', date:'', iarStatus:false, status:'open', priority:'high', isRecommendation:false, isSiteRecord:false, photos:[], responses:[] });
  renderDeficGroup(name);
  updateDeficSummary();
}

/* S730 — every removal confirms first (toolkit rule: destructive actions
   confirm, one tap, custom modal, never the browser's). The dialog is the
   host's _aConfirm — the same contract lib/ui/lightbox.js uses. A host with
   no confirm blocks the removal rather than performing it unasked. The
   removed entry is a tombstone, kept in the record and not printed; there is
   no undo on the screen, and the message says so. */
function _deficConfirmRemove(kind, a, b){
  var go = function(){ if(kind==='g') removeGeneralDeficItem(a); else removeDeficItem(a, b); };
  var msg = 'Remove this deficiency from the report? It is kept as a deleted record and will not be shown or printed. There is no undo on this screen.';
  if(typeof _aConfirm==='function'){ _aConfirm(msg, go, 'Remove'); return; }
  try{ console.error('[deficiencies] no confirm host \u2014 removal blocked'); }catch(_){}
}
function removeDeficItem(name, idx) {
  /* S605 — deletion is an EVENT, not an absence. A spliced item simply
     vanishes from this device's copy; under absence-never-deletes (doctrine
     I-2) every other device keeps theirs and the delete never propagates —
     Mark's 03-Aug field test. A tombstoned item carries its own deletion to
     every device through the ordinary merge. Renders and counts skip it. */
  if (deficiencies[name] && deficiencies[name][idx]) { deficiencies[name][idx].deleted = true; deficiencies[name][idx].delAt = new Date().toISOString(); }
  renderDeficGroup(name);
  updateDeficSummary();
}
...
function renderDeficGroup(name) {
  ...
  if (!el) return;
  /* S699 — tombstoned items never render. S605 made deletion an EVENT and its
     note said "renders and counts skip it"; the counts did (updateDeficSummary
     filters) but this list did not — it drew every item and the header counted
     them, so a deleted deficiency stayed on screen and in the "N items" tally
     until a reload rebuilt the panel. Index `i` still comes from the ORIGINAL
     array so every edit/delete binding keeps addressing the right item. */
  const items = (deficiencies[name] || [])
    .map(function(d, i){ return { d:d, i:i }; })
    .filter(function(o){ return o.d && !o.d.deleted; });
  const safe = name.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
  ...
  if (items.length === 0) {
    ...
    items.forEach((o) => { html += buildDeficItem(name, safe, o.d, o.i); });
  }
...
function buildDeficItem(name, safe, d, i) {
  ...
  resps.forEach(function(cr, ci) {
    ...
    respHtml += '<div style="margin-top:8px;padding:10px 12px;background:'+bgCol+';border:1px solid '+borderCol+';border-left:3px solid '+accentCol+';border-radius:6px;">'
      ...
      +'<button onclick="deficiencies[\''+safe+'\']['+i+'].responses['+ci+'].deleted=true;deficiencies[\''+safe+'\']['+i+'].responses['+ci+'].delAt=new Date().toISOString();renderDeficGroup(\''+name+'\');" style="padding:2px 8px;border:1px solid #ddd;border-radius:4px;background:white;color:#999;font-size:10px;cursor:pointer;">✕ Remove</button></div>'
  ...
  // Two add buttons together — deficiencies only (S510)
  if(_canAddResponse(d)){
  ...
  return '<div style="border-top:1px solid var(--border);padding:14px 16px;">'
    ...
    +'<div title="'+_deficChoice(d)+'" style="min-width:28px;width:28px;height:28px;border-radius:50%;background:'+_deficChoiceColorVar(_deficChoice(d))+';color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex-shrink:0;">'+(i+1)+'</div>'
    +'<div style="flex:1;min-width:0;">'
    +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap;">'
    +'<span style="font-size:13px;font-weight:600;color:var(--steel);">Deficiency '+(i+1)+'</span>'
    +'<span style="flex:1;"></span>'
    +_deficStatusPill(d, 'c:'+safe+':'+i)
    +_deficOwnerChip(d, 'c:'+safe+':'+i)
    +'</div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">'
    +'<div>'
    ...
    +'<button onclick="event.stopPropagation();deficUpload(\''+safe+'\','+i+')" style="padding:5px 10px;border:none;border-radius:5px;background:#4F6788;color:white;font-size:calc(12.5px + var(--ts));cursor:pointer;">📎 Upload</button>'
    ...
    +'<div><label style="font-size:11px;color:transparent;display:block;margin-bottom:2px;">.</label><button onclick="_deficConfirmRemove(\'c\',\''+safe+'\','+i+')" style="padding:5px 8px;border:1.5px solid #ddd;border-radius:5px;background:white;color:#aaa;font-size:11px;cursor:pointer;">✕ Remove</button></div></div>'
    +'</div></div>'
    + respHtml
    +'</div>';
}
...
function _checklistFindings(){
  /* S730 — a host whose checklist answers do not live in one clState (the
     multi-pump shell: a room review plus one frame per machine) supplies the
     rows itself. Same shape — {id,num,text,photos,comment}. Absent host hook,
     the roll-up reads clState exactly as it always has. */
  if(typeof _checklistFindingsHost==='function') return _checklistFindingsHost()||[];
...
function _jumpToChecklistItem(id){
  /* S730 — a host that supplied the findings rows also knows where they
     live; if it takes the jump, it is done. */
  if(typeof _jumpToChecklistItemHost==='function' && _jumpToChecklistItemHost(id)) return;
...
function updateDeficSummary() {
  ...
  const rows = contractors.map(name => {
    const items = (deficiencies[name] || []).filter(function(d){ return !(d && d.deleted); });   // S605 tombstoned items never count
  ...
  if(generalDeficiencies.length) {
    var _gLive=generalDeficiencies.filter(function(d){return !(d&&d.deleted);}); var gTot=_gLive.length, gOpen=_gLive.filter(function(d){return !d.status||d.status==='open';}).length, gRes=_gLive.filter(function(d){return d.status==='resolved';}).length;
...
function addGeneralDeficiency() {
  generalDeficiencies.push({ description:'', date:'', iarStatus:false, status:'open', priority:'high', isRecommendation:false, isSiteRecord:false, photos:[], responses:[] });
  renderGeneralDeficGroup();
  updateDeficSummary();
}

function removeGeneralDeficItem(idx) {
  if (generalDeficiencies[idx]) { generalDeficiencies[idx].deleted = true; generalDeficiencies[idx].delAt = new Date().toISOString(); }   // S605 tombstone, see removeDeficItem
  renderGeneralDeficGroup();
  updateDeficSummary();
}
...
function renderGeneralDeficGroup() {
  ...
  generalDeficiencies.forEach(function(d, i) {
    if(d && d.deleted) return;   // S605 tombstoned
    html += buildGeneralDeficItem(d, i); });
...
function buildGeneralDeficItem(d, i) {
  ...
  (d.responses || []).forEach(function(cr, ci) {
    if(cr && cr.deleted) return;   // S605 tombstoned
    var party = cr.party || 'contractor';
    var isContractor = party === 'contractor';
    var accentCol = isContractor ? '#E67E22' : '#1565C0';
    var bgCol = isContractor ? '#fffbf0' : '#f0f4ff';
  ...
  return '<div style="border-top:1px solid var(--border);padding:14px 16px;"><div style="display:flex;gap:10px;align-items:flex-start;"><div style="min-width:28px;width:28px;height:28px;border-radius:50%;background:'+_deficChoiceColorVar(_deficChoice(d))+';color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex-shrink:0;" title="'+_deficChoice(d)+'">G'+(i+1)+'</div><div style="flex:1;min-width:0;"><div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap;"><span style="font-size:13px;font-weight:600;color:var(--steel);">Deficiency G'+(i+1)+'</span><span style="flex:1;"></span>'+_deficStatusPill(d,'g:'+i)+_deficOwnerChip(d,'g:'+i)+'</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;"><div><div style="font-size:11px;font-weight:600;color:var(--steel);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">Describe Deficiency</div><textarea placeholder="Describe deficiency..." style="width:100%;height:150px;border:1.5px solid var(--border);border-radius:6px;padding:8px;font-size:13px;font-family:inherit;resize:vertical;box-sizing:border-box;" oninput="generalDeficiencies['+i+'].description=this.value">'+(d.description||'')+'</textarea></div><div><div style="font-size:11px;font-weight:600;color:var(--steel);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">Evidence Photos</div><div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:6px;">'+thumbsHtml+'</div><div class="photo-zone ev-clickable" onclick="_boxUp(event,function(){genDeficUpload('+i+')})" ondragover="event.preventDefault();this.classList.add(\'drag-over\');" ondragleave="this.classList.remove(\'drag-over\');" ondrop="event.preventDefault();this.classList.remove(\'drag-over\');genDeficDrop(event,'+i+')" style="padding:14px 12px;min-height:110px;border:2px dashed var(--border);border-radius:8px;font-size:11px;color:var(--silver);text-align:center;background:var(--smoke);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;box-sizing:border-box;"><span>Drag, drop or tap here</span><div style="display:flex;gap:5px;"><button onclick="event.stopPropagation();genDeficCamera('+i+')" style="padding:5px 10px;border:none;border-radius:5px;background:#5C7A65;color:white;font-size:calc(12.5px + var(--ts));cursor:pointer;">📷 Camera</button><button onclick="event.stopPropagation();genDeficUpload('+i+')" style="padding:5px 10px;border:none;border-radius:5px;background:#4F6788;color:white;font-size:calc(12.5px + var(--ts));cursor:pointer;">📎 Upload</button><button onclick="event.stopPropagation();_galleryReuseGenDefic('+i+')" style="padding:5px 10px;border:none;border-radius:5px;background:#8A7689;color:white;font-size:calc(12.5px + var(--ts));cursor:pointer;">🖼 Gallery</button></div></div></div></div>'
    +'<div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;align-items:flex-end;">'
    ...
    +'<div><label style="font-size:11px;color:transparent;display:block;margin-bottom:2px;">.</label><button onclick="_deficConfirmRemove(\'g\','+i+')" style="padding:5px 8px;border:1.5px solid #ddd;border-radius:5px;background:white;color:#aaa;font-size:11px;cursor:pointer;">✕ Remove</button></div></div></div></div>'+respHtml+'</div>';
}

...
(function (root) {
  ...
  var SEL = {
    ...
    // S461n (Mark): during TAP-SELECT picking the glow sits 10% more transparent
    // than the committed-member glow — a picked mark reads as provisional until ✓.
    haloAlphaPick:    0.72,   // = 0.80 * 0.9
  ...
  var M = {
    ...
    _drawSelHalo:function(ctx, s, k, active, isPick){
      ...
      if(pathed){
        ...
        ctx.globalAlpha = isPick ? SEL.haloAlphaPick : (active ? SEL.haloAlphaActive : SEL.haloAlpha);
    ...
    _drawSelChrome:function(ctx){
      ...
      if(inTap && this._pickIds && this._pickIds.length){
        ...
        this._pickIds.forEach(function(id){ var s=self._findStroke(id); if(!s) return;
          self._drawSelHalo(ctx, s, pk, true, true);   // S461n: isPick → 10% more transparent
        });
        var br=9*pk;
  ...
  var API = { install: install, VERSION: '2.8.0' };

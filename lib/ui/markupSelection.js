...
(function (root) {
  ...
  var M = {
    ...
    _selGeom:function(){
      ...
      var k=this.nw/Math.max(1,rect.width);                 // natural px per screen px
      ...
      return { k:k, pad:pad, hs:hs, b:{x1:bx1,y1:by1,x2:bx2,y2:by2},
        ...
        // v2.11.0 (S487h, agreed w/ Mark): 4 mid-edge handles for SINGLE-AXIS
        // scale — [top, right, bottom, left]. Corner = proportional (radial),
        // mid-edge = one axis only.
        mids:[{x:(bx1+bx2)/2,y:by1},{x:bx2,y:(by1+by2)/2},{x:(bx1+bx2)/2,y:by2},{x:bx1,y:(by1+by2)/2}],
    ...
    // S461t (Mark): ONE confirm rule for every host. The green check stays
    // available for the WHOLE selection lifecycle:
    //   picks pending  → merge them into the group (classic confirmPick)
    //   group active   → FINALIZE: accept the marks where they now sit and
    //                    exit selection (moves/resizes are already committed
    //                    to the strokes at drag-end; this just closes out).
    confirmSelection:function(){
      if(this._pickIds && this._pickIds.length){ this.confirmPick(); return; }
      if(this.selIds && this.selIds.length){
        this.selIds=[]; this._groupActiveId=null; this._dragState=null;
        if(this.ctx) this.render(); this._emitSel();
      }
    },
    ...
    // ── S574 TRASH MODE (LOCKED_TRASH_MODE.md, Mark-approved S489) ──────────
    // A tap-ONLY pick mode whose confirm DELETES the picks. It reuses the
    // exact tap-pick machinery — same _pickIds set, same halo + green-check
    // chrome — so nothing new is drawn or coloured. Collapses markup deletion
    // from 5 steps (Select → sub-mode → tap → ✓ group → ✗) to 2 (arm 🗑 →
    // tap → confirm): the group step exists for MOVEABILITY, irrelevant when
    // deleting. ONE implementation here; both hosts wire their own button.
    // No rubber-band by design — a destructive mode must not have a
    // bulk-capture gesture (Mark: "No rubber band, tap only is good").
    setTrashMode:function(on){
      on = !!on;
      if (on === !!this._trashMode) { return; }
      this._trashMode = on;
      // arming or leaving clears live selection state either way — a
      // destructive mode never inherits a selection it didn't build.
      this.selIds=[]; this._pickIds=[]; this._grouped=false; this._groupActiveId=null;
      this._dragState=null; this._rubberBand=null;
      if (on){ this._trashPrevSub=this._selectSub; this._selectSub='tap'; }   // pick chrome rides the tap rail
      else if (this._trashPrevSub){ this._selectSub=this._trashPrevSub; this._trashPrevSub=null; }
      if(this.ctx) this.render(); this._emitSel();
    },
    isTrashMode:function(){ return !!this._trashMode; },
    // Trash tap: toggle a mark in/out of the PICK set. Never a group move,
    // never a rubber-band, never a drag — down-only, sticky on empty taps.
    _trashDown:function(p){
      var hit=this._hitStrokeSel(p);
      if(!hit) return;                       // empty tap keeps the picks (sticky)
      var pix=this._pickIds.indexOf(hit.id);
      if(pix!==-1) this._pickIds.splice(pix,1); else this._pickIds.push(hit.id);
      this._dragState=null; this._rubberBand=null;
      if(this.ctx) this.render(); this._emitSel();
    },
    trashCount:function(){ return this._pickIds?this._pickIds.length:0; },
    // Deletion routes through deleteSelected so a host's wrapper (FRT drawing
    // viewer tombstones there) fires EXACTLY as it does on the select path —
    // trash mode adds an entry into the existing delete machinery, not a
    // second delete path. Returns how many were deleted.
    deleteTrashPicks:function(){
      if(!this._pickIds || !this._pickIds.length) return 0;
      var n=this._pickIds.length;
      this.selIds=this._pickIds.slice(); this._pickIds=[];
      this.deleteSelected();
      this.selIds=[];
      if(this.ctx) this.render(); this._emitSel();
      return n;
    ...
    _hitTextAt:function(p){
      var rect=this.canvas.getBoundingClientRect();
      var hr=10*(this.nw/Math.max(1,rect.width));
      for(var i=this.strokes.length-1;i>=0;i--){
    ...
    _hitStrokeSel:function(p){
      var rect=this.canvas.getBoundingClientRect();
      var hr=14*(this.nw/Math.max(1,rect.width));          // glove-friendly, screen-constant
      for(var i=this.strokes.length-1;i>=0;i--){
    ...
    _selDown:function(p, ev){
      ...
      if(g){
        ...
        // v2.11.0 (S487h): mid-edge handles — SINGLE-AXIS scale. mids = [top, right, bottom, left].
        // v2.11.1 (S487i, Mark's line-markup report): anchor at the GEOMETRY
        // edge (unpadded bounds), NOT the padded box edge. The 22-30px grab pad
        // offset made the stretch carry a translation component — invisible on
        // big shapes, but on a near-flat line the cross-axis "stretch" was PURE
        // translation (the line moved instead of growing). Geometry anchoring =
        // pure scaling; a degenerate axis now correctly does ~nothing.
        var _rb=this._groupBounds();
        var _axAnchors=[['y',_rb?_rb.y2:g.b.y2],['x',_rb?_rb.x1:g.b.x1],['y',_rb?_rb.y1:g.b.y1],['x',_rb?_rb.x2:g.b.x2]];
        for(var mi=0;mi<4;mi++){
          if(Math.abs(p.x-g.mids[mi].x)<=g.hs&&Math.abs(p.y-g.mids[mi].y)<=g.hs){
            this._dragState={type:'resizeAxis', axis:_axAnchors[mi][0], anchor:_axAnchors[mi][1],
              startX:p.x, startY:p.y, orig:this._snapSel()};
            return;
          }
        }
        // rotate handle
      ...
      if(this._selectSub==='tap'){
        // GROUPED: pressing inside the group box arms a move.
        // v2.10.1 F1-tap (S487d, Mark's field repro): was gated on !hit — so any
        // OTHER mark's ink under the press point (a dim label crossing a hollow
        // square's interior) defeated the box grab and dissolved the group into
        // picks, which reads as "my mark deselected". The box is the drag target
        // (locked S344 item 6, same as rubber); only a hit on a GROUP MEMBER
        // falls through, preserving the member tap semantics (v2.7.0 deselect /
        // unlink). Non-members inside the box can still be union-tapped after
        // clearing, or tapped where they lie outside the box (S410 union kept).
        if(this.selIds.length && g && (!hit || this.selIds.indexOf(hit.id)===-1) && p.x>=g.b.x1&&p.x<=g.b.x2&&p.y>=g.b.y1&&p.y<=g.b.y2){
        ...
        if(hit){
          if(this.selIds.indexOf(hit.id)!==-1){
            ...
            // DESELECT that one (un-glow, out of the group).
            // v2.10.2 (S487f, Mark): shapes hit ANYWHERE in their box (S461g),
            // so firing the deselect on pointer-DOWN made the member's whole
            // invisible AABB a deselect zone and killed box-drags that started
            // on it. Resolution: arm a MOVE now; the deselect fires at
            // pointer-UP only if the pointer never moved (a clean tap).
            // Drag = move the group (S461g honored); tap = deselect the member
            // (v2.7.0 honored). Both rulings kept.
            this._dragState={type:'move', startX:p.x, startY:p.y, moved:false,
              orig:this._snapSel(), tapMemberId:hit.id};
            return;
          }
      ...
      // Press inside a selection's bounds (even empty space between strokes)
      // starts a group move, not a rubber-band (FRT lesson).
      // v2.10.0 F1 (S487): was length>1 — written when interiors always ink-hit
      // (see S459e note above). v2.4.0's ink-precise hitInk made HOLLOW shapes
      // miss inside their own outline, so a SINGLE selected hollow shape
      // deselected on an interior press (the ring's hole). Locked rule: the
      // WHOLE interior box grabs — any grouped selection, same as tap mode.
      if(!multi && this.selIds.length && g){
    ...
    _selMove:function(p){
      ...
      if(ds.type==='resizeAxis'){
        // v2.11.0 (S487h): single-axis scale from a mid-edge handle. Factor is
        // ratioed to the PRESS point on that axis (same S314b rule as corners:
        // exactly 1.0 at the moment of touch). Geometry stretches on ONE axis;
        // stroke width / font size are NOT scaled (a stretch, not a zoom).
        var aA=ds.anchor, ax0=(ds.axis==='x');
        var _cur=ax0?p.x:p.y, _st=ax0?ds.startX:ds.startY;
        // v2.11.2 (S487j, Mark's bounce report): SIGNED ratio, not absolute.
        // Absolute distance rebounds once the pointer crosses the anchor — the
        // shape compressed, hit flat, then RE-GREW the other way while the drag
        // continued ("drag up, it goes down"). Signed: crossing the anchor now
        // holds at minimum compression. No mirroring (matches corner behavior).
        var _den=_st-aA;
        var fA=(_den===0)?1:((_cur-aA)/_den);
        if(!(fA>0.1)) fA=0.1;   // crossed anchor (negative) or tiny → hold at min
        if(fA>10) fA=10;
        if(Math.abs(fA-1)<0.01 && !ds.moved) return;
        ds.moved=true;
        var _fnA=ax0
          ? function(q){return {x:aA+(q.x-aA)*fA, y:q.y};}
          : function(q){return {x:q.x, y:aA+(q.y-aA)*fA};};
        for(var ai=0;ai<this.selIds.length;ai++){
          var as=this._findStroke(this.selIds[ai]); if(!as) continue;
          var ao=JSON.parse(ds.orig[ai]);
          as.pts=ao.pts.map(_fnA);
          if(ao.eraserMask){ as.eraserMask=JSON.parse(JSON.stringify(ao.eraserMask)); if(window.MarkupEraser) window.MarkupEraser.xformMask(as, _fnA, 1); }
        }
        this.render(); return;
      }
      if(ds.type==='rotate'){
    ...
    _selUp:function(){
      ...
      if(ds && (ds.type==='move'||ds.type==='resize'||ds.type==='resizeAxis'||ds.type==='rotate') && ds.moved && ds.orig){
        var after=this._snapSel();
      ...
      // v2.10.2 (S487f): deferred member deselect — the press armed a move with
      // tapMemberId; if the pointer never moved, it was a clean TAP on a group
      // member → deselect that one now (v2.7.0 rule, relocated from _selDown).
      if(ds && ds.type==='move' && !ds.moved && ds.tapMemberId){
        var tmix=this.selIds.indexOf(ds.tapMemberId);
        if(tmix!==-1){ this.selIds.splice(tmix,1); this._groupActiveId=null; }
      }
      this._dragState=null; this.render(); this._emitSel();
    },
    ...
    _selHaloPath:function(ctx, s){
      ...
      var isFree  = (s.tool==='pen' || s.tool==='highlight' || s.tool==='polyline');   // S479 (Mark 7.2/7.3): polyline is pen-class — its pts ARE the ink; trace it, never box it
    ...
    // S461p — the nw=0 guard, and the ONLY nw change in this engine.
    // A host can hand us nw=0 (FRT lightbox, when a photo's first load attempt
    // fails and the fallback lands later). Every chrome scale then multiplies
    // by zero → box and handles vanish. Repair the VALUE once, here. The locked
    // chrome math below stays byte-identical to the approved v2.2 build.
    _fixNW:function(){
      if(!this.nw || this.nw <= 0){
        this.nw = this.w || (this.canvas ? this.canvas.width/(this.dpr||1) : 0) || 1;
      }
    },
    _drawSelChrome:function(ctx){
      ...
      if(inTap && this._pickIds && this._pickIds.length){
        var self=this, pk=this.nw/Math.max(1,this.canvas.getBoundingClientRect().width);
        ctx.save();
        // glow boxes
      ...
      if(this._rubberBand){
        var rb=this._rubberBand, rk=this.nw/Math.max(1,this.canvas.getBoundingClientRect().width);
      ...
      // S461u (Mark): the member contour glow shows for a grouped selection in
      // BOTH sub-modes. It was tap-mode-only (v2.2 gate), so rubber-band
      // selections showed a bare box and read as "the glow disappeared" —
      // the DV and lightbox run this same engine; only the mode differed.
      if(this.selIds && this.selIds.length){
        var selfG=this, pkG=this.nw/Math.max(1,this.canvas.getBoundingClientRect().width), actId=this._groupActiveId;
        ctx.save();
      ...
      // v2.11.0 (S487h): mid-edge single-axis handles — same square chrome.
      if(g.mids) g.mids.forEach(function(q){ ctx.fillRect(q.x-g.hs/2,q.y-g.hs/2,g.hs,g.hs); ctx.strokeRect(q.x-g.hs/2,q.y-g.hs/2,g.hs,g.hs); });
  ...
  function install(E, hooks){
    ...
    if (E._trashMode === undefined) E._trashMode = false;
    if (E._trashPrevSub === undefined) E._trashPrevSub = null;
  ...
  var API = { install: install, VERSION: '2.12.0' };   // 2.12.0 = S574 trash mode (LOCKED_TRASH_MODE.md: tap-only pick-to-delete, one engine impl for both hosts); 2.11.2 = S487j signed axis factor (no rebound past anchor); 2.11.1 = S487i mid anchors at geometry edge; 2.11.0 = S487h mid-edge single-axis handles; 2.10.2 = S487f tap-member deferred deselect; 2.10.1 = S487d F1-tap box-grab; 2.10.0 = S487 F1 whole-interior grab; 2.9.0 = S479 polyline contour halo

 ...
 * two-SET pick/group model + S459e grab-band pad + S459f tight amber frames).
 ...
 *     Members show 1px-tight amber frames; tap a member → active (thick amber) →
 ...
 *   _rotPt(q, c, a)           — rotate point q about c by a
...
(function (root) {
  ...
  var M = {
    ...
    _groupBounds:function(){
      ...
      this.selIds.forEach(function(id){
        var s=self._findStroke(id); if(!s) return; var b=self._strokeAABB(s); any=true;
    ...
    deleteSelected:function(){
      ...
      this._pushOp({t:'gdel', items:items});
    ...
    _cloneSelection:function(){
      ...
      this.selIds=newIds; this.redo=[]; this.redoOps=[];
      this._pushOp({t:'gadd', items:added});
    ...
    applySel:function(field, val, log){
      ...
      if(log){
        ...
        if(after.join('\u0001')!==before.join('\u0001')) this._pushOp({t:'gmod', ids:ids, before:before, after:after});
      }
    ...
    commitSel:function(snap){
      ...
      if(after.join('\u0001')!==snap.before.join('\u0001')) this._pushOp({t:'gmod', ids:snap.ids, before:snap.before, after:after});
    },
    ...
    _hitStrokeSel:function(p){
      ...
      for(var i=this.strokes.length-1;i>=0;i--){
        var b=this._strokeAABB(this.strokes[i]);
        if(p.x>=b.x1-hr/2&&p.x<=b.x2+hr/2&&p.y>=b.y1-hr/2&&p.y<=b.y2+hr/2) return this.strokes[i];
      }
      return null;
    },
      ...
      if(g){
        ...
        for(var ci=0;ci<4;ci++){
          if(Math.abs(p.x-g.corners[ci].x)<=g.hs&&Math.abs(p.y-g.corners[ci].y)<=g.hs){
            var anchors=[[g.b.x2,g.b.y2],[g.b.x1,g.b.y2],[g.b.x2,g.b.y1],[g.b.x1,g.b.y1]];
      ...
      if(this._selectSub==='tap'){
        ...
        if(hit){
          if(this.selIds.indexOf(hit.id)!==-1){
            // tapped a mark that's ALREADY in the group → make it the "active" grouped mark
            // (shows the round amber Ungroup button so this one can be pulled out of the group).
            this._groupActiveId = (this._groupActiveId===hit.id) ? null : hit.id;
    ...
    _selMove:function(p){
      ...
      if(ds.type==='rotate'){
        ...
        for(var ti=0;ti<this.selIds.length;ti++){
          ...
          var oc=this._strokeCenter(to);                      // center of ORIGINAL stroke
          var nc=this._rotPt(oc,{x:cx,y:cy},dA);
          var mdx=nc.x-oc.x, mdy=nc.y-oc.y;
          ts.pts=to.pts.map(function(q){return {x:q.x+mdx, y:q.y+mdy};});
          if(to.eraserMask){ ts.eraserMask=JSON.parse(JSON.stringify(to.eraserMask)); if(window.MarkupEraser) window.MarkupEraser.xformMask(ts, function(q){return {x:q.x+mdx, y:q.y+mdy};}); }   // S459: masks are local-frame — translate with pts; s.rot spins them at render
          ts.rot=(to.rot||0)+dA;                              // Diesel render model: rotate at draw time
        }
        this.render(); return;
      }
    },
    _selUp:function(){
      ...
      if(ds && (ds.type==='move'||ds.type==='resize'||ds.type==='rotate') && ds.moved && ds.orig){
        ...
        if(after.join('\u0001')!==ds.orig.join('\u0001')){
          this._pushOp({t:'gmod', ids:this.selIds.slice(), before:ds.orig, after:after});
          if(this._onDirty) this._onDirty();
        }
      }
      this._dragState=null; this.render(); this._emitSel();
    ...
    _drawSelChrome:function(ctx){
      ...
      if(inTap && this._pickIds && this._pickIds.length){
        ...
        ctx.save();
        // glow boxes
        this._pickIds.forEach(function(id){ var s=self._findStroke(id); if(!s) return; var b=self._strokeAABB(s);
          var x=b.x1-3*pk, y=b.y1-3*pk, w=(b.x2-b.x1)+6*pk, h=(b.y2-b.y1)+6*pk;
          ctx.shadowColor='#46C5E8'; ctx.shadowBlur=10*pk;
          ctx.strokeStyle='#46C5E8'; ctx.lineWidth=Math.max(1.6,1.8*pk); ctx.globalAlpha=1;
      ...
      if(this._rubberBand){
        ...
        ctx.save(); ctx.setLineDash([4*rk,3*rk]); ctx.strokeStyle='#3F6E9C'; ctx.lineWidth=Math.max(1,1*rk); ctx.globalAlpha=1;
      ...
      if(inTap && this.selIds && this.selIds.length){
        ...
        ctx.save();
        this.selIds.forEach(function(id){ var s=selfG._findStroke(id); if(!s) return; var b=selfG._strokeAABB(s);
          // S459f (Mark): amber member frames sit as TIGHT to the ink as possible (1px);
          // the wide grab gap belongs to the blue GROUP box (S459e pad), not these.
          var x=b.x1-1*pkG, y=b.y1-1*pkG, w=(b.x2-b.x1)+2*pkG, h=(b.y2-b.y1)+2*pkG;
          if(id===actId){
            ctx.setLineDash([]); ctx.shadowColor='#EF9F27'; ctx.shadowBlur=10*pkG;
            ctx.strokeStyle='#EF9F27'; ctx.lineWidth=Math.max(2,2.4*pkG);
            ctx.strokeRect(x,y,w,h); ctx.shadowBlur=0; ctx.strokeRect(x,y,w,h);
          } else {
            ctx.shadowBlur=0; ctx.setLineDash([5*pkG,3*pkG]);
            ctx.strokeStyle='#BA7517'; ctx.lineWidth=Math.max(1.4,1.6*pkG);
            ctx.strokeRect(x,y,w,h);
          }
        });
        ctx.setLineDash([]); ctx.restore();
      ...
      var lw=Math.max(1.5,1.5*g.k);
      ctx.save(); ctx.globalAlpha=1;
      // dashed group box
      ctx.setLineDash([lw*3,lw*2]); ctx.strokeStyle='#3F6E9C'; ctx.lineWidth=lw;
      ctx.strokeRect(g.b.x1,g.b.y1,g.b.x2-g.b.x1,g.b.y2-g.b.y1); ctx.setLineDash([]);
      // corner scale handles
      ctx.fillStyle='#fff'; ctx.strokeStyle='#3F6E9C'; ctx.lineWidth=lw;
      ...
      ctx.beginPath(); ctx.arc(g.del.x,g.del.y,g.del.r,0,Math.PI*2); ctx.fillStyle='#C0445F'; ctx.fill();
      ...
      if(g.copy){
        ctx.strokeStyle='#3F6E9C'; ctx.lineWidth=lw;
        ctx.beginPath(); ctx.moveTo((g.b.x1+g.b.x2)/2,g.b.y2); ctx.lineTo(g.copy.x,g.copy.y-g.copy.r); ctx.stroke();
        ctx.beginPath(); ctx.arc(g.copy.x,g.copy.y,g.copy.r,0,Math.PI*2); ctx.fillStyle='#2C5E8E'; ctx.fill();
        ctx.strokeStyle='#2C5E8E'; ctx.stroke();
        ...
        ctx.fillStyle='#2C5E8E'; ctx.fillRect(g.copy.x-u*0.2, g.copy.y-u*0.2, u*1.8, u*2.2);
  ...
  var API = { install: install, VERSION: '1.0.0' };

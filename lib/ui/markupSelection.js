...
(function (root) {
  ...
  var M = {
    ...
    // S461i: _selHaloPath retired — uniform glow frames replaced the contour halo.
    // Paint the amber contour halo for ONE grouped member. off = px off the ink.
    // S461i: _drawSelHalo retired — uniform glow frames replaced the contour halo.
    _drawSelChrome:function(ctx){
      ...
      if(inTap && this._pickIds && this._pickIds.length){
        ...
        ctx.save();
        // glow boxes
        this._pickIds.forEach(function(id){ var s=self._findStroke(id); if(!s) return; var b=self._selHooks.aabb.call(self, s); if(!b) return;
      ...
      if(inTap && this.selIds && this.selIds.length){
        ...
        ctx.save();
        // S461i (Mark, LOCKED — the drawing-viewer select model is the source
        // of truth): grouped members show the SAME cyan glow frame as tap
        // picks, for EVERY mark type — pen, shape, text, dimension, cloud.
        // The contour halo (S461 orange → cyan-white) is RETIRED: it traced
        // pens organically but fell back to thin rects for dims/text, which
        // read as two different systems. One frame style, everywhere.
        this.selIds.forEach(function(id){ var s=selfG._findStroke(id); if(!s) return; var b=selfG._selHooks.aabb.call(selfG, s); if(!b) return;
          var x=b.x1-3*pkG, y=b.y1-3*pkG, w=(b.x2-b.x1)+6*pkG, h=(b.y2-b.y1)+6*pkG;
          var act=(id===actId);
          ctx.shadowColor='#46C5E8'; ctx.shadowBlur=(act?14:10)*pkG;
          ctx.strokeStyle='#46C5E8'; ctx.lineWidth=Math.max(1.6,(act?2.6:1.8)*pkG); ctx.globalAlpha=1;
          ctx.strokeRect(x,y,w,h);
          ctx.shadowBlur=0; ctx.strokeRect(x,y,w,h);
        });
        ctx.restore();
  ...
  var API = { install: install, VERSION: '2.5.0' };

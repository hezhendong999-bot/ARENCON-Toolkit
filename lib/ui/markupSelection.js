...
(function (root) {
  ...
  var M = {
    ...
    _drawSelChrome:function(ctx){
      ...
      if(inTap && this._pickIds && this._pickIds.length){
        ...
        ctx.save();
        // S461j: picks glow along their ink too (same contour as members) —
        // one visual language. The green ✓ badge marks the PICKED state.
        this._pickIds.forEach(function(id){ var s=self._findStroke(id); if(!s) return;
          self._drawSelHalo(ctx, s, pk, true);
        });
        var br=9*pk;
      ...
      if(inTap && this.selIds && this.selIds.length){
        ...
        ctx.save();
        // S461j (Mark): grouped members glow ALONG THEIR OWN INK — the v2.2
        // contour, for EVERY type. Shapes trace via the canonical renderer,
        // freehand its polyline, dimensions via the host haloPath hook, text
        // keeps the box (a text box IS its ink).
        this.selIds.forEach(function(id){ var s=selfG._findStroke(id); if(!s) return;
  ...
  var API = { install: install, VERSION: '2.6.0' };

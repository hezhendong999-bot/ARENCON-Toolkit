...
var _GAUGE_READINGS=[
  ...
  /* S576 (Mark): PITOT. Every flow point is established by a pitot reading at
     the hose stream or flow meter, and until now there was nowhere to put the
     photo of it — the one reading that proves the FLOW itself, as opposed to
     the pressures either side of the pump. Sits immediately after RPM because
     that is the capture order in the field: read the gauges, read the tach,
     read the pitot. Available at EVERY flow point in BOTH conditions (unlike
     PRV/PRdV, which are direct-run only) — a pitot is taken at each point
     whether or not the PLD is in circuit. */
  {k:'pitot',    label:'Pitot',    short:'Pit'},
...
var _GAUGE_TAG_SHORT={suction:'S',discharge:'D',bf_in:'BFi',bf_out:'BFo',rpm:'RPM',pitot:'Pit',prv:'PRV',prdv:'PRdV'};
var _GAUGE_TAG_LABEL={suction:'Suction',discharge:'Discharge',bf_in:'BF-in',bf_out:'BF-out',rpm:'RPM',pitot:'Pitot',prv:'PRV',prdv:'PRdV'};
...
function _gaugeReadingColor(k){ return k==='rpm' ? 'var(--gr-rpm)' : (k==='suction'?'var(--info)':k==='discharge'?'var(--no)':k==='bf_in'?'var(--warn)':k==='bf_out'?'var(--yes)':k==='pitot'?'var(--gr-pitot)':k==='prv'?'var(--gr-prv)':k==='prdv'?'var(--gr-prdv)':'var(--silver)'); }
...
function _renderFlowPhotoModal(){
  ...
  var pts=row.photos||[];
  var ttl='Gauge, RPM &amp; Pitot Photos'+(row.pct?(' — '+row.pct):'')+(row.flow?(' · '+row.flow+' gpm'):'');

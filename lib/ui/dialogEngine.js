...
.multi .mopt:hover{ border-color: color-mix(in srgb, var(--acc) 45%, var(--dlg-line)); }
.multi .mopt.on{
  color: var(--acc);
  border-color: color-mix(in srgb, var(--acc) 55%, transparent);
  ...
  return new Promise((resolve) => {
    ...
    fields.forEach((f) => {
      ...
      if (f.type === 'textarea') inner += '<textarea></textarea>';
        ...
        /* Tick-any-number-of. Values live on the wrapper's dataset, not on a
           single input, so the submit path below reads it as a special case. */
        inner += '<div class="multi">' + (f.options || []).map((o) => {
          const val = (o && typeof o === 'object') ? o.value : o;
          const lab = (o && typeof o === 'object') ? o.label : o;
          const on = Array.isArray(f.value) && f.value.indexOf(val) !== -1;
          return '<button type="button" class="mopt' + (on ? ' on' : '') +
            '" data-val="' + _esc(val) + '">' + _esc(lab) + '</button>';
        }).join('') + '</div>';
      }
      ...
      if (f.type === 'multi') {
        wrap.querySelectorAll('.mopt').forEach((b) => {
          b.addEventListener('click', () => b.classList.toggle('on'));
        });
        inputs[f.key] = { el: null, wrap, def: f };
        return;
      }
    ...
    const submit = () => {
      ...
      Object.keys(inputs).forEach((k) => {
        ...
        errEl.textContent = '';
        if (def.type === 'multi') {
          const picked = Array.prototype.slice.call(wrap.querySelectorAll('.mopt.on'))
            .map((b) => b.dataset.val);
          if (def.required && !picked.length) {
            errEl.textContent = 'Pick at least one.';
            if (!bad) bad = wrap.querySelector('.mopt');
          }
          out[k] = picked;
          return;
        }
...
/* S730 — one handle for classic scripts. lib/ui/lightbox.js is a classic
   script (window.LightboxShell) and cannot import an ES module, which is how
   it came to call the browser's own prompt() for the opacity value — the one
   forbidden native dialog left in the toolkit. Exposing the SAME engine object
   here (not a copy) lets any classic script use the shared modals wherever
   this module is loaded. Guarded: never throws in a worker or a test. */
try { if (typeof window !== 'undefined') window.ArcDialog = Dlg; } catch (_) {}

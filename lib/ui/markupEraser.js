...
(function (root) {
   ...
   *   rot         — optional rotation accessor (default s.rot||0); FRT passes
   *                 s.rotation for its shapes/text render-time rotation model
  ...
  function applyEraser(strokes, eraserPts, lineWidth, opts) {
    ...
    for (var i = 0; i < strokes.length; i++) {
      ...
      var t = canon(s.tool);
      // v1.2.0: rotation accessor — Diesel stores render-time rotation on s.rot;
      // FRT's engine uses s.rotation (pen/highlight bake rotation into points and
      // report 0 here, which is correct: their masks live in world/frame coords).
      var srot = (opts && opts.rot) ? (opts.rot(s) || 0) : (s.rot || 0);
      var ep = toLocal(eraserPts, (srot && opts && opts.center) ? opts.center(s) : null, srot);

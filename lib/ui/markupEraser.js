 ...
 *   • ON RELEASE (applyEraser):
 *       pen (unrotated)      → geometric FRAGMENT SPLIT: points under the path
 *                              are dropped, surviving runs (≥2 pts) become
 *                              separate strokes; the first keeps the id.
 *       highlight            → eraserMask (hit radius inflated by the visible
 *                              half-width so grazing the blob edge registers —
 *                              viewer S81 rule).
 *       text / every shape   → eraserMask: the eraser's EXACT path is carved
 *       (line, arrow, rect,    from that object's pixels at render time via
 *        circle, triangle,     destination-out on a per-object offscreen.
 *        cloud, + fills)
 *       pen WITH s.rot       → eraserMask too (splitting a rotated stroke
 *                              shifts fragment rotation centers; the mask
 *                              carve is visually identical and geometrically
 *                              safe). The viewer has no per-object rot, so
 *                              this case has no viewer precedent — mask is
 *                              the faithful generalization.
...
(function (root) {
   ...
   * @param lineWidth  eraser visual line width in the SAME coords
   *                   (callers: (size||2)*3*uiScale). Hit radius = lineWidth/2.
  ...
  function applyEraser(strokes, eraserPts, lineWidth, opts) {
    ...
    for (var i = 0; i < strokes.length; i++) {
      ...
      if (t === 'pen' && !s.rot) {
        // Fragment split — thin strokes get clean separation (viewer rule)
        var frags = splitPtsByEraser(s.pts, ep, eraserR2);
        if (frags === null) { next.push(s); continue; }
        if (frags.length === 1 && frags[0].length === s.pts.length) { next.push(s); continue; } // untouched
        changed = true;
        for (var f = 0; f < frags.length; f++) {
          var ns = {};
          for (var k in s) if (Object.prototype.hasOwnProperty.call(s, k)) ns[k] = s[k];
          ns.pts = frags[f];
          ns.id = (f === 0) ? s.id : newId();   // first fragment keeps the id
          next.push(ns);
        }
        // frags.length===0 → entire stroke erased: push nothing
      }
      else if (t === 'highlight' || (t === 'pen' && s.rot)) {
        // Highlight renders wider than s.size — inflate the spine hit radius by
        // the visible half-width (viewer S81). Rotated pen: mask (see header).
        var hw = (opts && opts.halfWidth) ? opts.halfWidth(s) : ((s.size || 2) * 2);
        var hlR = eraserR + hw;

...
export function openAccountPanel(cfg) {
  ...
  /* ── S665 — THE PANEL BRINGS ITS OWN DIALOG ENGINE ─────────────────────────
     Mark, 15 Aug: "you broke the account feature and it doesn't open anymore."
     The panel resolved its dialog engine from window.Dlg — a spot only the HUB
     fills. FRT holds Dlg as a private module import (never on window); Diesel
     parks it under window.ArenconDlg. In both tools the lookup found nothing,
     the guard returned, and a click on Account did nothing, silently, from the
     day they were wired (S653/S658).

     This file's own header says the panel OWNS EVERYTHING IT NEEDS — that has
     to include its dependencies, not just its styles. The engine lives in the
     SAME DIRECTORY as this file, so the panel now imports it itself when the
     host has not handed one over. Hosts pass data; the panel brings the rest.
     And the failure mode is LOUD now: a panel that cannot open says so in the
     console instead of eating the click. */
  var D = cfg.dlg || (typeof window !== 'undefined' ? window.Dlg : null);
  var engineReady = (D && D.panel)
    ? Promise.resolve(D)
    : import('./dialogEngine.js').then(function (m) {
        var E = m.Dlg || (m.default && m.default.panel ? m.default : null);
        if (!E || !E.panel) throw new Error('dialogEngine loaded but exposes no panel()');
        return E;
      });
  engineReady.then(function (engine) { _openWith(engine, cfg, toast, handle); })
    ['catch'](function (e) {
      console.error('[AccountPanel] cannot open \u2014 dialog engine unavailable:', e);
      toast('Account panel could not open \u2014 refresh the page', 'error');
    });
  var handle = { close: function () { if (handle._api) { var a = handle._api; handle._api = null; a.close(); } } };
  return handle;
}

function _openWith(D, cfg, toast, handle) {

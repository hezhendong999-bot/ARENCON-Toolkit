/**
 * ARENCON — Shared Checklist Engine  v1.0.0
 * ═══════════════════════════════════════════════════════════════════════
 ...
 *   CAPTURE    triggerPhoto(id) · triggerCamera(id) · handleDrop(e,id) ·
 *              _galleryReuseChecklist(id) · _boxUp(ev,fn)
    ...
    div.innerHTML = `
      ...
      <div class="item-detail${_detailOpen?' open':''}" id="id-${id}">
        ...
        <div>
          ...
          <div class="pm ev-clickable" onclick="_boxUp(event,function(){triggerPhoto('${id}')})" ondragover="event.preventDefault();this.classList.add('drag-over')" ondragleave="this.classList.remove('drag-over')" ondrop="event.preventDefault();this.classList.remove('drag-over');handleDrop(event,'${id}')">
            <div class="photo-grid" id="pg-${id}">${renderThumbs(id)}</div>
            <div class="pm-divider${_nPhotos>0?'':' hide'}" id="pmdiv-${id}"></div>
            <div class="pm-foot">
              <span class="pm-hint">${_nPhotos>0?'Drop more photos to add':'Drop or tap here, or use a button'}</span>
              <span class="pm-btns">
                <button class="pm-b cam" onclick="triggerCamera('${id}')">📷<span class="pm-b-txt">Camera</span></button>
                <button class="pm-b gal" onclick="_galleryReuseChecklist('${id}')">🖼<span class="pm-b-txt">Gallery</span></button>
              </span>
            </div>
          </div>
        </div>

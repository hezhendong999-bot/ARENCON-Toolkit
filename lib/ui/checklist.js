...
function create(cfg) {
  ...
  function buildItem(id, item, section, idx, isCustom) {
    ...
    /* S729 — scope chip. An item may declare who it is answered for:
       'visit' (this commissioning day), 'room' (the shared installation),
       'machine' (this pump — the default, no chip), or 'diesel-only'.
       Only VISIT and ROOM are chipped: they are the ones an inspector can
       carry across from the sister report on a two-pump job. Inside the
       diesel tool a DIESEL chip would mark most of a section and tell
       nobody anything, so the label is recorded in the data and not drawn.
       An item with no scope renders exactly as it does today. */
    var _scope = (item && item.scope) ? String(item.scope) : '';
    var _chip = (_scope==='visit' || _scope==='room')
      ? `<span class="cl-scope cl-scope-${_scope}">${_scope.toUpperCase()}</span>` : '';
    var _hint = (item && item.hint) ? `<div class="cl-hint">${item.hint}</div>` : '';
    div.innerHTML = `
      <div class="item-main">
        <div class="item-num">${_num}${_chip}</div>
        <div class="item-text" id="text-${id}">${s.customText || item.text}${_hint}</div>
      ...
      <div class="item-detail${_detailOpen?' open':''}" id="id-${id}">
        <div>
          ...
          <textarea class="comment-input" placeholder="Add a note or describe the deficiency..." oninput="clState['${id}'].comment=this.value;clState['${id}']._ts=Date.now();">${s.comment}</textarea>
        </div>
        <div>
  ...
  return {
    ...
    itemNum: itemNum,
    /* S723 — the host asks the engine "what are section X's built-in items?"
       in two places (the Summary/cover checklist walk and the gallery badge
       lookup). The engine has always taken that function in cfg and used it
       internally for itemNum, but never handed it back out, so both call
       sites hit their `_CLENG.sectionItems ?` guard, took the empty branch
       and silently counted ZERO — the Summary donut read 0 CHECKLIST with a
       fully answered report on screen. The guards made a missing door look
       like an empty room. Returning it is what the hosts always expected. */
    sectionItems: cfg.sectionItems
  };
}

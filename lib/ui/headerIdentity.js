...
export function saveColour(userId, hex) {
  ...
  return fetch(SB_URL + '/rest/v1/profiles?id=eq.' + encodeURIComponent(userId), {
    ...
    /* S671 — SAVING A COLOUR MUST REACH WHATEVER IS ALREADY ON SCREEN.
       The write and the header repaint were the whole story, so a drawing that
       was already open kept painting the old ring until the report was closed
       and reopened. Picking a colour and watching the pins ignore you is
       indistinguishable from the picker not saving — which is exactly how this
       was reported. This is the ONE write path for a ring colour, so it is the
       one place that can announce the change; every screen listens rather than
       each caller remembering to tell them. */
    try {
      window.dispatchEvent(new CustomEvent('arencon:ring-colour', {
        detail: { userId: userId, colour: hex }
      }));
    } catch (e) {}
    return true;
  });

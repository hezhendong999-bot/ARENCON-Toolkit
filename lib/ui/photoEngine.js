...
export function compressPhoto(ImageWorkerHost, file, opts) {
  ...
  return ImageWorkerHost.compressFile(file, {
    /* S702d — the shared default moves with the FRT call sites, so a host that
       omits the option does not silently store a smaller photograph than the
       tool beside it. */
    maxW: opts.maxW || 2048,
    quality: (typeof opts.quality === 'number') ? opts.quality : 0.85,

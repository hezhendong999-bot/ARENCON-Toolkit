...
function _accountFor(h){
  ...
  return {
    ...
    /* S683b — ROLE travels with the account block. This is the reason the
       avatar role badge did not appear in FRT, Diesel or Electric: they all
       build identity here, and this block carried name, initials and colour
       but never role, so the shared engine had nothing to derive a badge
       from. Auth already knows — isSuperAdmin() also covers Mark by email if
       his profile row ever reads 'admin'. The engine owns the role→badge
       mapping; this only reports the fact. */
    role: (A.isSuperAdmin && A.isSuperAdmin()) ? 'super_admin'
...
export function dieselHeaderConfig(h){
  ...
  return {
    title:h.title||'Diesel Fire Pump Commissioning Report',   /* S724: Electric ran this config verbatim and its header read "Diesel Fire Pump Commissioning Report". Override, not edit — Diesel passes no title and is unchanged. */
    ...
    actions:_clean([
      ...
      { key:'more', foldRank:35, type:'menu', label:'&#9881;&#65039; More &#9662;', bg:'#455A64',
        ...
        items:[
          ...
          { divider:true },
          /* S692 — RELOAD FROM CLOUD. The action that had to exist. Until now,
             a device holding a copy that had been corrected elsewhere could
             only be made to let go by clearing the browser's site data — which
             no commercial application asks of the person using it, and which
             also destroys every other project cached on that machine. This
             discards THIS report on THIS device and takes the cloud copy. It
             is destructive, so the host confirms first (one tap, custom modal)
             and the engine sets the outgoing copy aside before replacing it.
             hubOnly: there is no cloud copy to take in standalone mode. */
          { label:'&#9729;&#65039; Reload from Cloud', sub:'Discard this device\u2019s copy, take the cloud version',
            hubOnly:true, onClick:h.onReloadCloud||null },

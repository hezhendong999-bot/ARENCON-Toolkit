 ...
 *   • AI Review button  = burgundy #7d1f35 everywhere (FRT purple retired)
...
function _dayNight(onToggle){
  return { key:'dark', type:'icon', id:'dark-toggle',
...
function _textSize(onClick){
  return { key:'ts', type:'icon', id:'btn-text-size', icon:'M',
...
function _navArrows(onUndo,onRedo){
  return { key:'nav', type:'nav-arrows', id:'header-nav-arrows', drawerLabel:'Undo / Redo',
    items:[
...
export function dieselHeaderConfig(h){
  ...
  return {
    ...
    toolOwnsTheme:true,   // Diesel manages body.dark-mode + its own #dark-toggle icon
    onBack:h.onBack||null, onHome:h.onHome||null,
    // cloud sync status display — Diesel's JS reads/writes these (cloud-status,
    // cloud-dot, cloud-status-text, last-sync-text: ~24 refs). Engine renders the
    // spans; Diesel manages content + visibility.
    signalSlots:[
      { cloudStatus:true, id:'cloud-status', dotId:'cloud-dot', textId:'cloud-status-text', lastSyncId:'last-sync-text' }
    ],
    actions:[
      _navArrows(h.onUndo, h.onRedo),
      ...
      { key:'inspector', type:'chip', id:'inspector-chip', title:'Change inspector', drawerLabel:'Inspector',
        label:'<span class="ic-icon">👤</span><span class="ic-name" id="inspector-chip-name">Set Name</span>', onClick:h.onInspector||null },
      { key:'ai', type:'menu', id:'btn-ai-review', wrapId:'ai-review-wrap', menuId:'ai-mode-menu',
        label:'✨ AI Review ▾', bg:'#7d1f35', title:'AI text review', drawerLabel:'AI Review',
        items:[
      ...
      { key:'reports', type:'menu', id:'btn-reports-parent', wrapId:'btn-reports-wrap', menuId:'reports-menu',
        ...
        items:[
          { label:'📋 Issue Report', sub:'Bump revision &amp; mark issued', onClick:h.onIssue||null },
      ...
      { key:'more', type:'menu', id:'btn-more-wrap-btn', wrapId:'btn-more-wrap', menuId:'more-menu',
        ...
        items:[
          ...
          { divider:true, id:'menu-r2-divider' },
          { id:'menu-reupload', label:'☁️ Re-upload All Photos', sub:'Push all local photos to R2', hubOnly:true, onClick:h.onReupload||null },
          { id:'menu-r2cleanup', label:'🧹 R2 Orphan Report', sub:'List stranded cloud files', hubOnly:true, onClick:h.onR2Cleanup||null },
          { id:'menu-deldiag', label:'🔎 Photo Delete Log', sub:'Recent photo deletions (Bug 1 diag)', onClick:h.onDelDiag||null },
          { divider:true },
          { label:'🗑️ Reset Current Page', sub:'Clear this page\'s data', onClick:h.onResetPage||null },
          { label:'<span style="color:#C62828">⚠️ Reset All Pages</span>', sub:'Delete all project data', onClick:h.onResetAll||null }
        ]},
      _qr(h.onQR),
      // Diesel OWNS its theme + day-night icon (initDarkMode / toggleDarkMode /
      // updateDarkToggleIcon swap #dark-toggle innerHTML; line ~2306 fills it on
      // load). Render a plain #dark-toggle icon button (no engine icon/swap) and
      // just call toggleDarkMode(); diesel handles sun/moon + body.dark-mode +
      // localStorage itself. Engine must NOT double-swap or manage data-theme here.
      { key:'dark', type:'icon', id:'dark-toggle', icon:'', title:'Toggle Dark Mode',
        exemptUntilLast:true, exemptOrder:1, drawerLabel:'Day / Night', toolOwnsTheme:true,
        onClick:h.onToggleTheme||null },
      _textSize(h.onTextSize),
      _signout(h.onSignout)
    ]
  };
}
...
export function frtHeaderConfig(h){
  /* S488 Wave 3 prep — REWRITTEN against the LIVE FRT header (the S455 skeleton
     ...
     LOCKED standardization applies: AI Review = burgundy #7d1f35 (FRT purple
     #7B2D8E retired, S455 Mark). */
  h = h || {};
  return {
    ...
    actions:[
      ...
      { key:'idb', type:'meter', label:'IDB', title:'Browser storage usage', drawerLabel:'Storage usage' },
      { key:'inspector', type:'chip', title:'Change inspector', drawerLabel:'Inspector',
        label:'&#128100; Set Name', onClick:h.onInspector||null },
      { key:'load', type:'text', label:'&#128194; Load', title:'Load JSON',
        drawerLabel:'Load', bg:'#455A64', onClick:h.onLoad||null },
      { key:'exportall', type:'text', label:'&#128230; Export All', title:'Export All Projects',
        drawerLabel:'Export All', bg:'#455A64', onClick:h.onExportAll||null },
      { key:'ai', type:'menu', label:'&#10024; AI Review &#9662;', bg:'#7d1f35',
      ...
      { key:'reports', type:'menu', label:'&#128196; Reports &#9662;', bg:'#1A237E',
      ...
      { key:'more', type:'menu', label:'&#9881;&#65039; More &#9662;', bg:'#455A64',
        title:'More options', drawerLabel:'More', hubOnly:true,
        items:[
          { label:'&#128190; Download JSON', sub:'Save project data to file', onClick:h.onDownloadJSON||null },
          { label:'&#128230; Export Project Docs', sub:'ZIP: photos + JSON + README', onClick:h.onExportDocs||null },
          { label:'&#128194; Load Project', sub:'Open a JSON project file', onClick:h.onLoadProject||null },
          { divider:true },
          { repairSection:true, label:'&#128295; Repair', hubOnly:true, repairItems:[
            { label:'&#9729;&#65039; Re-upload All', sub:'Push all drawings & photos to R2', onClick:h.onReupload||null },
            { label:'&#128196; Fix Blurry', sub:'Re-upload PDFs at full resolution', onClick:h.onFixBlurry||null },
            ...
            { label:'&#128279; Repair R2 Links', sub:'Fix photo/drawing filename mismatches', adminOnly:true, onClick:h.onRepairLinks||null }
          ]},
          { label:'&#129658; Diagnostics', sub:'Check cloud, sync, photo subsystem state', hubOnly:true, onClick:h.onDiagnostics||null },
          { label:'&#128241; QR Code', sub:'Scan to open this tool on another device', onClick:h.onQR||null },
          { divider:true },
...
export function electricHeaderConfig(h){
  ...
  return {
    ...
    actions:[
      ...
      { key:'dark', type:'icon', id:'dark-toggle', iconLight:DAYNIGHT_SUN, iconDark:DAYNIGHT_MOON,
        title:'Toggle Dark Mode', drawerLabel:'Day / Night', onClick:h.onToggleTheme||null },
      _textSize(h.onTextSize),
      _signout(h.onSignout)
    ]
  };

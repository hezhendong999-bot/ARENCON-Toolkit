...
function html(o) {
  ...
  var h = '<div class="obs-media-col" data-pi-ns="' + _esc(ns) + '"' + da + '>';
...
function _delegate() {
  ...
  document.addEventListener('click', function(e) {
    ...
    var h = _mounted[m[1]];
    if (!h) return;
    var zone = el.closest('.obs-media-col[data-pi-ns]');
    if (!zone) return;
    var ctx = _ctxOf(zone);
  ...
  document.addEventListener('dragover', function(e) {
    var z = e.target.closest && e.target.closest('.obs-media-col[data-pi-ns]');

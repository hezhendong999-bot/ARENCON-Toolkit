...
export function searchHelp(q, scope) {
  const qw = _norm(q).split(' ').filter(w => w && !STOP[w]);
  if (!qw.length) return null;
  return _scopedCards(scope)
...
/* ── scope (S505f, Mark — STANDING RULE) ──────────────────────────────────────
   A Help panel shows exactly ONE scope and NEVER mixes: the Dashboard shows only
   Dashboard cards, a project's Hub page only Hub-page cards, Diesel only Diesel,
   and so on for every future tool. Registration is per scope (registerHelp's
   `tool`), so scoping is just a filter applied to What's New, the Guide and
   search alike — every consumer passes its own scope and inherits the rule.
   Scope omitted = legacy pooled behaviour, kept so nothing breaks mid-rollout. */
function _scopedCards(scope) {
  return scope ? _allCards().filter(c => c.tool === scope) : _allCards();
}
function _scopedAreas(scope) {
  if (!scope) return _allAreas();
  const r = _registry.find(x => x.tool === scope);
  if (!r) return [];
  return (r.areas.length ? r.areas.slice() : _toolAreas(scope));
}
...
function _cardHtml(c, hideTool) {
  /* hideTool: a scoped panel only ever shows one tool, so tagging every title
     with that tool's name is noise. Unscoped (pooled) panels still tag. */
  /* S508: data-help-card is the anchor the contextual "?" scrolls to. It is an
     addressing hook only — it changes nothing about how a card renders. */
  return '<div class="help-card' + (c.compact ? ' compact' : '') + '" data-help-card="' + _esc(c.id || '') + '">'
    + '<h4 class="help-card-h">' + _esc(c.title) + (c.tool && !hideTool ? ' <span class="help-card-tool">' + _esc(c.tool) + '</span>' : '') + '</h4>'
...
/* S505f: the dot is per SCOPE, keyed separately, so Dashboard news never lights
   the dot on Diesel and vice versa. Unscoped keeps the original single key.
   One-time effect on rollout: each scope's dot shows once, because the old
   single marker does not map onto the new per-scope keys. */
function _seenKey(scope) { return scope ? _SEEN_KEY + ':' + scope : _SEEN_KEY; }
export function latestId(scope) {
  const all = _scopedCards(scope).slice().sort((a, b) => (b.date || '').localeCompare(a.date || ''));
  return all[0] && all[0].id;
}
export function hasUnseen(scope) {
  const l = latestId(scope); if (!l) return false;
  let seen = ''; try { seen = localStorage.getItem(_seenKey(scope)) || ''; } catch (e) {}
  return l !== seen;
}
export function markSeen(scope) {
  const l = latestId(scope); if (!l) return;
  try { localStorage.setItem(_seenKey(scope), l); } catch (e) {}
}
...
export function mountHelp(host, opts) {
  opts = opts || {};
  const scope = opts.scope || '';   /* S505f: one panel = one scope, never mixed */
  /* S508 CONTEXTUAL HELP (Mark): a caller can open the panel AT the thing the
     user is looking at — opts.card:'<card id>' or opts.area:'<area name>'. The
     Guide still renders in full (browsing context is the point of a guide); the
     target is scrolled to and briefly ringed. A target implies the Guide tab
     unless the caller asked for a tab explicitly. */
  let pending = (opts.card || opts.area) ? { card: opts.card || '', area: opts.area || '' } : null;
  let tab = opts.tab || (pending ? 'guide' : 'wn');
  let q = '';
  ...
  function renderWn() {
    const recent = _scopedCards(scope).slice()
    ...
    body.innerHTML = Object.keys(byMonth).map(m =>
      '<div class="help-grouphdr">' + _esc(m) + '</div>' + byMonth[m].map(c => _cardHtml(c, !!scope)).join('')
    ).join('');
  }

  async function renderGuide() {
    if (q) {
      let hits = searchHelp(q, scope);
      if ((!hits || !hits.length) && _aiFallback) {
        body.innerHTML = '<div class="help-empty">Searching\u2026</div>';
        try { hits = await _aiFallback(q, _scopedCards(scope)); } catch (e) { hits = null; }
      }
      if (!hits || !hits.length) {
      ...
      body.innerHTML = '<div class="help-grouphdr">' + hits.length + ' result' + (hits.length === 1 ? '' : 's') + '</div>'
        + hits.map(c => _cardHtml(c, !!scope)).join('');
      return;
    }
    const areas = _scopedAreas(scope);
    const pool = _scopedCards(scope);
    body.innerHTML = areas.map(a => {
      const inA = pool.filter(c => c.area === a);
      if (!inA.length) return '';
      return '<div class="help-grouphdr areahdr" data-help-area="' + _esc(a) + '">' + _esc(a) + '</div>' + inA.map(c => _cardHtml(c, !!scope)).join('');
    }).join('');
    _focusPending();
  }

  /* S508: consume a pending contextual target once the Guide has painted. The
     panel may not be laid out yet when build() runs, so the scroll is deferred
     a frame. A miss is silent — the guide simply opens at the top. */
  function _focusPending() {
    if (!pending) return;
    const t = pending; pending = null;
    const sel = t.card
      ? '[data-help-card="' + String(t.card).replace(/["\\]/g, '') + '"]'
      : '[data-help-area="' + String(t.area).replace(/["\\]/g, '') + '"]';
    let el = null;
    try { el = body.querySelector(sel); } catch (e) { el = null; }
    if (!el) return;
    const go = () => {
      ...
      try {
        ...
        setTimeout(() => { try { el.classList.remove('help-focus'); } catch (e) {} }, 2600);
      } catch (e) {}
    };
    if (typeof requestAnimationFrame === 'function') requestAnimationFrame(() => setTimeout(go, 40));
  ...
  return {
    ...
    /* S508: jump an already-mounted panel to a card or area. Same contract as
       the mount options — { card } or { area }. */
    openAt(t) {
      if (!t) return;
      pending = { card: t.card || '', area: t.area || '' };
      tab = 'guide'; q = '';

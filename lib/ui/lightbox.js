...
function build(hooks) {
  ...
  function _build(){
    ...
    opVal.addEventListener('click',function(){
      /* S730 — the shared dialog engine, not the browser's prompt(). Same
         contract: a string, or null on cancel. The native call survives only
         as the fallback for a host that has not loaded the engine, so this
         classic script keeps working everywhere it is embedded. */
      var _apply=function(v){
        if(v===null||v===undefined) return; var n=parseInt(v,10); if(isNaN(n)) return;
        _mkAlpha=Math.max(10,Math.min(100,n)); _applyOp();
      };
      var D=(typeof window!=='undefined')&&window.ArcDialog;
      if(D&&typeof D.prompt==='function'){
        D.prompt({ title:'Opacity', label:'10 \u2013 100', value:String(_mkAlpha), confirmText:'Set',
                   validate:function(v){ var n=parseInt(v,10); return (isNaN(n)||n<10||n>100)?'Enter a whole number from 10 to 100':''; } })
         .then(_apply).catch(function(){});
        return;
      }
      _apply(prompt('Opacity (10\u2013100):', String(_mkAlpha)));
    });
    // ── Undo / Redo / actions ──

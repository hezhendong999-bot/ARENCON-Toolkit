...
function build(hooks) {
  // hooks reserved (v1 consumers share the global contract above; per-host
  // divergence gets hook overrides in a future minor, same as the markup
  // family did when FRT joined).
  'use strict';
  ...
  function _build(){
    ...
    if(window.DieselMarkup){
      DieselMarkup._onTextStart=function(controller){
      ...
      DieselMarkup._onTextEnd=function(){
    ...
    function setActiveTool(tool){
      _mkTool=tool;
      if(tool!=='select' && window.DieselMarkup && DieselMarkup.hasSel && DieselMarkup.hasSel()) DieselMarkup.deselect();   // S314
    ...
    function _selLive(fn){
      if(!(window.DieselMarkup&&DieselMarkup.hasSel&&DieselMarkup.hasSel())) return;
      if(!_selGest) _selGest=DieselMarkup.snapSel();
      fn();
    }
    function _selCommit(){ if(_selGest){ DieselMarkup.commitSel(_selGest); _selGest=null; } }
    function setSwatch(col){
      ...
      if(window.DieselMarkup&&DieselMarkup.hasSel&&DieselMarkup.hasSel()){
        var snap=DieselMarkup.snapSel();
        DieselMarkup.applySel('color',col,false);
        DieselMarkup.commitSel(snap);
      }
    }
    ...
    function _hiliteSub(){
      var cur=(window.DieselMarkup&&DieselMarkup.getSelectSub)?DieselMarkup.getSelectSub():'rubber';
    ...
    Array.prototype.forEach.call(subFly.querySelectorAll('button'), function(sb){
      sb.addEventListener('click', function(){
        if(window.DieselMarkup&&DieselMarkup.setSelectSub) DieselMarkup.setSelectSub(sb.dataset.sub);
    ...
    function _refreshConfirmBar(){
      var E=window.DieselMarkup; if(!E||_mkTool!=='select'){ cBar.style.display='none'; return; }
    ...
    cOk.addEventListener('click',function(){ if(window.DieselMarkup&&DieselMarkup.confirmPick){ DieselMarkup.confirmPick(); } _refreshConfirmBar(); });   // ✓ = group the picks
    cUn.addEventListener('click',function(){ if(window.DieselMarkup&&DieselMarkup.ungroupActive){ DieselMarkup.ungroupActive(); } _refreshConfirmBar(); });   // ⛓ = remove this mark from the group
    cNo.addEventListener('click',function(){ if(window.DieselMarkup&&DieselMarkup.cancelSelect){ DieselMarkup.cancelSelect(); } _refreshConfirmBar(); });   // ✗ = clear
    if(window.DieselMarkup&&DieselMarkup.onSelChange) DieselMarkup.onSelChange(_refreshConfirmBar);
    ...
    function _applySize(){
      ...
      _selLive(function(){
        DieselMarkup.applySel('size',function(s){ return s.tool==='text'?s.size:_mkSize*3; },false);
        DieselMarkup.applySel('fontSize',function(s){ return s.tool==='text'?Math.max(18,_mkSize*8):s.fontSize; },false);
      });
      _selCommit();
    }
    ...
    function _applyOp(){
      ...
      _selLive(function(){ DieselMarkup.applySel('alpha',function(s){ var a=_mkAlpha/100; return s.tool==='highlight'?0.35*a:a; },false); });
      _selCommit();
    }
    ...
    // ── Undo / Redo / actions ──
    bUn.addEventListener('click',function(){ DieselMarkup.undo(); });
    bRd.addEventListener('click',function(){ DieselMarkup.redoOp(); });
    bCr.addEventListener('click',function(){ DieselMarkup.clear(); });
    ...
    _resetBarState=function(){
      ...
      try{ if(window.DieselMarkup){ if(DieselMarkup._textController&&DieselMarkup._textController.cancel) DieselMarkup._textController.cancel(); if(DieselMarkup.cancelSelect) DieselMarkup.cancelSelect(); if(DieselMarkup.setSelectSub) DieselMarkup.setSelectSub('rubber'); } }catch(_){}
  ...
  function _apply(){
    ...
    if(mkV && window.DieselMarkup && !_markupActive){
      try{ DieselMarkup.composite(ctx, mkV, nw, nh); }catch(e){ console.warn('[DLB] saved-mk composite failed', e); }
    }
    var z=_el('dlb-zoom');
    ...
    if(_markupActive){ _placeMarkupCanvas(); try{ if(window.DieselMarkup && DieselMarkup._repositionTextBox) DieselMarkup._repositionTextBox(); }catch(_){} }
  }
  ...
  function _maybeCommitOnExit(){
    var p=_photos[_idx];
    if(window.DieselMarkup && DieselMarkup.strokes && DieselMarkup.strokes.length){ _saveMk(); }
    else if(p && (p._annotated || p.mk) && window.DieselMarkup){
  ...
  function _clearMarkupState(p){
    try{
      ...
      if(window.DieselMarkup) DieselMarkup.clear();
  ...
  function _toggleMarkup(){
    ...
    if(!window.DieselMarkup){ if(typeof showToast==='function') showToast('Markup engine not loaded'); return; }
    var img=_el('dlb-image'), cv=_el('dlb-canvas');
    ...
    var _doAttach=function(){
      ...
      DieselMarkup.attach(cv, img, (p.id&&_mkDraft[p.id])||p.mk||null, function(){
        var _a=_mkAlpha/100;
      ...
      _placeMarkupCanvas();
      try{ DieselMarkup.render(); }catch(e){ console.warn('[DLB] markup initial render failed', e); }  // paint base photo + loaded strokes
    };
  ...
  function _exitMarkup(keepStrokes){
    ...
    try{ if(window.DieselMarkup && DieselMarkup._textController && DieselMarkup._textController.cancel) DieselMarkup._textController.cancel(); }catch(_){}   // S344: never leave an open text box behind
    if(window.DieselMarkup && !keepStrokes){
    ...
    try{
      ...
      if(window.DieselMarkup && DieselMarkup.cancelSelect) DieselMarkup.cancelSelect();
    }catch(_e){}
  ...
  function _revertMk(){
    ...
    var dirty = !!(window.DieselMarkup && DieselMarkup.isDirty());
  ...
  function _download(){
    ...
    var mkLive = (_markupActive && window.DieselMarkup && DieselMarkup.canvas) ? DieselMarkup.toMk() : null;
    ...
    _dslLoadBakeImage(p).then(function(bake){
      ...
      if(mkV && window.DieselMarkup) DieselMarkup.composite(cx,mkV,c.width,c.height);
      bake.revoke();
  ...
  function _wireViewEvents(){
    ...
    document.addEventListener('keydown',function(e){
      ...
      if(e.key==='Escape'){
        ...
        if(_markupActive){
          if(window.DieselMarkup && DieselMarkup.hasSel && DieselMarkup.hasSel()){ DieselMarkup.deselect(); }   // S314
      ...
      if(_markupActive&&(e.ctrlKey||e.metaKey)){
        if(e.key==='z'||e.key==='Z'){ DieselMarkup.undo(); e.preventDefault(); return; }
        if(e.key==='y'||e.key==='Y'){ DieselMarkup.redoOp(); e.preventDefault(); return; }
      }
      ...
      if(_markupActive&&(e.key==='Delete'||e.key==='Backspace')){
        ...
        if(!(ae&&(ae.tagName==='INPUT'||ae.tagName==='TEXTAREA')) &&
           window.DieselMarkup && DieselMarkup.hasSel && DieselMarkup.hasSel()){   // S314
          DieselMarkup.deleteSelected(); e.preventDefault(); return;
        }
      }
  ...
  function _close(){
    if(_markupActive && window.DieselMarkup && DieselMarkup.strokes && DieselMarkup.strokes.length && !_persistBusy){
  ...
  function _handleBack(){
     ...
     if(_markupActive){
       if(window.DieselMarkup && DieselMarkup.hasSel && DieselMarkup.hasSel()){ DieselMarkup.deselect(); return true; }

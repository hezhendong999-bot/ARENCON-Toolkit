...
(function (root) {
  ...
  var M = {
    ...
    _promptText:function(p, st, editId){
      ...
      var startFont  = editStroke ? (editStroke.fontSize||24) : (st.fontSize||24);   // natural px
      ...
      var host=(self.canvas&&self.canvas.parentNode)||document.body;
      host.appendChild(box);
      self._textInput=box;
      if(startText){ box.textContent=startText; }
      function curScaleY(){ var dh=self.canvas?self.canvas.offsetHeight:self.nh; return dh/self.nh; }
      function curScaleX(){ var dw=self.canvas?self.canvas.offsetWidth:self.nw; return dw/self.nw; }
      ...
      function positionBox(){
        ...
        var sf=screenFont();
        var cl=self.canvas?self.canvas.offsetLeft:0, ct=self.canvas?self.canvas.offsetTop:0;
        var sx=cl + anchor.x*curScaleX();
        var sy=ct + anchor.y*curScaleY();
        box.style.left=Math.round(sx-4)+'px';
        box.style.top =Math.round(sy-sf)+'px';
      }
      // expose so the lightbox can keep the box glued during pan/zoom
      self._repositionTextBox=function(){ if(self._textInput===box){ box.style.fontSize=screenFont()+'px'; positionBox(); } };
      function applyStyle(){
      ...
      function commit(){
        ...
        if(editStroke){
          ...
          else { var before=JSON.stringify(editStroke); editStroke.text=v; editStroke.fontSize=fontN; editStroke.color=curColor; editStroke.bg=curBg; editStroke.pts[0]={x:lx,y:ly};
            var eix=self.strokes.indexOf(editStroke); self._pushOp({t:'mod', idx:eix, before:before, after:JSON.stringify(editStroke)}); }
          delete editStroke._editing; self.redo=[]; if(self._onDirty) self._onDirty(); self.render(); cleanup(); return;
        }
        if(v.trim()){
          var ns={id:self._uid(), tool:'text', pts:[{x:lx,y:ly}], text:v, color:curColor, bg:curBg, fontSize:fontN, alpha:(st.alpha==null?1:st.alpha)};
          self.strokes.push(ns); self._pushOp({t:'add'}); self.redo=[]; if(self._onDirty) self._onDirty();
        }
        self.render(); cleanup();
      }
      ...
      var controller={
        ...
        getSize:function(){ return Math.round(fontN); },
        ...
        stepSize:function(dir){
          ...
          i=Math.max(0,Math.min(steps.length-1,i+dir)); fontN=steps[i]; applyStyle(); box.focus();
          return fontN;
        },
  ...
  var API = { drawText: drawText, install: install, VERSION: '1.0.0' };

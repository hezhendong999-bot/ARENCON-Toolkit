...
(function (root) {
  ...
  // ── S550 — TEXT WEIGHT. ───────────────────────────────────────────────────
  // The engine drew every text bold. FRT's drawing viewer has always drawn its
  // text at REGULAR weight and carries an explicit bold flag per object, so
  // adopting the engine as-is would have re-rendered the text on every drawing
  // in every report already issued to a client. Weight is therefore read off
  // the stroke: a stroke with no bold field (Diesel, Electric, FRT photos) is
  // bold exactly as before — provably unchanged, no host has to opt in.
  function _weightOf(s){
    if (!s || s.bold === undefined || s.bold === null) return 'bold';
    return s.bold ? '700' : '400';
  }

  // ── Paint (verbatim S459f branch body, weight hooked) ──
  function drawText(ctx, s, sx, sy){
    ...
    sx = sx || 1; sy = sy || 1;
        if(s._editing){ /* the on-photo box is showing; don't double-draw */ }
          ...
          ctx.font=_weightOf(s)+' '+fp+'px Calibri,Arial,sans-serif'; ctx.textBaseline='alphabetic';
  ...
  var M = {
    ...
    _promptText:function(p, st, editId){
      ...
      // S550: null = "engine default (bold)". Hosts that care return true/false.
      var curBold    = self._txtHooks.readBold.call(self, editStroke, st);
      ...
      box.style.cssText='position:absolute;z-index:10050;min-width:8px;max-width:calc(100% - 4px);outline:none;'+
        'font-family:Calibri,Arial,sans-serif;line-height:1.25;white-space:pre-wrap;'+
      ...
      function applyStyle(){
        ...
        box.style.fontWeight=_weightOf({bold:curBold});   // S550: match the painted weight
        box.style.color=curColor;
        box.style.background='rgba(20,18,24,.55)';   // editing backing pill (readability)
        positionBox();
      }
      // S550 — EDGE-DRAG-TO-MOVE, opt-in. Grabbing the box within a few px of its
      // border repositions the text while still editing; the interior keeps normal
      // caret behaviour. anchor is the SAME object commit reads, so the move sticks.
      // Coarse pointers get a wider band (gloves). Hosts that do not ask for it are
      // untouched — no listeners are attached at all.
      if (self._txtHooks.edgeDrag.call(self)) {
        ...
        box.addEventListener('pointerdown', function(ev){
          ...
          if (!(ix < EDGE || iy < EDGE || (r.width - ix) < EDGE || (r.height - iy) < EDGE)) return;
          ev.preventDefault(); ev.stopPropagation();
          _edrag = { sx: ev.clientX, sy: ev.clientY, ax: anchor.x, ay: anchor.y };
          try { box.setPointerCapture(ev.pointerId); } catch(_){}
        });
        box.addEventListener('pointermove', function(ev){
          if (!_edrag) return;
          var kx = curScaleX() || 1, ky = curScaleY() || 1;
          anchor.x = _edrag.ax + (ev.clientX - _edrag.sx) / kx;
          anchor.y = _edrag.ay + (ev.clientY - _edrag.sy) / ky;
          positionBox();
        });
        function _endEdge(ev){ if (!_edrag) return; _edrag = null; try { box.releasePointerCapture(ev.pointerId); } catch(_){} }
        box.addEventListener('pointerup', _endEdge);
        box.addEventListener('pointercancel', _endEdge);
      }
      ...
      function commit(){
        ...
        if(editStroke){
          ...
          else { var before=JSON.stringify(editStroke); self._txtHooks.applyEdit.call(self, editStroke, v, fontN, curColor, curBg, lx, ly, curBold);
        ...
        if(v.trim()){
          var ns=self._txtHooks.buildStroke.call(self, v, fontN, curColor, curBg, lx, ly, st, curBold);
  ...
  var TEXT_DEFAULT_HOOKS = {
    ...
    // S550 defaults: no host currently sets these, so behaviour is unchanged.
    readBold: function(){ return null; },      // null = engine default (bold)
    edgeDrag: function(){ return false; },     // no listeners attached
    placement: function(){
      var self=this;
      return {
  ...
  var API = { drawText: drawText, install: install, VERSION: '2.1.0' };

...
export const BACK_CSS = `
  ...
  button {
    ...
    padding:0;
    border-radius:6px;
    /* S523 (Mark): MUST be calc(16px + var(--ts)) — the header back uses
       exactly this. Hardcoding 16px made the viewer arrow 2px smaller on
       Mark's device (--ts = 2px) and silently broke text-size scaling for
       this button. Custom properties DO inherit through a shadow boundary,
       so --ts from body reaches in; the seal blocks page RULES, not tokens. */
...
export function upgradeBackButton(el) {
  ...
  let root;
  try {
    root = el.attachShadow({ mode: 'open' });   /* S523: open so console scans can read inside */
  } catch (e) {
    ...
    try {
      ...
      /* S523: `all:initial` collapsed the host to 0x0 (measured). Explicit
         reset instead — sized to the button so the flex row lays out
         correctly, with every property page CSS might set neutralised. */
      host.style.cssText = 'display:inline-flex;align-items:center;justify-content:center;' +
        'width:40px;height:34px;flex:0 0 40px;padding:0;margin:0;border:0;' +
        'background:none;box-shadow:none;line-height:0;font-size:0;' +

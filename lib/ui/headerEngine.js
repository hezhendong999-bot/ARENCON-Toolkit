...
function buildHeader(mountEl, config){
  ...
  function _rebuildDrawer(){
    // NODE-RELOCATION model (Mark's spec): the drawer holds the REAL header
    // controls (same DOM nodes), not rebuilt rows — so they look EXACTLY like the
    // bar buttons and their real dropdowns/onclick still work. Folded nodes are
    // physically moved into the drawer (in survival order → declared order for
    // readability); unfolded nodes are moved back to the bar in declared order.
    // Sign Out (if folded) pins to the bottom.
    // Move NON-folded controls back to the actions bar in DECLARED order first.
    built.forEach(function(b){
      if(folded.indexOf(b.node) === -1 && b.node.parentNode !== actions){
        // reinsert before the ☰ button, preserving declared order
        actions.insertBefore(b.node, moreBtn);
      }
    });
    // Re-order the bar: ensure declared order (nodes may have been moved around)
    built.forEach(function(b){
      if(folded.indexOf(b.node) === -1){ actions.insertBefore(b.node, moreBtn); }
    });

    // Move FOLDED controls into the drawer. Order: by survival rank ascending
    // (same as fold order) so the drawer reads top→bottom = least→most important,
    // but Sign Out always pinned last.
    var signoutNode = null;
    // clear drawer (nodes only; keep it empty of stale rebuilt content)
    while(drawer.firstChild) drawer.removeChild(drawer.firstChild);
    // gather folded in declared order
    var foldedInOrder = built.filter(function(b){ return folded.indexOf(b.node) !== -1; });
    foldedInOrder.forEach(function(b){
      if(b.item.isSignout){ signoutNode = b.node; return; }
      b.node.style.removeProperty('display');   // clear measurement-time hide → visible in drawer
      b.node.classList.add('in-drawer');
      drawer.appendChild(b.node);
    });
    if(signoutNode){
      signoutNode.style.removeProperty('display');
      signoutNode.classList.add('in-drawer');
      drawer.appendChild(signoutNode);
    }
  }
  ...
  function _measure(){
    ...
    // RESET for measurement: return every control to the actions bar (some may
    // be sitting in the drawer from a previous measure), in declared order, and
    // clear fold state so we measure true bar widths.
    built.forEach(function(b){
      b.node.classList.remove('in-drawer');
      if(b.node._hidden){ b.node.style.setProperty('display','none','important'); }
      else { b.node.style.removeProperty('display'); }
      if(b.node.parentNode !== actions) actions.insertBefore(b.node, moreBtn);
    });
    // enforce declared order in the bar (nodes may have been reordered by drawer moves)
    built.forEach(function(b){ actions.insertBefore(b.node, moreBtn); });
    folded = [];

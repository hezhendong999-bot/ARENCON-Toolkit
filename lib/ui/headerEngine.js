...
function buildHeader(mountEl, config){
  ...
  function _measure(){
    ...
    // ☰ is the ANCHOR — ALWAYS visible (Sign Out + Day/Night live in the drawer,
    // so ☰ must never disappear). Green cloud dot also never folds (rendered as a
    // signalSlot, not in `built`, so it's already exempt).
    ...
    // ☰ is ALWAYS in the bar, so its width is ALWAYS reserved.
    var moreW = _ctrlWidth(moreBtn) + _gap();

    // FOLD PRIORITY (Mark's spec, S455):
    //   Never fold: Back, logo, ☰, green dot (anchors).
    ...
    //      QR, then Undo/Redo, then Day/Night LAST. Day/Night survives longest in
    //      the bar but CAN still fold into ☰ (the anchor).
    // survivalRank: higher = survives longer in the bar. Default 0 (folds first).
    function rank(b){
      var k = b.item.key;
    ...
    // Fold sequence = ascending survivalRank (lowest folds first). Within the same
    // rank, fold rightmost-first (declared order reversed).
    var seq = built.filter(function(b){ return !b.node._hidden; });
    seq = seq.map(function(b,i){ return {b:b, i:i, r:rank(b)}; });
    ...
    // fold in sequence until the bar (+ the ALWAYS-reserved ☰) fits
    var si = 0;
    ...
    moreBtn.classList.add('has-folded');   // ☰ always present + styled
    _rebuildDrawer();
  }

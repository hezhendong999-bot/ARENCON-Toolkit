...
/* ═══ S505f (Mark) — HUB PAGE cards ═══════════════════════════════════════════
   "Hub page" is Mark's term for the per-project page you land on after clicking a
   project — the one that shows which report to use. "Dashboard" is the page
   listing ALL projects. They are DIFFERENT SCOPES and their help never mixes:
   the Dashboard's panel shows only Dashboard cards, the Hub page's only these.
   Small 66×66 icons, same compact scale as the rest of the platform. ══════════ */
function _hpIc(inner){
...
var HPART = {};
HPART.info = _hpIc('<rect x="15" y="16" width="36" height="34" rx="4" fill="#fff" stroke="#94A3B8"/><rect x="21" y="24" width="24" height="4" rx="2" fill="#CBD5E1"/><rect x="21" y="32" width="18" height="4" rx="2" fill="#CBD5E1"/><path d="M38,42 l6,-6 l4,4 l-6,6 h-4 z" fill="#9C2742" fill-opacity=".8"/>');
HPART.photos = _hpIc('<rect x="13" y="20" width="24" height="20" rx="3" fill="#fff" stroke="#94A3B8"/><rect x="27" y="27" width="24" height="20" rx="3" fill="#fff" stroke="#2C4770"/><circle cx="36" cy="35" r="4" fill="none" stroke="#2C4770" stroke-width="1.8"/>');
HPART.storage = _hpIc('<path d="M22,40 a8,8 0 0 1 0,-16 a11,11 0 0 1 21,3 a7,7 0 0 1 -1,13 z" fill="#D6E8F4" stroke="#2C7FB8" stroke-width="1.5"/><rect x="24" y="44" width="18" height="3" rx="1.5" fill="#CBD5E1"/>');
HPART.actions = _hpIc('<rect x="15" y="20" width="16" height="12" rx="3" fill="#FBEDD3" stroke="#D98A1E"/><rect x="35" y="20" width="16" height="12" rx="3" fill="#FBE3E9" stroke="#C0445F"/><rect x="15" y="38" width="36" height="10" rx="3" fill="#F1F5F9" stroke="#94A3B8"/>');
HPART.qr = _hpIc('<rect x="16" y="16" width="13" height="13" rx="2" fill="none" stroke="#1B1A22" stroke-width="2.4"/><rect x="37" y="16" width="13" height="13" rx="2" fill="none" stroke="#1B1A22" stroke-width="2.4"/><rect x="16" y="37" width="13" height="13" rx="2" fill="none" stroke="#1B1A22" stroke-width="2.4"/><rect x="39" y="39" width="4" height="4" fill="#1B1A22"/><rect x="46" y="46" width="4" height="4" fill="#1B1A22"/>');
HPART.fold = _hpIc('<rect x="15" y="17" width="36" height="8" rx="3" fill="#EDE9F2" stroke="#CBD5E1"/><rect x="15" y="29" width="36" height="8" rx="3" fill="#EDE9F2" stroke="#CBD5E1"/><path d="M28,45 l5,5 l5,-5" fill="none" stroke="#5E5B68" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/>');

var HUBPAGE_CARDS = [
{
  id:'hp-info', area:'This project', date:'2026-07-26', compact:true,
  title:'Project info and editing',
  pts:['The top card carries the <b>client, address and the rest of the project record</b>.',
       'Edit Project sits in that card\u2019s own header \u2014 changes apply to the project everywhere.',
...
{
  id:'hp-photos', area:'This project', date:'2026-07-26', compact:true,
  title:'Project photos in one place',
  pts:['<b>Every photo from every report</b> on this project gathers in the Project Photos section.',
       'Download all pulls the lot in one go \u2014 handy for a client package.',
       'If it says none found, the reports either have no photos yet or they are still uploading.'],
  chips:[['Everyone','c-where']],
  terms:'project photos gallery all photos download all bulk client package where are my pictures images none found empty',
  art:HPART.photos
},
{
  id:'hp-storage', area:'This project', date:'2026-07-26', compact:true,
  title:'Cloud storage for this project',
  pts:['Shows <b>how much space this project\u2019s photos and files use</b> in cloud storage.',
       'Refresh re-reads it live \u2014 the number is a snapshot, not a running total.',
       'Useful when a project feels slow to sync or you are checking a big photo set landed.'],
  chips:[['Everyone','c-where']],
  terms:'cloud storage space used size r2 photos files refresh how big is this project sync check usage',
  art:HPART.storage
},
{
  id:'hp-actions', area:'This project', date:'2026-07-26', compact:true,
  title:'Archive, delete and restore',
  pts:['<b>Archive</b> takes a finished project off the active board without losing anything.',
       '<b>Delete Project</b> is reversible \u2014 it goes to the Dashboard\u2019s deleted view and can be brought back.',
       'Delete Forever is permanent and only appears where that is allowed. Restore returns an archived project.'],
  chips:[['Everyone','c-where']],
...
{
  id:'hp-qr', area:'This project', date:'2026-07-26', compact:true,
  title:'QR code for this project',
  pts:['Puts <b>this project</b> on screen as a QR code \u2014 scan it to open the same project on a tablet.',
       'Saves typing a project number into a field device.',
       'It points at this project, not at the tool in general.'],
  chips:[['Everyone','c-where']],
  terms:'qr code scan open on tablet phone transfer project to device barcode send to field',
  art:HPART.qr
},
{
  id:'hp-sections', area:'Getting around', date:'2026-07-26', compact:true,
  title:'Folding the page',
  pts:['Every section header <b>folds and unfolds on tap</b> \u2014 handy on a tablet.',
       'Fold All collapses the lot so you can see the whole page at once.',
       'Back to Dashboard returns to the full project list; the star marks this project as a favourite.'],
  chips:[['Everyone','c-where']],
...
/* ═══ SCOPED REGISTRATION (S505f, Mark's standing rule) ═══════════════════════
   Two scopes from one file, because the Dashboard and the Hub page are two
   different pages living in the same document. Nothing is copied: the cards are
   PARTITIONED by id, so each card's text has exactly one home. The three global
   chrome cards (theme / PIN lock / tools portal) describe controls present on
   BOTH pages, so both scopes reference the SAME card objects — one source, no
   drift (registerHelp copies each card and stamps its scope). ═════════════════ */

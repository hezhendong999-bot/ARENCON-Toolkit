...
function _exportPDFGo() {
  ...
  // S503: warm the annotation overlay before baking. On a COLD export (S4 tab never
  // opened) the first renderChartAnnotations hits the self-heal path, which disables
  // Chart.js datalabels and schedules the real overlay render on an 80ms timeout — so a
  // synchronous bake read an EMPTY overlay and the PDF lost every label. Pre-rendering
  // here hooks each chart first; the bake's own render call then populates synchronously.
  [['chart3pt',chart3pt],['netChart3pt',netChart3pt],['pldChart',pldChart],['pldNetChart',pldNetChart]].forEach(function(p){
    try{ if(p[1] && typeof renderChartAnnotations==='function') renderChartAnnotations(p[1],p[0]); }catch(_){}
  });

...
function _exportPDFGo() {
  ...
  setTimeout(function(){
    // 1) overlay is authoritative on screen — (re)render it now that the resize has settled
    _chartPairs.forEach(function(p){ try{ if(p[1] && typeof renderChartAnnotations==='function') renderChartAnnotations(p[1],p[0]); }catch(_){} });
    // 2) bake the freshly-rendered overlay onto each canvas bitmap
    _chartPairs.forEach(function(p){ _bakeAnnotationsOntoCanvas(p[1],p[0]); });
    // 3) capture immediately — synchronous, so no patched update() can clear the bake first
    _realExportPDF();

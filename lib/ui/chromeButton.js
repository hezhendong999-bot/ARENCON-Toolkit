...
export function upgradeChromeButton(el, opts) {
  ...
  /* S581 (Mark, console noise): this inner button is presentational — the HOST
     element owns focus and the accessible name. It used to carry BOTH
     tabindex=-1 and aria-hidden=true, and Chrome logs a hard warning for every
     one of them ("Blocked aria-hidden on an element because its descendant
     retained focus") because aria-hidden must never wrap something focusable
     or focused, and a click still lands focus here. That warning fired dozens
     of times per page and buried real errors in the console.
     Correct pattern, per the warning's own advice: role=presentation removes
     it from the accessibility tree WITHOUT the aria-hidden focus conflict, and
     inert (where supported) keeps it unfocusable. Same end result — the host
     is the only thing screen readers and the tab order see — with no warning. */
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.tabIndex = -1;                       // the host owns focus
  btn.setAttribute('role', 'presentation'); // the host owns the accessible name
  try { btn.inert = true; } catch (e) {}    // unfocusable without aria-hidden's conflict

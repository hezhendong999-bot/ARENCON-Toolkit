...
export function createChangeBadges(cfg) {
  ...
  function _toggle(path) {
    ...
    /* ═══ S604b — THE POPOVER LEAVES THE FIELD CELL ═════════════════════════
       It was absolutely positioned INSIDE the field's own small cell. Two
       field failures from one root: on desktop a 300px panel hanging off a
       cell at the card's right edge was clipped by the card and painted under
       the next card's stacking layer (each Bold card's backdrop-filter makes
       its own layer, so z-index cannot cross them); on the phone it inherited
       the dark theme's TRANSLUCENT card token as its background and rendered
       see-through over the content behind it. Now: a viewport-level layer on
       <body> with a SOLID theme-resolved surface, positioned from the field's
       rectangle, clamped to the screen, flipping above the field when there
       is no room below, scrolling internally when taller than the viewport.
       Scrolling the page closes it (its anchor moves; stale placement lies). */
    var dark = false;
    try {
      dark = document.documentElement.getAttribute('data-theme') === 'dark' ||
             document.body.classList.contains('dark-mode');
    } catch (_) {}
    if (dark) pop.classList.add('acb-dark');
    document.body.appendChild(pop);
    var r = input.getBoundingClientRect();
    var vw = window.innerWidth, vh = window.innerHeight;
    var w = Math.min(320, vw - 24);
    pop.style.width = w + 'px';
    var left = Math.max(12, Math.min(r.right - w, vw - w - 12));
    pop.style.left = left + 'px';
    pop.style.visibility = 'hidden';
    var h = pop.offsetHeight;
    var below = r.bottom + 8, above = r.top - 8 - h;
    var top = (below + h <= vh - 12) ? below : (above >= 12 ? above : Math.max(12, vh - 12 - h));
    pop.style.top = top + 'px';
    pop.style.visibility = '';
    var _onScroll = function () { _closePop(); };
    window.addEventListener('scroll', _onScroll, { capture: true, once: true });
    window.addEventListener('resize', _onScroll, { once: true });
    setTimeout(function () {
  ...
  function _injectCss() {
    ...
    s.textContent =
      ...
      '.acb-pop{position:fixed;z-index:100000;width:300px;' +
        /* S604b: SOLID surfaces, deliberately not the card tokens — the Bold
           dark card fill is translucent-with-blur, which is exactly what made
           this popover see-through on the phone. A floating review panel must
           be opaque in both modes. */
        'background:#ffffff;color:#1B1A22;border:1px solid rgba(27,26,34,.16);' +
        'border-radius:14px;box-shadow:0 12px 34px rgba(27,26,34,.28);padding:12px 14px;' +
        'font-family:Calibri,sans-serif;text-align:left;max-height:min(60vh,420px);overflow-y:auto;overscroll-behavior:contain;}' +
      '.acb-pop.acb-dark{background:#17151c;color:#f4f3f6;border-color:rgba(244,243,246,.16);' +
        'box-shadow:0 12px 34px rgba(0,0,0,.6);}' +
      '.acb-pop.acb-dark .acb-row{border-top-color:rgba(244,243,246,.1);}' +
      '.acb-pop.acb-dark .acb-keep{border-color:rgba(244,243,246,.22);color:#a09aa8;}' +

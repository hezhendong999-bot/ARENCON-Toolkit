...
export const ENGINE2_VERSION = '2.4.1';
...
.title{ font:600 calc(19px + var(--ts,0px)) Calibri,sans-serif; letter-spacing:.5px; color:#fff;
  white-space:nowrap; overflow:hidden; text-overflow:ellipsis; min-width:64px;
  /* S488: min-width:64px = the S448 lock 'title truncates via ellipsis but
     ...
     floor — later declaration wins) is deleted; 64px now IS the flex minimum. */ }
...
export function buildHeader2(mountEl, cfg){
  ...
  function refit(){
    ...
    /* S488 v2.4.1 — MEASUREMENT DONE RIGHT (per the S448 lock, after two
       ...
       never hides at any width), presence/badges when on — plus a 64px title

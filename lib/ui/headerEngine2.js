...
export const ENGINE2_VERSION = '2.3.0';
...
.savets{ display:none; font-size:10.5px; color:rgba(255,255,255,.5); margin-right:4px;
  ...
  if (t === 'menu'){
    ...
    (item.items || []).forEach(mi => {
      ...
      if (mi.repairSection){
        ...
        if (mi.hubOnly){ tog._hubOnly = true; tog.classList.add('hide'); box._secBox = true; }
        menu.appendChild(sec);
        return;
      }
      const b = _el('button', { 'class': mi.danger ? 'danger' : null });
 ...
 *   ctl.setSaveStamp(text) · ctl.setAdmin(bool)

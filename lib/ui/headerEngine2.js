...
/* S683 (Mark) — ROLE BADGE on the account avatar. Elevated accounts only:
   SA for super admin, A for admin. Inspectors and viewers get nothing, which
   is the point — a badge everyone carries says nothing.

   BOTTOM-RIGHT, not top-right: top-right is where unread counts sit in every
   app anyone uses, and a role there reads as "1 notification".

   The 2px ring is load-bearing. Ring colours run from #0FFF7F to #99A0BA, and
   without a ring in the BAR colour a light badge on a light avatar disappears.
   Same trick .pav already uses one line above.

   Colours are passed by the host, never hard-coded here: the Hub owns what a
   role means and already has values for it. --rbc is the light-mode fill;
   --rbc-dark lifts it on the dark bar, where the Hub's #7C3AED sits too close
   to #1d1b24 to read at 16px. */
.rolebadge{
  position:absolute; right:-3px; bottom:-3px;
  min-width:16px; height:16px; padding:0 3px; border-radius:9px;
  display:inline-flex; align-items:center; justify-content:center;
  font:800 9px/1 Calibri,sans-serif; letter-spacing:.2px;
  color:#fff; background:var(--rbc,#4A4750);
  box-shadow:0 0 0 2px var(--b-chrome-bg,#1d1b24);
  pointer-events:none;
}
:host([data-theme="dark"]) .rolebadge{
  background:var(--rbc-dark,var(--rbc,#4A4750)); color:#12101a;
}
...
export function buildHeader2(mountEl, cfg){
  ...
  if (cfg.account) mkMe();
  /* S683b — THE ONE role→badge mapping in the toolkit. Every tool's header
     reads this; none of them define their own, so a role cannot mean one
     thing in the Hub and another in FRT.

     Accepts EITHER spelling, because two paths feed it: the profiles table
     stores 'super_admin' / 'admin', while headerIdentity.js hands back the
     display strings 'Super Admin' / 'Admin'. Normalising here means neither
     caller has to know about the other.

     Colours are the Hub's existing .role-super / .role-admin values, so a
     role is the same purple and the same blue on the avatar as on its pill
     in the admin panel. The dark pair exists because those pills were written
     pale-background + saturated-text with no dark-mode override anywhere: at
     16px on the #1d1b24 bar the saturated purple is nearly the bar itself.

     Inspector and viewer return null on purpose. A badge everyone carries
     says nothing; this one only appears where it means something. */
  /* S730 — WHAT A PERSON CAN TYPE ABOUT THEMSELVES IS ESCAPED HERE.
     Names, initials and emails come from profiles, which the signed-in user
     edits. This header is shared by every tool, so a payload in one person's
     name would run in every colleague's header on FRT, Hub, Diesel and
     Electric. Four-entity escape (& < > "), identical to frt/js/lib/esc.js;
     copied rather than imported because lib/ must never depend on frt/.
     Colours land inside style="" as custom properties: only #hex passes, so a
     quote or semicolon in a roster colour cannot break out of the attribute. */
  function _escHtml(s){
    return String(s == null ? '' : s)
      .replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  }
  function _safeColour(c, fb){
    return /^#[0-9a-fA-F]{3,8}$/.test(String(c || '')) ? c : (fb || '#888');
  }
  function _roleBadgeFor(role){
    const k = String(role || '').toLowerCase().replace(/[\s_-]/g, '');
    if (k === 'superadmin') return { text:'SA', colour:'#7C3AED', colourDark:'#C4B5FD', title:'Super Admin' };
    if (k === 'admin')      return { text:'A',  colour:'#1D4ED8', colourDark:'#93C5FD', title:'Admin' };
    return null;
  }
  ...
  const paintMe = () => {
    ...
    const a = cfg.account;
    meBtn.innerHTML = `<span class="pav" style="--pc:${_safeColour(a.colour)}">${_escHtml(a.initials||'?')}</span>`
      /* S683b: the badge is DERIVED from the role the account block already
         carries. S683 made it opt-in — each host had to pass a roleBadge — and
         only the Hub did, so FRT/Diesel/Electric showed a bare avatar on the
         same shared header. Opt-in on a unified engine is drift with extra
         steps. An explicit roleBadge still wins for anything that needs one,
         but nothing has to supply it. */
      + (() => {
          const rb = a.roleBadge || _roleBadgeFor(a.role);
          return (rb && rb.text)
            ? `<span class="rolebadge" title="${rb.title||''}" style="--rbc:${rb.colour||'#4A4750'};--rbc-dark:${rb.colourDark||rb.colour||'#4A4750'}">${rb.text}</span>`
            : '';
        })();
    meMenu.innerHTML =
      `<div style="display:flex;align-items:center;gap:10px;padding:9px 10px 11px;border-bottom:1px solid currentColor;opacity:1">
         <span class="pav" style="--pc:${_safeColour(a.colour)};width:32px;height:32px;font-size:11.5px;margin:0"> ${_escHtml(a.initials||'?')}</span>
         <span><span style="display:block;font-weight:700">${_escHtml(a.name||'')}</span>
         <span style="display:block;font-size:11.5px;opacity:.65">${_escHtml(a.email||'')}</span></span></div>`
  ...
  if (!cfg.account && (cfg.resolveIdentity !== false)) {
    import('./headerIdentity.js').then(m => m.resolveIdentity()).then(id => {
      ...
      ctl.setAccount({
        ...
        /* S683b: role travels with identity. Without it the engine can resolve
           WHO you are but not WHAT you are, and the avatar badge silently never
           appears on any tool that takes this path. */
        role: id.role,
  ...
  const ctl = {
    ...
    setInspector(o){
      ...
      c.innerHTML = o.name ? _escHtml(o.name) : (o.placeholder || '&#128100; Set Name');
    },
    setUndoRedo(o){
      o = o || {};
    ...
    setPresence(o){
      ...
      if (Array.isArray(o.people)){
        ...
        presence.querySelector('.pstack').innerHTML =
          show.map(p => `<span class="pav" title="${_escHtml(p.name||'')}" style="--pc:${_safeColour(p.colour)}">${_escHtml(p.initials||'?')}</span>`).join('')
    ...
    setControlIcon(key, html){
      ...
      /* S697 — ROOT CAUSE of "the More button is not even a button". A MENU
         control's node is its WRAPPER (.mwrap: the button PLUS its dropdown),
         not the button. Writing innerHTML here therefore deleted both and left
         a bare text node that looks like a label and does nothing — and it
         happened about a second after load, when the host flipped the
         save-flag dot on, so the header came up correct and then went dead.
         Icon controls still are their own button, so they are unchanged: aim
         at the .hbtn when there is one, at the node itself otherwise. */
      const target = n.classList && n.classList.contains('mwrap')
        ? (n.querySelector(':scope > .hbtn') || n)
        : n;
      target.innerHTML = html; target._iconLight = html; target._iconDark = html;
    },
    setProjectBar(o){
      o = o || {};

...
export const ENGINE2_VERSION = '2.4.0';
...
export function buildHeader2(mountEl, cfg){
  ...
  function refit(){
    ...
    /* S488 FIX (the real desktop-fold bug): .left is a flex-grow container —
       at 1919px it stretches to ~1100px, so subtracting its offsetWidth made
       avail self-referentially equal whatever the actions already used, and
       the rightmost control (Sign Out) sat permanently 1px from folding.
       Measure the left cluster's CONTENT edge instead of its stretched box. */
    let leftEdge = 0;
    const barL = bar.getBoundingClientRect().left;
    Array.from(left.children).forEach(c => {
      if (getComputedStyle(c).display === 'none') return;
      const r = c.getBoundingClientRect().right - barL;
      if (r > leftEdge) leftEdge = r;
    });
    const avail = bar.clientWidth - leftEdge - (moreBtn.offsetWidth || 42) - 12;

...
export function buildHeader2(mountEl, cfg){
  ...
  const paintMe = () => {
    ...
    meBtn.innerHTML = `<span class="pav" style="--pc:${a.colour||'#888'}">${a.initials||'?'}</span>`
      /* S683: absent roleBadge renders nothing, so every tool that does not
         pass one is byte-identical to before. Opt-in, never a default. */
      + (a.roleBadge && a.roleBadge.text
          ? `<span class="rolebadge" title="${a.roleBadge.title||''}" style="--rbc:${a.roleBadge.colour||'#4A4750'};--rbc-dark:${a.roleBadge.colourDark||a.roleBadge.colour||'#4A4750'}">${a.roleBadge.text}</span>`
          : '');
    meMenu.innerHTML =

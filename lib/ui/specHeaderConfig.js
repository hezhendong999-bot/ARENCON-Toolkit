 ...
 *   • Word         — S11. Exports the document as a native .docx through
 *                    spec/js/docx.js. The tool's deliverable: before this the
 *                    stack could show a spec but not hand one to a client.
 ...
 *   h.onImport        Import…
 *   h.onWord          Word export (.docx)
 *   h.onMaster        open the Master File (spec/master/)
 *   h.onPublish       Publish…
 *   h.onHelp          Help panel
...
export function specHeaderConfig(h) {
  ...
  return {
    ...
    actions: [
      ...
      /* Word export is the OUTPUT: it reads, writes nothing to the master, and
         can be run as often as anyone likes — graphite, the same family neutral
         as Import. Not burgundy: brand stays inside the page (S10 ruling). */
      { key: 'word', type: 'text', id: 'btn-word', label: 'Word',
        bg: '#4A4750', title: 'Export the document as a Word file',
        drawerLabel: 'Word', onClick: h.onWord || null },

      /* S13. The Master File is the firm's side of the same product: system
         packages, standards, closeouts, the proposal inbox. One door leads to
         the other; neither is linked from the portal (Owner ruling). */
      { key: 'master', type: 'text', id: 'btn-master', label: 'Master file',
        bg: '#4A4750', title: 'System packages, standards, closeouts and proposals',
        drawerLabel: 'Master file', onClick: h.onMaster || null },

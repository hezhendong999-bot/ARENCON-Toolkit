...
export function specHeaderConfig(h) {
  ...
  return {
    ...
    actions: [
      ...
      /* Publish IS this tool's primary commit — it puts wording in front of
         every future job in the firm. Brand burgundy is reserved for primary
         CTAs and this is the one primary CTA the spec tool has, so it earns
         the colour rather than borrowing it. Fixed #9C2742 with white text
         reads correctly in both modes; it is not the theme token, so it does
         not shift when the page does. The HOST keeps it disabled until the
         caller is a principal AND the version navigator exists. */
      { key: 'publish', type: 'text', id: 'btn-publish', label: 'Publish&hellip;',
        bg: '#9C2742', title: 'Publish a new master version',

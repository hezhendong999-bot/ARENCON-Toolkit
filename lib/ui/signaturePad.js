...
function initSig(canvasId) {
  ...
  canvas.addEventListener('mousedown', e=>{ drawing=true; ctx.strokeStyle=_sigInk(); const p=getPos(e); _cur={pts:[{x:p.x,y:p.y}]}; ctx.beginPath(); ctx.moveTo(p.x,p.y); });
  ...
  canvas.addEventListener('touchstart', e=>{ e.preventDefault(); drawing=true; ctx.strokeStyle=_sigInk(); const p=getPos(e); _cur={pts:[{x:p.x,y:p.y}]}; ctx.beginPath(); ctx.moveTo(p.x,p.y); }, {passive:false});

...
/* ═══ S736 — MINIMUM SAVE FORMAT (the one interruption commercial apps allow) ═══
   The database refuses a save from a page that is too old to save correctly.
   How: every request this page sends to the database carries the label
   "arencon-gen/<SAVE_GEN>" (in x-client-info, a header Supabase already
   accepts). A guard on tool_data compares it with app_min_save_gen.min_gen.
   Below the minimum the save is refused with HTTP 426 and this page shows one
   quiet notice: out of date, tap to reload. Nothing is lost — a refused save is
   an ordinary failed push, so every tool keeps its own local copy and sends it
   after the reload, exactly as it does after a dropped signal.
   TO USE IT (when a push changes HOW reports are saved): raise SAVE_GEN here in
   that same push; once it is live, set min_gen to the same number. min_gen = 0
   is OFF. Never raise min_gen above a SAVE_GEN that is not yet live — that
   would refuse every page, current ones included.
   ONE place covers every tool: all live tools load this engine, and it wraps
   the page's fetch once, touching only requests to the database's /rest/v1/.
   Anything else passes through untouched, and any fault in the wrapper falls
   through to the original fetch — this code can never be why a save fails. */
export var SAVE_GEN = 1;
function _showOutdated() {
  try {
    if (document.getElementById('arcOutdated')) return;
    var p = document.createElement('div');
    p.id = 'arcOutdated';
    p.setAttribute('role', 'alert');
    p.textContent = 'This page is out of date \u00b7 tap to reload and keep saving';
    p.style.cssText =
      'background:rgba(201,138,74,.97);color:#fff;border:1px solid rgba(255,255,255,.25);' +
    ...
    p.style.setProperty('z-index', '2147483001', 'important');
    p.style.setProperty('max-width', '90vw', 'important');
    p.addEventListener('click', function () { try { window.location.reload(); } catch (e) {} });
    document.body.appendChild(p);
    _log('save refused: this page is below the minimum save format — asked for a reload');
  } catch (e) {}
}
(function _installSaveLabel() {
  try {
    if (typeof window === 'undefined' || typeof window.fetch !== 'function') return;
    if (window.__arcSaveGen) return;            // once per page
    window.__arcSaveGen = SAVE_GEN;
    var orig = window.fetch;
    window.fetch = function (input, init) {
      var tagged = false;
      try {
        var url = (typeof input === 'string') ? input : ((input && input.url) || String(input));
        if (url.indexOf('.supabase.co/rest/v1/') !== -1) {
          var h = new Headers((init && init.headers) ||
                              (typeof input !== 'string' && input && input.headers) || undefined);
          if (!h.has('x-client-info')) h.set('x-client-info', 'arencon-gen/' + SAVE_GEN);
          init = Object.assign({}, init || {}, { headers: h });
          tagged = true;
        }
      } catch (e) { tagged = false; }
      if (!tagged) return orig.apply(window, arguments);
      return orig.call(window, input, init).then(function (resp) {
        try { if (resp && resp.status === 426) _showOutdated(); } catch (e) {}
        return resp;
      });
    };
  } catch (e) {}
})();
/* S736 — THE IDLE AND BACKGROUND SWAPS ARE GONE (Mark: "not commercial grade,
   very messy"). They reloaded a page on a guess that nobody was using it, and
   every wrong guess grew another guard. A new build now goes live the way
   commercial apps do it: on the next real page load (the worker has already
   switched, so opening any tool or relaunching gets it free), on a navigation
   the host hands us via applyIfStaged(), in the first moments of a page, or on
   a tap. Nothing reloads a page someone is using. */
...
function _blockReason() {
  ...
  /* S733 — A VISIBLE PASSWORD FIELD MEANS SOMEONE IS SIGNING IN. Focus alone
     is not enough: tapping Sign In moves focus to the button and the request
     is then in flight. A reload at that moment lands on an empty sign-in
     form and the person is asked again — which is how a cold-start swap
     became "the login never works the first time". Any tool, any host. */
  try {
    /* S735 — every password field, not the first. querySelector returned the
       first match; a hidden one earlier in the DOM would mask a visible
       sign-in field (not live today — Lane B, S734). */
    var pws = document.querySelectorAll('input[type="password"]');
    for (var pi = 0; pi < pws.length; pi++) {
      if (pws[pi] && pws[pi].offsetParent !== null) return 'finish signing in to apply';
    }
  } catch (e) {}
...
function _showPill() {
  ...
  p.title = 'A new version is ready — it loads next time you open a page; tap to load it now';
...
function _onStaged() {
  ...
  /* S595 — DO NOT APPLY HERE. The first version applied immediately if the
     ...
     idle five minutes, or tap the pill.
     S736 — superseded: it waits for the next real page load, a host navigation
     (applyIfStaged) or a tap. Leaving the tab or going idle no longer
     reloads anything. */
}
...
export function initLiveUpdate(cfg) {
  ...
  // 3. Coming back to the tab asks whether something newer exists (S736: it
  //    never reloads — the activity watchers that fed the idle swap are gone).

  /* S736 — RETIRED: the swap-while-away described below was removed. Kept
     as history; the handler now only asks for a newer build on return.
  ...
  document.addEventListener('visibilitychange', function () {
    if (document.hidden) return;   // S736: no swap while away
    checkNow();                    // back in front: ask, never reload
  });
...
/* S734 — A TOOL MAY SAY "THIS IS A FRESH ARRIVAL" AFTER BOOT.
   The arrival window (S680) opens when the engine is armed. A tool that holds
   people at a sign-in screen correctly blocks updates there (S733), so by the
   time the person is through, the window has closed and a waiting build sits
   until idle or a tap. The moment just after sign-in is as safe as a fresh page:
   nothing typed, nothing open, the session already on disk. The host calls this
   there; the window re-opens and a staged build applies once the ordinary checks
   pass (retried each second while the window lasts, because a reload blocked for
   one beat — a field still releasing focus — would otherwise never be retried).
   Every rule still applies: busy, dialogs, focus, and the S717 back-to-back
   cooldown. Hosts that never call it behave exactly as before. */
/* S736 — REPLACED BY applyIfStaged. markFreshArrival re-opened a 20 s window
   and retried every second: another timer guessing at a safe moment. A host
   now calls applyIfStaged() AT the moment the person navigates or finishes
   signing in. One check, no timer: if a build is waiting and nothing is open,
   the new version loads right there and lands where they were going (flush,
   then capture, then reload — the ordinary apply). Otherwise nothing happens
   and the pill stays. Returns true if the reload has started. The S717
   back-to-back guard still holds. */
export function applyIfStaged(why) {
  if (!_staged || _applying) return false;
  if (!_safeNow() || _recentAutoApply()) return false;
  _apply('navigation (' + (why || 'host') + ')');
  return _applying;
}

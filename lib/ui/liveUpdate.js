...
function _showPill() {
  ...
  p.style.cssText =
    'position:fixed;right:14px;bottom:14px;z-index:9001;cursor:pointer;' +
    'background:rgba(46,158,114,.95);color:#fff;border:1px solid rgba(255,255,255,.25);' +
    'font:600 12px/1.3 Calibri,sans-serif;padding:7px 13px;border-radius:16px;' +
...
export function initLiveUpdate(cfg) {
  ...
  document.addEventListener('visibilitychange', function () {
    ...
    var wasAway = _hiddenSince ? (Date.now() - _hiddenSince) : 0;
    _hiddenSince = 0;
    _lastActivityAt = Date.now();
    checkNow();                                  // is there something newer?
    if (_staged && wasAway >= BG_APPLY_MS) {
      // They left and came back — the normal, invisible case. Applying on
      // RETURN rather than on leaving means the reload happens against a
      // foregrounded page, which mobile browsers handle reliably.
      _apply('returned after ' + Math.round(wasAway / 1000) + 's away');
    }
  });

...
function srcOf(ph) {
  if (!ph) return '';
  /* S493 — `d` added. FRT stores inline photo bytes as `dataUrl`; Electric and
     DD store them as `d` (DD declares the shape literally: {d: dataUrl}).
     Reading only `dataUrl` meant any Electric or DD photo not yet uploaded to
     R2 resolved to empty and was dropped by the caller's `if (!origin) return`.
     Silent: the export succeeded, the ZIP was just missing photos. */
  return r2h(ph.r2Url || ph.dataUrl || ph.d || '');
}
...
function photoIdentityKey(photo) {
  ...
  var bytes = photo.dataUrl || photo.d || photo.thumb || null;
...
function ddPhotos(data) {
  ...
  function push(ph, ref, sectionKey) {
    ...
    var idk = photoIdentityKey(ph);
    if (idk) { if (seen[idk]) return; seen[idk] = true; }
    /* S493: DD photo objects are {d, caption} — no date of their own — so most
       land in "No date". Left as-is deliberately: inventing a date would be
  ...
  /* S493 — key names corrected against DD's ACTUAL saved payload (collectState).
     DD writes `clState` and `deficList`. This read `itemState || items` and
     `deficiencies`, none of which DD has ever written, so checklist and
     deficiency photos silently exported as nothing — only site photos came
     through. The old names are kept as fallbacks in case an older record used
     them, but DD's live shape is the one that matters. */
  var items = data.clState || data.itemState || data.items || {};
  Object.keys(items).forEach(function (id) {
    var st = items[id];
    if (!st || !st.photos) return;
    st.photos.forEach(function (p, pi) { push(p, 'Item ' + id, 'item_' + id + '_' + pi); });
  });
  var dlist = data.deficList || data.deficiencies || [];
  dlist.forEach(function (d, di) {

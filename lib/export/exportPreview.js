...
export function createFlowLayout(env){
  ...
  function _cardMeta(el){
    ...
    // S468 T2 (+S469: two rows per Mark — 72 chars cut findings off mid-meaning
    // on 6360.08; ~170 chars ≈ two lines gives the actual finding while staying
    // bounded by construction): bounded context stub — a short grey line
    // identifying the item on a continuation page, so the contractor never
    // answers a box whose subject is on another sheet. TRUNCATED, never the
    // full write-up: a full repeat re-triggers the very overflow that forced
    // the split and reads as a duplicate deficiency (a real double-response
    // hazard). It is an identifier, not a second copy.
    // S469: text gathered per block element and joined with spaces —
    // textContent alone welds "…sprinkler room:The new…" across block edges.
    var _dEl=el.querySelector(SEL.desc);
    var _dTxt='';
    if(_dEl){
      var _parts=[];
      (function _dw(n){
        for(var di=0;di<n.children.length;di++){
          var dc=n.children[di];
          if(dc.tagName==='UL'||dc.tagName==='OL'){_dw(dc);}
          else _parts.push(String(dc.textContent||'').trim());
        }
      })(_dEl);
      _dTxt=(_parts.length?_parts.join(' '):String(_dEl.textContent||'')).replace(/\s+/g,' ').trim();
    }
    if(_dTxt.length>170)_dTxt=_dTxt.slice(0,170).replace(/\s+\S*$/,'')+'\u2026';
    var _dEsc=_dTxt.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
    return{
      num:num,pin:pin,
      hasMini:!!el.querySelector(SEL.mini),
      contBand:'<div class="item-contband"><span class="dc-itemnum">'+num+'</span>'+(pin?' <span class="pinref-dark">'+pin+'</span>':'')
        +(_dEsc?' <span style="color:#928E9C;font-weight:400;font-style:italic;">\u2014 '+_dEsc+'</span>':'')
        +' <span class="cont">continued</span></div>',
  ...
  function _breakCandidates(el){
    ...
    // S468 T3: a long bullet list is ONE desc child — a 30-bullet <ul> was an
    // unsplittable atom, and that is exactly the shape of a half-page write-up
    // (pdf.js _descHtml folds bullet runs into a single <ul class="dc-bul">).
    // Offer seams between <li> items, from the SECOND on: a list never leaves
    // its first bullet behind, and MIN_LAND still guards sliver landings.
    for(var d3=0;d3<ds.length;d3++){
      var uls=ds[d3].querySelectorAll('ul,ol');
      for(var u2=0;u2<uls.length;u2++){var ls=uls[u2].children;for(var l2=1;l2<ls.length;l2++)push(ls[l2]);}
    }
    ...
    while(idx<blocks.length){
      ...
      if(isBand(blocks[idx].type)){
        ...
        if(end-1>idx&&blocks[end-1].type==='defCard'){
          ...
          var cLand=cExt;
          // S468 T1: a response-box card reserves its FULL height under a band,
          // not just its first-seam landing — otherwise the band stays put and
          // the card splits beneath it even when band + whole card would fit a
          // fresh page together. Non-crb cards keep the S457 landing discount.
          if(!firstEls[end-1].querySelector(SEL.crb)){
      ...
      // S468 T1 (Mark's call): a card carrying a response box moves WHOLE to a
      // fresh page whenever it fits one — the finding + its response form are
      // one record and must read together; a contractor answering a box whose
      // subject is overleaf is how wrong responses come back. This supersedes
      // the FILL_OK fill-the-page rule for .crb cards only (accepted cost:
      // more page-bottom white space). A card that cannot fit even a fresh
      // page falls through to flowCard, where T2's context stub and T3's
      // bullet seams take over. Non-crb cards keep the S457 behaviour exactly.
      var _crbPair=(blocks[idx].type==='defCard')&&!!firstEls[idx].querySelector(SEL.crb);
      if(!groupOpen&&pageHasBody&&(_crbPair||env.getUsed()>=PAGE_H*FILL_OK)){
        newPage();
        if(ext<=PAGE_H-env.getUsed()){placeBlock(idx);groupOpen=false;idx++;continue;}
      }
      flowCard(idx);

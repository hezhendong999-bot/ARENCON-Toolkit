...
export function createFlowLayout(env){
  ...
  function _cardMeta(el){
    ...
    // S468 T2: bounded context stub — one grey line identifying the item on a
    // continuation page, so the contractor never answers a box whose subject
    // is on another sheet. TRUNCATED by construction, never the full write-up:
    // reprinting a long description re-triggers the very overflow that forced
    // the split, and a full repeat reads as a duplicate deficiency (a real
    // double-response hazard). It is an identifier, not a second copy.
    var _dEl=el.querySelector(SEL.desc);
    var _dTxt=_dEl?String(_dEl.textContent||'').replace(/\s+/g,' ').trim():'';
    if(_dTxt.length>72)_dTxt=_dTxt.slice(0,72).replace(/\s+\S*$/,'')+'\u2026';

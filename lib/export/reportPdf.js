    ...
    try {
      ...
      function _splitGeneric(block, head){
        ...
        kids.forEach(function(k){
          ...
          // Non-splittable (chart/image/short block). If the PAGE already carries
          // other content, force a FRESH PAGE so this block rides one page whole and
          ...
          /* S721 — ROOT CAUSE of a remaining bleed, found by the rebuilt harness:
             the test read `w.children.length > 0` — whether the WRAPPER (a clone
             made empty a moment ago) already held a child. For the FIRST child of
             every block that is always false, so a photo row or chart that did not
             fit the room left was placed on the full page anyway and its excess
             painted past the sheet edge; only the SECOND child earned a fresh page.
             The question was always about the page: does it hold anything besides
             its running header, the section band and this empty wrapper? If so,
             the wrapper goes (it holds nothing), the band travels with it when it
             is the last thing on the page (a heading must sit above what it
             titles), and the block starts a fresh page. A block alone on its page
             stays, as before. */
          var _pgOther = Array.prototype.some.call(curPage.children, function(n){
            return n !== w && !(n.classList && (n.classList.contains('compact-header') || n.classList.contains('sh')));
          });
          if(w.children.length > 0 || _pgOther){
            var _bandTail = null;
            if(!w.children.length && w.parentNode === curPage) curPage.removeChild(w);
            var _lt = curPage.lastElementChild;
            if(_lt && _lt.classList && _lt.classList.contains('sh') && curPage.children.length > 1){ curPage.removeChild(_lt); _bandTail = _lt; }
            _push();
            w = block.cloneNode(false);
            if(_bandTail){ curPage.appendChild(_bandTail); }
            else if(head){ curPage.appendChild(_bandClone(head,true)); }
            curPage.appendChild(w);
          }
          w.appendChild(k);

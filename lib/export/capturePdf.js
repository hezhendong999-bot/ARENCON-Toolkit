...
function _collectTextWords(pageEl){
  ...
  /* S692 — each word learns where its right-hand neighbour STARTS. Collision
     is a relationship between two words, so it cannot be judged from one word
     in isolation; the draw pass squeezes a word only when its own drawn width
     would reach the next anchor on the same line. Same line = tops within half
     a line height; the last word on a line has no neighbour and is untouched. */
  try{
    var _ord=words.slice().sort(function(a,b){ return (a.top-b.top)||(a.l-b.l); });
    for(var _wi=0;_wi<_ord.length-1;_wi++){
      var _a=_ord[_wi], _b=_ord[_wi+1];
      if(Math.abs(_b.top-_a.top)<=_a.h*0.5 && _b.l>_a.l) _a.nx=_b.l;
    }
  }catch(_nx){}
    ...
    try{
      ...
      for(var i=0;i<pages.length;i++){
        ...
        try{
          ...
          /* ═══ S692 — GEOMETRY PASS. THE STRUCTURAL FIX. ════════════════════
             The loop below loads and encodes each photograph, and it AWAITS to
             do so. Measuring inside it meant the first tile and the last were
             measured seconds apart, with the document free to move in between —
             which is why two tiles sharing one grid row came out 4.1 pt apart,
             and why photographs landed below their card on one page and over
             the paragraph text on another. Every previous attempt (page rect
             beside the tile, re-reading at draw time) tried to compensate for
             that gap. The gap itself is the defect.

             So ALL geometry is taken HERE, in one uninterrupted pass: no
             awaits, no image loading, no encoding, nothing that yields to the
             browser between the first tile and the last. The measurements are
             therefore mutually consistent by construction — they describe one
             instant. Positions are stored as fractions of the page, so they
             stay valid however the document is scrolled afterwards.

             RULE FOR FUTURE SESSIONS: nothing that can yield — no await, no
             image decode, no canvas work — may EVER be placed inside this
             pass. If it needs to happen per tile, it happens in the loop
             below, using the geometry already captured here. */
          var _geo=[];
          try{
            var _geoPage=pageEl.getBoundingClientRect();
            var _geoW=_geoPage.width||1, _geoH=_geoPage.height||1;
            for(var _gi=0;_gi<_tiles.length;_gi++){
              var _gr=_tiles[_gi].getBoundingClientRect();
              _geo.push({fx:(_gr.left-_geoPage.left)/_geoW, fy:(_gr.top-_geoPage.top)/_geoH,
                         fw:_gr.width/_geoW, fh:_gr.height/_geoH, w:_gr.width, h:_gr.height});
            }
            /* Tiles side by side in a grid share a top edge. If they do not,
               the page moved during this pass — which must be impossible — and
               the geometry cannot be trusted. Say so; never ship it silently. */
            for(var _gj=1;_gj<_geo.length;_gj++){
              var _pv=_geo[_gj-1], _cu=_geo[_gj];
              if(Math.abs(_cu.fx-_pv.fx)>0.01 && Math.abs(_cu.fy-_pv.fy)>0.0005 &&
                 Math.abs(_cu.fy-_pv.fy)<0.02){
                try{ console.error('[PDF VERIFY] tiles in one row disagree on their top edge by '+
                  Math.round(Math.abs(_cu.fy-_pv.fy)*_geoH)+' px on page '+(i+1)+
                  ' — the page moved during the geometry pass.'); }catch(_g1){}
              }
            }
          }catch(_g0){ _geo=[]; }
          ...
          for(var _ti=0;_ti<_tiles.length;_ti++){
            ...
            /* S692 — geometry comes from the atomic pass above, never from a
               reading taken here: this point is reached only after an await,
               so a reading here describes a different instant to its
               neighbours. _tr is kept solely for the aspect-ratio maths. */
            var _g=_geo[_ti];
            if(!_g||!(_g.w>4)||!(_g.h>4)) continue;
            var _tr={left:0,top:0,width:_g.w,height:_g.h};
            var _tfx=_g.fx;
            var _tfy=_g.fy;
            var _tfw=_g.fw;
            var _tfh=_g.fh;
            ...
            /* ═══ S710 — THE SECOND COPY OF THE 720px CEILING. ════════════════
               S704 made the tier's photoPx the binding rule in
               frt/js/export/pdf.js and left THIS engine untouched. Two engines,
               one sizing rule, one of them fixed — so whichever export actually
               ran decided whether the change existed, and Mark's finished
               report was unchanged because it came through here.

               Same rule as S704: the tier ceiling binds (1400/1800/2200), the
               printed cell is the FLOOR, never upscale the source. These two
               engines must not be allowed to disagree about this again. */
            ...
            _phOverlays.push({rect:_tr,el:_tile,fx:_tfx,fy:_tfy,fw:_tfw,fh:_tfh,img:_jb});   // S692: fx/fy/fw/fh are page fractions
        ...
        if(_phOverlays.length){ try{
          ...
          _phOverlays.forEach(function(o){
            ...
            /* ═══ S692 — REVERTED (draw-time re-measure). ════════════════════════
               Re-reading the tile at draw time was WRONG and measurably worse:
               photographs were placed at 1005-1295 pt on a 792 pt page (off the
               paper entirely), and two tiles sharing one grid row came out 168
               pt apart where the older code had them 4.1 apart. Whatever the
               page element reports at draw time, after the rasteriser has run,
               is not the page's true box. Reverted to the fraction captured
               beside the tile's own page reading during the collection pass.
               Do not reinstate a draw-time reading: it has been tried and it
               put client photographs off the page. */
            ...
            /* S692 — a photograph may never leave the paper. If the geometry
               says otherwise, the geometry is wrong: say so loudly and clamp,
               rather than shipping a report with a photo nobody can see. */
            if(!isFinite(_yT)||_yT<-1||_yT+_hh>ph+1){
              try{ console.error('[PDF VERIFY] photo would fall OUTSIDE page '+(i+1)+
                ' (top '+Math.round(_yT)+' of '+Math.round(ph)+') — clamped onto the page.'); }catch(_c9){}
              _yT=Math.max(0,Math.min(_yT,ph-_hh));
            }
        ...
        if(_txtMode==='vector'&&_txtWords&&_txtWords.length){ try{
          ...
          _txtWords.forEach(function(wd){
            ...
            if(wd.ls>0.05){
              ...
              /* ═══ S692 — EVERY WORD FITS ITS OWN MEASURED SLOT. ════════════
                 "Contractorshall" (7155.34 FRT #2, sent to a client): the
                 browser laid the line out in the machine's own Calibri; the
                 PDF draws Carlito. The faces agree closely but not exactly,
                 so each word starts at its measured anchor and then runs its
                 OWN width — a long word can overrun the following space and
                 weld itself to the next word, or fall short and open a gap.
                 Anchoring alone cannot prevent this: it is a WIDTH problem.

                 The word's measured slot width has been in hand all along
                 (wd.w). The drawn width is now compared against it and the
                 difference is spread across the letter gaps with the PDF's
                 native tracking operator — the same one the letter-spaced
                 labels already use — so every word occupies EXACTLY the box
                 the browser gave it, by construction, on every machine,
                 whatever fonts it has. Beyond ±0.9pt per gap something else
                 is wrong: draw naturally and let the overlap self-check say
                 so, rather than silently squeezing garbage to fit. */
              /* S692 REVISED — fit to the GAP TO THE NEXT WORD, not to the
                 word's own measured box. The first attempt fitted each word to
                 wd.w and "Contractorshall" survived it, which disproved the
                 assumption: Chrome's range rect for a word can INCLUDE the
                 trailing space, so stretching the word to fill that box drives
                 it straight into its neighbour — the fit made the collision
                 certain instead of preventing it.
                 What actually matters is never touching the next word. Each
                 word may only be SQUEEZED, never widened, and only enough to
                 leave a real space before the next anchor on the same line.
                 A word with no neighbour to its right is left alone. */
              var _natPt=0, _gapsN=wd.t.length-1, _fitTc=0;
              try{ _natPt=face.widthOfTextAtSize(wd.t,size); }catch(_nw){ _natPt=0; }
              if(_natPt>0&&_gapsN>0&&wd.nx!=null){
                var _nxPt=(wd.nx-_txtPr.left)*_vsx;
                var _spPt=0; try{ _spPt=face.widthOfTextAtSize(' ',size)*0.55; }catch(_sw){ _spPt=size*0.14; }
                var _avail=_nxPt-x-_spPt;
                if(isFinite(_avail)&&_avail>0&&_natPt>_avail){
                  var _dpg=(_avail-_natPt)/_gapsN;
                  if(_dpg>-0.9)_fitTc=_dpg;
                }
              }
              if(_fitTc&&PDFLib.setCharacterSpacing){
                try{
                  pg.pushOperators(PDFLib.setCharacterSpacing(_fitTc));
                  drawSafe(wd.t,0);
                }finally{
                  try{ pg.pushOperators(PDFLib.setCharacterSpacing(0)); }catch(_t2){}
                }
              }else{
                drawSafe(wd.t,0);
              }
      ...
      // COPY watermark.
      try{
        /* S695b — Model is FRT's global; Diesel and Electric run this same
           shared pipeline without it, so the bare reference threw (caught,
           but a red '[S480 registry] ReferenceError' on every export is a
           false alarm that trains people to ignore the console). In a host
           with no Model there is nothing to register — clean no-op. */
        if(typeof _expId==='string'&&_expId&&typeof Model!=='undefined'&&Model&&Model.registerExport){

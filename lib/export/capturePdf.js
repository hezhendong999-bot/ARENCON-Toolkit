      ...
      for(var i=0;i<pages.length;i++){
        ...
        if(_phOverlays.length){ try{
          ...
          _phOverlays.forEach(function(o){
            ...
            /* ═══ S692 — MEASURE AT THE MOMENT OF USE, AND CHECK. ═══════════
               Proven from the 7155.34 FRT #2 export: two photos sharing one
               grid row came out 4.1 pt apart vertically. Tiles in a row share
               a top edge, always — no layout can separate them. They were
               measured at two different instants and the document moved in
               between, and the same movement put a photograph on top of the
               paragraph text on another page.

               So the geometry is read HERE, beside its own page, in one tick,
               with nothing in between. The fraction taken at capture time is
               kept only to compare against: if the two disagree, the document
               moved during the export, and that is stated out loud instead of
               silently producing a misplaced photograph in a client's report. */
            var _fr=null;
            try{
              if(o.el&&o.el.getBoundingClientRect){
                var _nr=o.el.getBoundingClientRect(), _np=pageEl.getBoundingClientRect();
                if(_nr.width>1&&_np.width>1){
                  _fr={fx:(_nr.left-_np.left)/_np.width, fy:(_nr.top-_np.top)/_np.height,
                       fw:_nr.width/_np.width, fh:_nr.height/_np.height};
                  if(typeof o.fy==='number'&&Math.abs(_fr.fy-o.fy)>0.004){
                    try{ console.error('[PDF VERIFY] photo tile MOVED during export — '+
                      'drift '+Math.round((_fr.fy-o.fy)*ph)+' pt on page '+(i+1)+
                      '; drawing at the position measured now.'); }catch(_m1){}
                  }
                }
              }
            }catch(_m0){ _fr=null; }
            var _hasFr=!!_fr||(typeof o.fw==='number'&&o.fw>0);
            var _uf=_fr||o;
            var _x0=_hasFr? _uf.fx*pw : (o.rect.left-_ppr.left)*_psx;
            var _yT=_hasFr? _uf.fy*ph : (o.rect.top-_ppr.top)*_psy;
            var _ww=_hasFr? _uf.fw*pw : o.rect.width*_psx;
            var _hh=_hasFr? _uf.fh*ph : o.rect.height*_psy;
        ...
        if(_txtMode==='vector'&&_txtWords&&_txtWords.length){ try{
          ...
          _txtWords.forEach(function(wd){
            ...
            if(wd.ls>0.05){
              ...
              var _slotPt=wd.w*_vsx, _natPt=0, _gapsN=wd.t.length-1, _fitTc=0;
              try{ _natPt=face.widthOfTextAtSize(wd.t,size); }catch(_nw){ _natPt=0; }
              if(_natPt>0&&_gapsN>0&&isFinite(_slotPt)&&_slotPt>0){
                var _dpg=(_slotPt-_natPt)/_gapsN;
                if(_dpg>-0.9&&_dpg<0.9&&Math.abs(_slotPt-_natPt)>0.15)_fitTc=_dpg;
              }

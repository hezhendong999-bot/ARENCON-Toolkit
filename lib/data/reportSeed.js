...
(function (global) {
  ...
  function buildSeed(srcData, toolKey, nextNum) {
    ...
    /* ═══ S731 — THE REST OF THE ISSUE LIFECYCLE. ══════════════════════════
       S692 caught dateOfIssue, S700a caught the revision letter. The ledger
       and the print record grew afterwards and were never added here, so a
       brand-new FRT #2 would be born holding #1's issued letters: the
       navigator would draw #1's chips, and the next Issue would compare #2's
       words against #1's digest. Signature ink is the same shape — a new
       draft pre-signed in the previous inspector's hand, on the previous
       date. No live row carries these yet only because the ledger is days
       old and no #2 has been seeded from an issued #1 since; this is the
       fix landing before the first one is.

       tombstones travel. A pin deleted on the last visit must stay deleted
       on this one. Stripping the ledger would let it walk back in from any
       copy that still held the row. Do not tidy this. */
    try{ delete seed.versions; }catch(_cv){}
    try{ delete seed.exportRecords; }catch(_ce){}
    try{ delete seed._intentionalClear; }catch(_cic){}
    try{
      if (seed.signatures) {
        /* S735 — BLANK ONLY KEYS THAT EXIST. Assigning all four created keys
           the source never had; cloud kept map{4}, the device map{6}, and
           every pull reported the two empty extras as "local dirty kept"
           forever (Franz's tablet, 22 Sep). Ink and dates are still cleared
           wherever they were present. */
        ['sigInspectorData', 'sigInspectorDate', 'sigWitnessData', 'sigWitnessDate']
          .forEach(function (k) {
            if (Object.prototype.hasOwnProperty.call(seed.signatures, k)) seed.signatures[k] = '';
          });
      }
    }catch(_cs){}

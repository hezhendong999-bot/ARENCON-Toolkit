...
/* ═══ S731 — A DRAWING BELONGS TO THE PROJECT, NOT TO ONE REPORT. ═══════════
   Report #2 is seeded by deep-cloning report #1, so drawing ids and pdfBufKeys
   are SHARED across the reports of a project by design — 50 pin ids and 101
   photo keys are shared in live data, and drawings the same way. Two doors
   delete drawing binaries: "Delete drawing" and "Replace image (file)", which
   frees the superseded buffer. Both checked only the OPEN report's drawings
   array, so removing a carried sheet on #2 took out the markup #1 still draws.

   This asks the other live reports of the same project before any byte is
   removed. It FAILS OPEN in every uncertain case — no token, no project, a
   failed query, an unparseable body — because an orphaned object costs storage
   and a deleted one costs a site visit. The answer is cached only for the
   length of the gesture (a delete fires markup + buffer checks back to back);
   it is never cached across page loads, where another device may have changed
   what exists. */
var _xrepCache = null, _xrepAt = 0;
function _otherReportBodies(projectId) {
  if (!projectId) return Promise.resolve(null);              // unknown → fail open
  var now = Date.now();
  if (_xrepCache && _xrepCache.pid === projectId && (now - _xrepAt) < 15000) {
    return Promise.resolve(_xrepCache.rows);
  }
  var tok = _getToken();
  if (!tok || !Auth || typeof Auth.request !== 'function') return Promise.resolve(null);
  var path = '/rest/v1/tool_data?select=id,data&project_id=eq.' +
             encodeURIComponent(projectId) + '&tool_key=eq.' + encodeURIComponent(toolKey) +
             '&deleted_at=is.null';
  /* Auth.request, not a bare fetch: it carries the anon key and the bearer
     token and transparently refreshes a stale JWT, so a token that expired
     mid-session cannot turn into a false "nothing else uses this". */
  return Auth.request(path, { method: 'GET' })
    .then(function (rows) {
      if (!Array.isArray(rows)) return null;
      _xrepCache = { pid: projectId, rows: rows }; _xrepAt = Date.now();
      return rows;
    })
    .catch(function () { return null; });                    // network / auth → fail open
}
/* True when some OTHER live report of this project still references the
   drawing id or the pdf buffer key. null/unknown counts as SHARED. */
function _sharedByAnotherReport(projectId, drawingId, pdfBufKey) {
  return _otherReportBodies(projectId).then(function (rows) {
    if (rows === null) {
      console.warn('[R2 S731] could not read the project\'s other reports — keeping the bytes.');
      return true;                                           // fail open
    }
    var myPid = null;
    try { myPid = new URLSearchParams(window.location.search).get('instance'); } catch (_) {}
    for (var i = 0; i < rows.length; i++) {
      var r = rows[i];
      if (!r || !r.data) continue;
      if (myPid && r.id === myPid) continue;                 // the report we are standing in
      var dws = r.data.drawings;
      if (!Array.isArray(dws)) continue;
      for (var j = 0; j < dws.length; j++) {
        var d = dws[j];
        if (!d || d._migratedAwayTo) continue;
        if (drawingId && d.id === drawingId) return true;
        if (pdfBufKey && d.pdfBufKey === pdfBufKey) return true;
      }
    }
    return false;
  }).catch(function () { return true; });                    // any throw → keep the bytes
}
...
var R2 = {
  ...
  delPhotoGuarded: function(r2Key, opts) {
    ...
    /* S729 — THE SHARED-KEY RULE COMES FIRST AND NOTHING OVERRIDES IT.
       copyPhotoToPin / copySitePhotoToPin share r2Key by design, so one image
       can back several live photo records. If any OTHER live record still
       points at this key, deleting the bytes breaks that record. The S481
       branch here read "another live ref → allowed" — the exact inverse of
       what the function exists for; it was latent only because the one caller
       that could reach it has since stopped deleting bytes at trash time.
       `liveRefs` is the count of OTHER live (non-deleted) records on this key.
       `refCount` is accepted as the old name for the same number so no legacy
       caller is misread — and is now a reason to KEEP, never to delete.
       `force` means "I know this is the last copy and I want it gone"; it does
       not mean "the user clicked trash", and it cannot outrank a live sibling. */
    var _live = (typeof opts.liveRefs === 'number') ? opts.liveRefs
              : (typeof opts.refCount === 'number') ? opts.refCount : null;
    if (_live !== null && _live > 0) {
      console.warn('[R2Guard] BLOCKED photo delete — ' + _live + ' other live record(s) still share this key. Bytes kept:', r2Key,
        '(photoId=' + (opts.photoId || 'n/a') + (opts.force ? ', force ignored' : '') + ')');
      return Promise.resolve(false);
    }
    if (opts.force === true) {
      console.log('[R2Guard] forced photo delete (deliberate, last copy):', r2Key);
      return R2.del(r2Key);
    }
    ...
    return checkLocal.then(function(haveLocal){
      ...
      console.error('[R2Guard] BLOCKED orphan photo delete — no force, no local bytes. Key kept:', r2Key,
  ...
  deleteDrawingAssets: function(projectId, drawingBeingRemoved, allDrawings) {
    ...
    // S126 Phase B — Markup binary is per-drawing. S731: per-drawing is not
    // per-report — deleteMarkup now asks the project's other live reports
    // before removing anything, and keeps the bytes when unsure.
    ...
    /* S731 — exclusive IN THIS REPORT is not exclusive in the project. The
       same buffer key rides into every report seeded from this one, and
       "Replace image (file)" reaches this path too. Ask the other live
       reports; on any uncertainty keep the bytes. */
    return _sharedByAnotherReport(projectId, drawingBeingRemoved.id, key).then(function (xrep) {
      if (xrep) {
        console.log('[R2 S731] PDF buffer ' + key + ' is still used by another report of this project; not deleting.');
        return { pdfBufDeleted: false, sharedSkipped: true, crossReport: true };
      }
  ...
  deleteMarkup: function(projectId, drawingId) {
    /* S731 — markup is per-drawing, but a DRAWING is shared across the reports
       of a project (seed clones drawing ids). "Per-drawing" was read as "cannot
       be shared", and it deleted the vectors report #1 still draws. Ask first;
       keep the bytes whenever the answer is not a clear no. */
    return _sharedByAnotherReport(projectId, drawingId, null).then(function (shared) {
      if (shared) {
        console.log('[R2 S731] markup for drawing ' + drawingId + ' is still used by another report of this project; not deleting.');
        return false;
      }
      return R2._deleteMarkupUnchecked(projectId, drawingId);
    });
  },

  /* The raw delete, kept separate so the sharing check above is the only way
     in from the app. Do not call this from UI code. */
  _deleteMarkupUnchecked: function(projectId, drawingId) {

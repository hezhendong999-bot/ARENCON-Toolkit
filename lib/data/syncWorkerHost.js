/* ARENCON /lib/ — Sync Worker Host (S446 extraction)
   From FRT frt/js/data/syncWorkerHost.js. ONE change vs FRT: the worker is
   located via new URL('./syncWorker.js', import.meta.url) so it resolves from
   any tool/page, not the FRT-relative 'js/data/syncWorker.js' string. The
   inline-fallback architecture is unchanged — if the Worker can't boot,
   serialize/merge/parse run on the main thread with an identical API. */
/**
...
function _bootWorker() {
  ...
  try {
    ...
    // (lib) self-locating: resolve the worker relative to THIS module's URL,
    // so it works from any tool/page regardless of page depth. Replaces the
    // FRT page-relative string 'js/data/syncWorker.js' (S446 extraction).
    _worker = new Worker(new URL('./syncWorker.js', import.meta.url), { type: 'module' });
    _diag.workerOK = true;

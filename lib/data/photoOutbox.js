/**
 * ARENCON /lib/ - Binary (photo) Outbox (parameterized, S446)
 * Extracted VERBATIM from FRT frt/js/data/photoOutbox.js (the audit winner).
 * All behaviour preserved - S172 retry policy + backoff, S176 abort plumbing,
 * S199/S201a post-PUT Range-GET verify, S171 cloud-push reconciliation,
 * S414 R2 dead-key self-heal (Range GET, never HEAD), S200a kind-tagging.
 *
 * ONE API change from the FRT original (Plan v2 amendment): the FRT `Model`
 * import is replaced by an INJECTED adapter. Static analysis of the source
 * shows the outbox touches exactly TWO model methods:
 *     model.getProject()   -> returns the live project object (may be null)
 *     model.saveNow?()     -> optional; flush persist (already null-guarded)
 * The photo-add methods (addObservationPhoto / addPoolPhoto) referenced in
 * comments are the CALLER's job, not the outbox's - they are not called here.
 * This 2-method contract is a strict subset of the sync adapter that Step 3
 * defines, so the outbox slots into the canonical model adapter with no rework.
 *
 * All other deps are injected too, so each tool supplies its own instances:
 *   import { createBinaryOutbox } from '../lib/data/photoOutbox.js';
 *   export const BinaryOutbox = createBinaryOutbox({
 *     IDB: ELECTRIC_IDB, R2: ELECTRIC_R2, Auth, toast,
 *     model: { getProject: () => electricState.project, saveNow: () => electricState.persist() }
 *   });
 *
 * The photoOutbox IDB store ('photoOutbox') and photoBlobs store must exist in
 * the tool's createIDB({ stores }) list, same as FRT.
 */

export function createBinaryOutbox(config) {
  config = config || {};
  var IDB   = config.IDB;
  var R2    = config.R2;
  ...
  if (!IDB || !R2 || !model) {
    throw new Error('[lib/photoOutbox] createBinaryOutbox requires { IDB, R2, Auth?, toast?, model:{getProject, saveNow?} }');
  }
  if (typeof model.getProject !== 'function') {
    throw new Error('[lib/photoOutbox] model.getProject() is required');
  }
  // Optional deps degrade safely: toast() no-ops if absent; Auth falls back to
  // the "local" userId path (same as FRT standalone/pre-auth boot).
  if (typeof toast !== 'function') { toast = function(){}; }

  // ═══════════════════════════════════════════════════════════════════════
  // S534 — TOOL-NEUTRALITY. This engine lived in /lib/ but was still FRT's:
  // it walked FRT's report shape, asked for FRT's field names, and read FRT's
  // local blob store. That is why Diesel could not use it and passed
  // BinaryOutbox:null — opting out of upload verification, dead-key repair and
  // the sourceless rescue stage entirely. Three injection points below turn
  // those assumptions into per-tool configuration.
  //
  // EVERY default is EXACTLY the previous hard-coded FRT behaviour, so a host
  // that passes nothing behaves byte-for-byte as before. That is deliberate:
  // this file runs in the field on FRT, and the migration must not be able to
  // change it. Verify by diffing the defaults against the S533 code, not by
  // reading this comment.
  //
  //   photoWalk(proj)   -> array of every photo record in the report
  //   photoFields       -> { bytes, thumb } — names of the inline-image fields
  //   localBytes(id)    -> Promise<Blob|null> — this device's original bytes
  // ═══════════════════════════════════════════════════════════════════════
  var _photoWalk = (typeof config.photoWalk === 'function') ? config.photoWalk : function (proj) {
    var out = [];
    if (!proj) return out;
    (proj.photos || []).forEach(function (ph) { out.push(ph); });
    (proj.contractors || []).forEach(function (c) {
      (c.deficiencies || []).forEach(function (d) {
        (d.photos || []).forEach(function (ph) { out.push(ph); });
      });
    });
    return out;
  };
  var _phFields = config.photoFields || { bytes: 'dataUrl', thumb: 'thumb' };
  var _F_BYTES = _phFields.bytes || 'dataUrl';
  var _F_THUMB = _phFields.thumb || 'thumb';
  var _localBytes = (typeof config.localBytes === 'function') ? config.localBytes : function (id) {
    return IDB.get('photoBlobs', id).then(function (rec) { return (rec && rec.dataBlob) || null; });
  };
  // S544 — FOURTH injection point: which project these photos belong to.
  // FRT's report object carries its own id, so the rescue pass could read
  // proj.id directly. Diesel's cloud payload is a flat field bag with NO
  // top-level id (verified against the live collect, not assumed) — so on
  // Diesel verifyR2Keys returned null on its first line and did nothing at
  // all. Registering the tool without this would be coverage the code does
  // not deliver, which is the S532/S540 failure mode. Default is EXACTLY the
  // previous hard-coded behaviour, so FRT is unchanged.
  var _projectId = (typeof config.projectId === 'function') ? config.projectId : function (proj) { return proj && proj.id; };
...
function _r2HealOne(projectId, ph, res, proj) {
  ...
  return _localBytes(ph.id).then(function(blob){
    if (blob) {
    ...
    // S481 DATA-LOSS FIX (Mark, 1490.04 Obs 4A) — ported to /lib/ S491; the
    // S446 extraction predates it, so every lib consumer (Electric live) was
    // nulling keys without ever probing for the reachable backup copy.
    // Before nulling, probe the PREDICTABLE markup-backup address. Markup-save
    // parks a clean original at photos/{slug}/{toolKey}/original/orig_{photoId}.jpg.
    // If a backup exists there, this record can be REPOINTED (repair) instead
    // of nulled (loss). Probe the record's own key-derived slug AND the current
    // projectId slug (cross-generation backups may sit under either). 1-byte
    // Range GET only; repoint solely on a real hit. Never delete anything here.
    var _tk = (R2 && R2.TOOL_KEY) ? R2.TOOL_KEY : 'frt';
    ...
    // address — no slug guessing.
    if (proj && ph._origBackupId) {
      var _all = _photoWalk(proj);
...
function _processRow(row) {
  ...
  /* S735 — RELEASE THE UPLOAD SLOT EXACTLY ONCE PER ATTEMPT. The slot was
     released in .then after the PUT and again in .catch; a verify failure
     after a good PUT ran both, so the counter under-read by one and allowed
     an extra parallel PUT under flaky R2. Timing is unchanged: the slot is
     still freed when the PUT returns, before the verify. */
  var _slotReleased = false;
...
function _findPhotoInProject(proj, photoId) {
  ...
  // the canonical home.
  /* S729 — contractor-thread photos live under each observation's
     responses[].rectPhotos[] and arenconReviews[].followupPhotos[], not in
     the pin pool. They were never on the outbox because this walker could
     not find them to patch r2Key on confirm. Now it can, so the CRB thread
     upload path can enqueue like every other photo and a dropped connection
     retries instead of losing the photograph. */
  var found = null;
  function _walkList(L) {
    if (found || !Array.isArray(L)) return;
    for (var i = 0; i < L.length; i++) {
      if (L[i] && L[i].id === photoId) { found = L[i]; return; }
    }
  }
  function _walkDefic(d) {
    ...
    for (var o = 0; o < obs.length && !found; o++) {
      var ob = obs[o]; if (!ob) continue;
      var rs = ob.responses || [];
      for (var r = 0; r < rs.length && !found; r++) { if (rs[r]) _walkList(rs[r].rectPhotos); }
      var rv = ob.arenconReviews || [];
      for (var v = 0; v < rv.length && !found; v++) { if (rv[v]) _walkList(rv[v].followupPhotos); }
    }
  }
  (proj.contractors || []).forEach(function(c) {
...
var BinaryOutbox = {
  ...
  verifyR2Keys: function(proj) {
    ...
    var _pid = _projectId(proj);
    if (!proj || !_pid) return Promise.resolve(null);
    if (_r2VerifiedProjects[_pid]) return Promise.resolve(null);
    _r2VerifiedProjects[_pid] = true;   // once per session per project
    var photos = _photoWalk(proj);
    ...
    targets.forEach(function(ph){
      chain = chain.then(function(){
        ...
        return fetch(ph.r2Url, { method: 'GET', headers: { 'Range': 'bytes=0-0' } }).then(function(r){
          ...
          return _r2HealOne(_pid, ph, res, proj);
        }).catch(function(){ /* network blip — never heal on failure to reach */ });
      });
    });
    // ── S462 durability rescue stage (ported to /lib/ S491) ──
    ...
    var ghosts = photos.filter(function(ph){
      return ph && !ph.deleted && !ph.r2Key && !ph[_F_BYTES] && !ph[_F_THUMB];
    });
    ...
    ghosts.forEach(function(ph){
      rchain = rchain.then(function(){
        ...
        return _r2HealOne(_pid, ph, res, proj).then(function(){
    ...
    return rchain.then(function(){
      if (res.checked || res.sourceless) console.log('[R2Heal] ' + String(_pid).slice(0,8) + '\u2026 checked:' + res.checked +
...
// (window.BinaryOutbox global exposure is a per-tool opt-in in /lib/ -
// omitted here so two tools on a page can't clobber the same global.)
  return { BinaryOutbox: BinaryOutbox, OUTBOX_STATUS: OUTBOX_STATUS };
}

/* ARENCON /lib/ — 3-Way Merge Engine (S446 extraction)
   VERBATIM from FRT frt/js/data/merge.js at the current HEAD. Pure functions,
   zero imports. Carries the S43x deletion-wins tombstone logic (_tombstoneWins /
   _isDeletedItem) that fixes photo resurrection, and the 14-test / 30-assert
   self-hook on window._frt_mergeDiag. The engine touches NO model; only the
   .simulate() console helper reads an OPTIONAL injected model. */
/**
...
var mergeDiagModel = null;  // (lib) optional; console-helper use only
 ...
 */
/* S583 (Mark, 1490.04 forensics): 'pct' added. Diesel/Electric flow-test rows
 * (stdData / pldData) are identified by their flow percentage — they carry no
 * 'id'. Without an identity, merge3 saw the whole flow table as one anonymous
 * lump: any two-sided edit conflicted as "stdData — (3 items)" with no way to
 * name the reading. With 'pct' as identity, conflicts resolve PER READING and
 * surface as e.g. stdData[100%].discharge. 'pct' is used as a data field only
 * by these flow rows anywhere in the toolkit (verified against frt/, lib/,
 * diesel-app/ at HEAD 05d5e226). */
var ID_FIELDS = ['id', '_id', 'pct'];
...
/* S625 — own-property check that survives data shadowing the method. Item ids
   and map keys come from data (imported JSON, user-typed strings); an id or
   key of literally "hasOwnProperty" turns obj.hasOwnProperty into a plain
   value and calling it throws. Always borrow the prototype's method. */
...
function _indexById(arr) {
  var byId = Object.create(null);   /* S625: no prototype — item ids come from
     data, and an id like "constructor" on a {} either shadowed a method (crash)
     or read truthy off the prototype (silent skip). A null-prototype object has
 ...
 */
// S481 PHOTO POINTER-PROTECTION (Mark: permanent fix). Ported to /lib/ S491 —
// the S446 extraction predates S481, so the lib copy merged WITHOUT this guard
// and any consumer on it (Electric live) could have a good photo pointer wiped
// by a bytes-less device on every sync. "Cloud owns structure, local owns
// binary" applied at the pointer level: given a merged photo item and the two
// source versions, a GOOD (non-null string) r2Key must never be left wiped
// (null) when either source had one. Returns the (possibly repaired) value.
// Scoped to non-deleted photo items; all other values pass through untouched.
// Idempotent and deterministic — safe to apply on every merge path.
...
/* ═══ S616 — EMPTINESS IS NEVER AN EDIT, AT THE RECONCILIATION DOOR ════════
   Doctrine I-2 ("a blank never beats content") lived only in the
   background-refresh merge, _mergeLWW in sync.js. The OTHER door — two people
   saving the same report within the same moment, where the second save is
   rejected and the two versions are reconciled HERE — had no such rule.

   A blank carries no entry stamp, so the S590 latest-entry tiebreak above
   could not settle it, and every unsettled field is handed to the CLOUD by
   the tools' auto-resolver. So a device holding a blank skeleton erased
   another inspector's typed content on any near-simultaneous save. This is
   the FIFTH copy of the "emptiness counts as an edit" fault (array items
   S600, nested children S605, statusMaps and fieldMaps S610) — the lane law
   said to assume one existed, and it did.

   The rule is the same one the pull side already runs: a blank never beats
   content. It wins only when it carries a genuinely NEWER entry stamp, which
   the S590 tiebreak handles before this ever runs — so a real, deliberate
   Clear still propagates untouched (permanently guarded by the
   'A2 stamped-clear' check in tools/sim/converge.mjs).

   BLANK IS DELIBERATELY NARROW. Lane A's S613 lesson: widening what counts as
   blank risks discarding an inspector's in-progress row, which is worse than
   the bug being fixed. A numeric 0 and a boolean false are CONTENT — a gauge
   reading of 0 psi and a toggle answered No are things a person entered. */
function _s616IsBlank(v) {
...
/* Returns the side that must win when exactly one side is blank, else null. */
function _s616BlankLoses(mv, tv) {
  var mb = _s616IsBlank(mv), tb = _s616IsBlank(tv);
  if (mb && !tb) return 'theirs';
  if (tb && !mb) return 'mine';
  return null;
}
...
function _merge3Object(base, mine, theirs, pathStr) {
  ...
  /* S590 (Mark's ruling, final): LATEST ENTRY WINS — AUTOMATICALLY. The
     entry-time stamps (_ts, set once at the save that first contains a
     change, S583) exist precisely to answer "which of these two edits is
     newer" without asking a person. When both sides changed the same field
     inside a stamped item, the side whose ENTRY is newer takes the field.
     Only a genuinely simultaneous edit (identical or missing stamps) falls
     through to the conflict list — which the tools auto-resolve too; no
     modal exists anywhere anymore. */
  var _tb = null;
  try {
    var _mtsO = (mine && typeof mine._ts === 'number') ? mine._ts : 0;
    var _ttsO = (theirs && typeof theirs._ts === 'number') ? theirs._ts : 0;
    if (_mtsO && _ttsO && _mtsO !== _ttsO) _tb = (_mtsO > _ttsO) ? 'mine' : 'theirs';
  } catch (_) {}
  ...
  Object.keys(allKeys).forEach(function(key) {
    /* S625 (battle test T21) — OWN properties only. `mine[key]` on a side that
       lacks the key falls through to the PROTOTYPE: a report containing the
       key "constructor" (statusMaps and similar maps are keyed by user-typed
       strings, so one person typing that word is enough) made the other side
       return Function, _clone() ran JSON.stringify(Function) → undefined →
       JSON.parse threw, and the ENTIRE merge3 crashed — every 412
       reconciliation on that report dead until the key was hand-removed.
       hasOwnProperty gives undefined for a genuinely absent key, which is
       exactly what the absent-key paths below already expect. */
    var bv = _hasOwn(base, key)   ? base[key]   : undefined;
    var mv = _hasOwn(mine, key)   ? mine[key]   : undefined;
    var tv = _hasOwn(theirs, key) ? theirs[key] : undefined;
    var subPath = pathStr ? pathStr + '.' + key : key;

    // ── S583: per-item entry-time stamps (_ts) and field-stamp maps (_fts)
    // are BOOKKEEPING, never a question for a person. The 1490.04 conflict
    // modal was dominated by "CLSTATE → S1_0 → _TS: keep 1785710020199 or
    // 1785710000345?" — unanswerable. Resolve deterministically: the newer
    // stamp wins (it is by definition the more recent entry time). Never
    // emit a conflict for either key.
    if (key === '_ts') {
      var _mts = (typeof mv === 'number') ? mv : 0;
      var _tts = (typeof tv === 'number') ? tv : 0;
      if (mv !== undefined || tv !== undefined) result[key] = Math.max(_mts, _tts);
      return;
    }
    if (key === '_fts') {
      if (mv !== undefined || tv !== undefined) {
        var _mf = (mv && typeof mv === 'object') ? mv : {};
        var _tf = (tv && typeof tv === 'object') ? tv : {};
        var _out = {};
        Object.keys(_mf).forEach(function(fk) {
          var mo = _mf[fk] || {}, to = _tf[fk] || {}, o = {};
          Object.keys(mo).forEach(function(f) { o[f] = mo[f]; });
          Object.keys(to).forEach(function(f) {
            var a = (typeof o[f] === 'number') ? o[f] : 0;
            var b = (typeof to[f] === 'number') ? to[f] : 0;
            o[f] = Math.max(a, b);
          });
          _out[fk] = o;
        });
        Object.keys(_tf).forEach(function(fk) { if (!_out[fk]) _out[fk] = _tf[fk]; });
        result[key] = _out;
      }
      return;
    }
    ...
    /* S625 — same fault family as the own-property reads above: data whose OWN
       key is literally "hasOwnProperty" shadows the method, and calling it
       crashes. _hasOwn borrows the prototype's method, always. */
    var mineHas = _hasOwn(mine, key);
    var theirsHas = _hasOwn(theirs, key);
    var baseHas = _hasOwn(base, key);
    ...
    if (!theirsHas && mineHas && !_deepEq(mv, bv)) {
      /* S616 — DELETION IS AN EVENT, NOT AN ABSENCE. A cloud copy that simply
         does not carry this key has not deleted anything; a real deletion
         writes tombstones. This was emitted as a conflict, and the tools'
         auto-resolver then chose the absent side — deleting a value this
         device had just typed. Content with no stamp evidence against it
         stays. Real deletions are unaffected: they arrive as tombstone
         fields, which are content and merge normally. */
      if (!_s616IsBlank(mv)) { result[key] = _clone(mv); return; }
    ...
    /* S590 — a contested field inside a stamped item resolves to the side
       whose ENTRY is newer. Latest entry wins, silently (Mark's ruling). */
    if (sub.conflicts.length && _tb) {
      result[key] = _clone(_tb === 'mine' ? mv : tv);
    } else if (sub.conflicts.length && _s616BlankLoses(mv, tv)) {
      /* S616 — no stamp could settle this, and one side is blank. Content
         wins, silently: this is not a question worth asking and, left as a
         conflict, the auto-resolver would hand it to the cloud. */
      result[key] = _clone(_s616BlankLoses(mv, tv) === 'mine' ? mv : tv);
    } else {
...
function _merge3IdArray(base, mine, theirs, pathStr) {
  ...
  var conflicts = [];
  var result = [];
  var seenIds = Object.create(null);  /* S625: plain {} made seenIds["constructor"]
     truthy BEFORE anything was seen — items with prototype-member ids were
     silently DROPPED from the merge. Null prototype: nothing inherited, a key
     is truthy only if this merge actually set it. */
  ...
  if (Array.isArray(base)) {
    base.forEach(function(baseItem) {
      ...
      var inMine = _hasOwn(mineIdx.byId, bid);
      var inTheirs = _hasOwn(theirsIdx.byId, bid);
 ...
 *   _frt_mergeDiag.simulate()        // takes current (mergeDiagModel && mergeDiagModel.getProject ? mergeDiagModel.getProject() : (typeof Model!=='undefined' && Model.getProject ? Model.getProject() : null)),
 *                                       creates a fake conflict, returns the result
 */
if (typeof window !== 'undefined') {
  // (lib) optional model for the .simulate() console helper only — the merge
  // ENGINE is pure and never touches a model. Set via _frt_mergeDiag.useModel(m).
  window._frt_mergeDiag = {
    useModel: function(m){ mergeDiagModel = m; },
    merge3: merge3,
     ...
     * (mergeDiagModel && mergeDiagModel.getProject ? mergeDiagModel.getProject() : (typeof Model!=='undefined' && Model.getProject ? Model.getProject() : null)) state, fabricates a "their" state that differs,
    ...
    simulate: function() {
      ...
      var current = (mergeDiagModel && mergeDiagModel.getProject ? mergeDiagModel.getProject() : (typeof Model!=='undefined' && Model.getProject ? Model.getProject() : null));
      if (!current) {

...
function _computePartialPayload(mine, base) {
  try {
    ...
    for (var j = 0; j < mk.length; j++) {
      ...
      var b = (k in base) ? JSON.stringify(base[k]) : undefined;
      if (a !== b) { changed[k] = mine[k]; keys.push(k); sentLen += a.length + k.length + 4; }
    }
...
var SyncEngine = {
  ...
  push: function(projectId, _attempt) {
    ...
    return SyncWorkerHost.serializePush(proj).then(function(serialized) {
      ...
      if (method === 'PATCH' && _partialSaveCfg && _instanceId &&
      ...
      return _partialAttempt.then(function(_pRows) {
        if (_pRows) return _pRows;   // confirmed scoped save — skip the full push

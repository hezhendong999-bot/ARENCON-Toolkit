...
var _svrOffsets = [];        // recent server-minus-device samples (ms)
var _svrOffset = 0;          // smoothed current offset
...
function _learnServerTime(res) {
  try {
    ...
    _svrOffset = (med > 1500 || med < -1500) ? med : 0;
    try { if (typeof window !== 'undefined') window.__arcSvrOffset = _svrOffset; } catch (_) {}   // diagnosable in the field too
    try { localStorage.setItem('arc-clock-offset', String(_svrOffset)); } catch (_) {}
  } catch (_) {}
}
function _nowSync() {
  var t = Date.now() + _svrOffset;
  if (t <= _lastMint) t = _lastMint + 1;   // monotonic on this device
  _lastMint = t;
  return t;
}
...
function _rawFetch(path, opts, _isRetry) {
  ...
  return fetch(_url() + path, {
    ...
    _learnServerTime(res);   // S622i: every round trip refines the shared clock

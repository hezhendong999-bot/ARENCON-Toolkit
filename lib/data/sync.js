...
/* S608 — HARNESS-ONLY EXPORT (frt/tests/sim/stalemate.mjs). Builds a throwaway
   engine with stub dependencies purely to reach the closure-internal per-item
   merge. The stub model never receives data; the replay path performs no I/O.
   The harness shims window/navigator/localStorage before calling this. */
...
var _LWW_SPECS = {
  ...
  frt: {
    arrays: {
      /* S608 — THE S605 TREATMENT, applied to FRT's photos (Lane A). FRT photo
         records carry local-only binary the cloud strips by design (dataUrl,
         thumb), so whole-object dirtiness read every photo as "locally edited"
         on every device on every pull — local won forever, and a caption edit,
         rotation or delete made on another tablet could NEVER land here (the
         exact disease Mark field-caught on Diesel, 03 Aug). Dirtiness is now
         ...
         and _lwwStripFields serializes non-scalar fields canonically so stroke
         edits are content-compared, not String()-flattened.
         Harness: frt/tests/sim/stalemate.mjs (fails on S587, passes here). */
      ...
      // S608 SCOPE HOLD — deficiency and observation entries stay UNtyped on
      // purpose: Lane C's deficiency-propagation investigation is open, and
      // changing how FRT judges deficiency dirtiness mid-investigation would
      // contaminate their forensics. Photo children are orthogonal (nested
      // lists are dropped from the parent's own signature and merge in their
      // own right). Type these via frt/tests/sim/deficsync.mjs once Lane C
      // closes — verify that against live HEAD, not a handoff's claim.
      contractors:         { key: 'id', nested: { deficiencies: { key: 'id', nested: { observations: { key: 'id' }, photos: { key: 'id', fields: ['caption','rotation','deleted','delAt','delState','deletedBy','_annotated','_markupStrokes','_mkFrame','_origBackupId'] } } } } },
      generalDeficiencies: { key: 'id', nested: { observations: { key: 'id' }, photos: { key: 'id', fields: ['caption','rotation','deleted','delAt','delState','deletedBy','_annotated','_markupStrokes','_mkFrame','_origBackupId'] } } },   // S541; S608 photo fields
...
function _lwwStripFields(o, fields) {
  ...
  for (var i = 0; i < fields.length; i++) {
    var f = fields[i];
    /* S608 — non-scalar typed fields are serialized canonically. String() on
       an array of objects yields "[object Object],[object Object]": only the
       COUNT survives, so a mark MOVED on this device (same stroke count, new
       ...
       fully content-sensitive, deterministic on both sides of the compare.
...
var SyncEngine = {
    ...
    return _restoreSyncMeta(projectId, instanceId).then(function() {
      ...
      if (data) {
        ...
        try {
          ...
          try { SyncEngine.lastPullKeptLocal = !!(_lwwMergeStats && (_lwwMergeStats.keptLocalDirty > 0 || _lwwMergeStats.keptLocalAbsent > 0)); } catch (_) {}   // S604→S605
          /* S599 — report what the merge decided, before the gate can swallow it. */
          try {
            if (_onDiag) {
              var _pick = function (st) {
                try {
                  var r = (st && st.stdData || []).filter(function (x) { return x && x.pct === '100%'; })[0];
                  return r ? { v: r.discharge, ts: r._ts } : null;
                } catch (_) { return null; }
              };
              var _cloudRaw = _pick(_rawCloudForDiag), _merged = _pick(data), _screen = _pick(_lpg);
              if (_cloudRaw && _screen && String(_cloudRaw.v) !== String(_screen.v)) {
                _onDiag('pull_decision', {
                  cloudRaw: _cloudRaw, afterMerge: _merged, screen: _screen,
                  mergeGaveCloud: !!(_merged && _cloudRaw && String(_merged.v) === String(_cloudRaw.v)),
  ...
  /* S608 — HARNESS HOOK (frt/tests/sim/stalemate.mjs). Replays the per-item
     LWW merge against the SHIPPED code with no I/O: no network, no IDB, no
     model mutation. Inputs are deep-cloned so a harness can never leak state
     into a live engine. Not for production use. */
  _lwwReplay: function(field, localArr, cloudArr, snapArr) {
    var localProj = {}; localProj[field] = JSON.parse(JSON.stringify(localArr || []));
    var cloudData = {}; cloudData[field] = JSON.parse(JSON.stringify(cloudArr || []));
    var snap = {};      snap[field]      = JSON.parse(JSON.stringify(snapArr  || []));

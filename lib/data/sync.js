...
var _LWW_STRIP_DROP = { _ts: 1, _fts: 1 };
...
function _lwwStampOf(item, prevItem, unchanged, now) {
  if (prevItem) return unchanged ? (prevItem._ts || item._ts || 0) : now;
  // No previous state for this item — never fabricate recency.
  return (typeof item._ts === 'number' && item._ts) ? item._ts : now;
}

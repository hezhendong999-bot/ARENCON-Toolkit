...
function _mergeLWW(localProj, cloudData, localSnap) {
  ...
  try {
    ...
    (spec.scalars || []).forEach(function (field) {
      ...
      if (!cHas) { cloudData[field] = lv; return; }
      ...
      if (lBlank && !cBlank) { cloudData[field] = (lts > cts) ? lv : cv; return; }
      if (cBlank && !lBlank) { cloudData[field] = (cts > lts) ? cv : lv; return; }
      cloudData[field] = (cts > lts) ? cv : lv;
    });
...
var SyncEngine = {
  ...
  /* ═══ S617 — STAMP AT EDIT TIME, WITH OR WITHOUT A NETWORK ═══════════════
     ...
     an un-flushed edit re-stamps at next serialize; persisting the ledger
     is queued work, not quietly claimed here. */
  stampLocal: function () {

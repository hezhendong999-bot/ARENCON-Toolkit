...
/* S643b — the host declares that getProject() is a LIVE READ of the screen
   (a fresh collect), not a reference to an in-memory model. Only such a host
   can have a screen that silently disagrees with what the engine applied, and
   only such a host may be ledger-anchored. Absent = off: a tool must opt in
   deliberately, because getting this wrong the other way puts the whole model,
   photo binaries and all, into a ledger that is persisted to disk. */
var _anchorHostReadsScreen = !!(config.model && config.model.getProjectReadsScreen === true);
var _pendingSync = false;
/* S693 — ISSUED-REPORT LOCK. True after the server has answered PT423 for
   this report: it is issued and closed to edits. A permanent refusal is a
   STATE, not bad luck — the engine never retries one (before this, a locked
   report was retried every 15 s forever, filling the console, burning tablet
   battery and burying real errors). Cleared at the pull door the moment the
   cloud row's status is no longer 'issued' (the deliberate unlock). */
var _issuedLocked = false;
/* ═══ S699 — ONE OWNER FOR RECONNECT. ═══════════════════════════════════════
   Before this, TWO things fired on the browser's `online` event: this engine's
   listener called flush() (which is push-only), and the Diesel/Electric facades
   ran their own pull-then-save. They raced.

   The fix is NOT to fold a pull into flush(). `pull()` applies through
   model.setProject, but the HOST owns the repaint — FRT repaints via
   _repaintAfterPull, and Diesel/Electric collect their state from the SCREEN
   (getProjectReadsScreen). A pull inside flush would change the model with no
   repaint, and the push immediately after would send pre-pull screen state.
   That is worse than the race.

   So the ORDER lives here, once, and the host supplies the two steps only it
   can do. `afterPull` is MANDATORY: a host that cannot repaint has no business
   pushing after a pull. A host that registers nothing keeps today's behaviour
   exactly — no default pull+push is ever invented for it. */
var _reconnectAfterPull = null;   // host repaint, required
var _reconnectPushAhead = null;   // host's own "am I ahead" push, optional
...
/* ═══ S692 — THE AUTHORITY MARK. ═══════════════════════════════════════════
   Every safety net in this engine exists to stop a report SHRINKING: the
   empty-array guard, the shrinkage guard, the content-collapse guards, the
   dirty-key retention in the LWW merge. They are all correct, and they all
   answer the same question — "did the cloud lose something my device still
   holds?" — with "then keep mine".

   That question has no right answer when the cloud copy is shorter ON PURPOSE.
   An authorised correction (an admin restore, a repair of a contaminated
   report) is byte-for-byte indistinguishable from the corruption these nets
   were built to stop, so on 25 Aug 2026 three correct repairs to 7155.34 were
   each put back by a cached device within minutes. The nets were right; there
   was simply no way to TELL them.

   `authority_rev` is that way. A counter the SERVER owns, on each row, which
   only ever goes up and which no client may write (a server trigger refuses
   it). When the cloud's mark is higher than the one this device last accepted,
   the cloud copy is not merged and not guarded — it is taken WHOLE, after this
   device's own copy is set aside in IDB and the person is told.

   Default-inert by construction: the column defaults to 0 and `_authSeen`
   defaults to 0, so `cloud > seen` is false everywhere until somebody
   deliberately raises the mark. No tool changes behaviour until then. */
var _authSeen = 0;
...
var _GUARDED_ARRAY_FIELDS = [
  ...
  /* S728 — the version ledger. Added after 9000.01 lost an issue: the report
     was issued as B05 (status flipped to issued, B05 appended to the ledger),
     then a pull arrived from another session carrying a copy taken BEFORE the
     issue. Unguarded, it replaced the ledger with its shorter one and the B05
     entry was gone. `revision` and `dateOfIssue` survived because they are
     plain fields, leaving a report reading B05 with an issue date, a draft
     status, and no record of ever being issued — looking locked and behaving
     unlocked. The ledger is the only record of who issued what and when, so a
     cloud copy that is a strict subset of the local one is never right. Every
     entry carries an id (seedLedger mints a deterministic one), so the subset
     test below applies to it exactly as it does to drawings and photos. */
  'versions',
  /* S730 — the delete ledger. A cloud copy taken before a delete is shorter;
     letting it win would resurrect the deleted item on the next sweep-less
     device. Same reasoning as versions. */
  'tombstones',
  /* S731 — a print record must not be lost to a same-length array swap. */
  'exportRecords',
  // Diesel / Electric commissioning
  'recordPhotos', 'pldData', 'stdData', 'batData', 'contractorSignRows',
  'pumpCurvePoints', 'pldPumpCurvePoints',
  /* S732 (Lane C) — three pump arrays that were replaced wholesale by a pull
     but never guarded: the WITNESS sign-off rows (an AHJ or owner signature —
     contractorSignRows was here, its twin was not), and the two flow-test
     photo lists. All three carry ids the subset test can compare: sign rows
     are minted at creation (S611), photos through _photoOut. sketchEntries
     (uid, not id) and distribution (plain strings) are NOT added — the guard
     could not judge them and listing them would only look like protection. */
  'witnessSignRows', 'flowTestPhotos', 'flowTestPhotosPld'
];
...
//   • Enabled per tool via _LWW_SPECS. Diesel, Electric and FRT are ALL
//     live — FRT's spec has been populated (and therefore active) since
//     S612. An earlier version of this comment claimed FRT "activates after
//     its two-device acceptance test"; that was STALE and misled the S646
//     session into hunting for a switch that did not exist (S666 corrects
//     the record). The two-device acceptance run itself now exists:
//     frt/tests/sim/pincomment.mjs + soloinputs.mjs, 20/20 back-to-back.
// ═══════════════════════════════════════════════════════════════════════
var _LWW_SPECS = {
  diesel: {
    arrays: {
  ...
  frt: {
    arrays: {
      ...
      sketches:            { key: 'id' },
      /* S730 — delete tombstones: { id:'kind:target', kind, target, delAt, _ts }.
         Union by id across devices; items never change once written, so no
         typed fields. The host (FRT model) sweeps whatever this names at load. */
      tombstones:          { key: 'id' },
      /* S731 — the issue ledger and the print record. Both were guarded
         against a subset wipe but were NOT keyed, so two tablets minting the
         same letter with different entry ids produced a wholesale array win:
         one device's issue record simply disappeared. Keyed by id they both
         survive and the navigator shows a tie instead of a guess. */
      versions:            { key: 'id' },
      exportRecords:       { key: 'id' }
    },
    statusMaps: [],
...
/* ═══ S643 — THE LEDGER MUST DESCRIBE THE DEVICE, NOT THE DECISION ══════════
   Mark, 09 Aug: typed 660 into NPSH on the tablet and 88 into the PRV on the
   phone. Both died. The cloud row afterwards held the OLD 30 wearing a stamp
   minted three minutes earlier — an old value certified as the newest thing
   anyone had typed, which then outranked every genuine edit on every device.

   ROOT CAUSE, and it is an ordering fault, not a stamping fault. The pull
   path wrote `_lastStampedLocal = <the merged document>` and only THEN handed
   that document to the host to paint. The assignment says "this device now
   holds these values." It was made before the device held anything, and it
   was made unconditionally — the apply that follows is wrapped in a silent
   catch in three separate places (the engine's own, the facade's
   _applyCloudSilent, and _applyLoadedState's terminal handler), so a paint
   that fails, fails invisibly and the claim stands anyway.

   From that moment the ledger is a lie about this device, and every later
   decision built on it is wrong in the same direction:
     • the stamper diffs screen against ledger, sees 30 where the ledger says
       660, and concludes a person just typed 30 — so ANY keystroke anywhere
       on the report launders the whole unpainted screen into "newest truth";
     • `_stampedAlready` reads false, so the merge treats the stale value as
       an unpushed edit and defends it;
     • the recorder reports a local stamp that no one earned.
   Three sessions were spent hunting the stamper. The stamper was reasoning
   correctly from a false record.

   THE FIX IS TO STOP GUESSING AND LOOK. After the apply, re-read what the
   device actually holds and make THAT the ledger. Stamps are then re-assigned
   by value identity under the S625 law — a value never wears another value's
   stamp:
     • the screen took the applied value  ⇒ the applied document's stamp;
     • the screen still holds what it held ⇒ its own previous stamp, so a
       stale reading argues with its real age and lawfully loses to the newer
       cloud copy instead of beating it;
     • it matches neither (someone typed during the apply) ⇒ 0, entry time
       awaits its mint at the next save.
   NO TIME IS INVENTED HERE. Times are moved, exactly as at the 412 door.

   Deliberately NOT done: a "is this stale or typed?" discriminator on the
   stamper. That question cannot be answered from the data — a screen reading
   30 looks identical whether the paint failed or an inspector deliberately
   typed 30 back to correct a colleague — and the version that guesses would
   have eaten the correction. Fix the record, and the stamper needs no help.

   IT IS ALSO THE DETECTOR. Any arbitrated value the screen did not take is
   named to sync_diag. That is a direct observation of the repaint failure
   (handoff OPEN 2) rather than another theory about it, and it fires whether
   or not an exception was thrown.

   NEVER RUNS ON THE BOOT PULL (`opts.allowStaleOverwrite`). There the host
   applies on its own schedule and the model is still the default skeleton;
   anchoring to it would record a blank as this device's state and hand the
   next save a fabricated entry for every real value — the S593/S597 fault. */
function _anchorLedgerToDevice(applied, prevLedger, where) {
  try {
    var spec = _LWW_SPECS[_toolKey];
    if (!spec) return;
    /* ═══ S643b — ONLY HOSTS WHOSE getProject() READS THE SCREEN ══════════════
       The anchor exists to catch a SCREEN that did not take the value. It can
       only do that where getProject() is a fresh read of what the inspector is
       actually looking at — Diesel and Electric, whose adapters run a live
       _collectCloudState() on every call.

       FRT is a different animal and must be left alone. Its getProject()
       returns the in-memory model BY REFERENCE, so it always agrees with what
       was just applied: there is nothing for the anchor to detect, and FRT is
       structurally incapable of the fault in the first place — its save path
       reads the model, never the screen, so a failed repaint cannot launder a
       stale value into the cloud there.

       Worse, running it anyway was actively harmful. FRT's model carries photo
       dataUrls and thumbnails that the cloud shape strips; the ledger would
       have become a deep clone of the whole model, cloned on every applying
       pull and PERSISTED TO INDEXEDDB alongside the snapshot. On a photo-heavy
       review that is tens of megabytes per pull on a field tablet — memory
       spikes, slow pulls, and an IDB quota failure that would surface as
       "sync stopped working" with no obvious cause. Caught before the field
       saw it; FRT's own converge/stalemate probes were green either way,
       because probe reports carry no real photo payloads. */
    if (!_anchorHostReadsScreen) return;
    var held = null;
    try { held = model.getProject(); } catch (_) { return; }
    if (!held || typeof held !== 'object') return;
    held = JSON.parse(JSON.stringify(held));
    applied = applied || {};
    prevLedger = prevLedger || {};
    /* Scalars and field maps keep their entry times OUTSIDE the value, in
       _fts, so they must be re-assigned by value identity. Same law, same
       helper the 412 door already uses. */
    _alignSpecStamps(held, applied, prevLedger);
    /* Status maps with plain values (checklist ticks) keep theirs in
       _fts[map][key] too. Object values carry _ts inline and travel with the
       object, so they are left alone. */
    (spec.statusMaps || []).forEach(function (sm) {
      var g = held[sm];
      if (!g || typeof g !== 'object' || Array.isArray(g)) return;
      var a = (applied[sm] && typeof applied[sm] === 'object') ? applied[sm] : {};
      var p = (prevLedger[sm] && typeof prevLedger[sm] === 'object') ? prevLedger[sm] : {};
      var aF = (applied._fts && applied._fts[sm]) || {};
      var pF = (prevLedger._fts && prevLedger._fts[sm]) || {};
      held._fts = held._fts || {};
      var out = held._fts[sm] = held._fts[sm] || {};
      Object.keys(g).forEach(function (k) {
        if (g[k] && typeof g[k] === 'object') return;
        var gv = stableKey(g[k]);
        if (gv === stableKey(a[k]))      out[k] = aF[k] || 0;
        else if (gv === stableKey(p[k])) out[k] = pF[k] || 0;
        else                             out[k] = 0;
      });
    });
    /* The observation, reported: which cloud values did the screen refuse? */
    var unpainted = [];
    (spec.scalars || []).forEach(function (f) {
      if (unpainted.length >= 12) return;
      if (!Object.prototype.hasOwnProperty.call(applied, f)) return;
      if (stableKey(held[f]) !== stableKey(applied[f])) unpainted.push(f);
    });
    (spec.fieldMaps || []).forEach(function (fm) {
      var a = applied[fm];
      if (!a || typeof a !== 'object' || Array.isArray(a)) return;
      var g = (held[fm] && typeof held[fm] === 'object') ? held[fm] : {};
      Object.keys(a).forEach(function (k) {
        if (unpainted.length >= 12) return;
        if (stableKey(g[k]) !== stableKey(a[k])) unpainted.push(fm + '.' + k);
      });
    });
    _lastStampedLocal = held;
    if (unpainted.length) {
      console.warn('[Sync S643] the screen did not take ' + unpainted.length +
        ' cloud value(s): ' + unpainted.join(', '));
      try {
        if (_onDiag) _onDiag('apply_unpainted', {
          where: where || 'pull', count: unpainted.length, fields: unpainted
        });
      } catch (_) {}
    }
  } catch (e) {
    console.warn('[Sync S643] ledger anchor skipped:', e && e.message);
  }
}
...
function _resolveSpecConflicts(mergeResult, mine, theirs) {
  ...
  Object.keys(spec.arrays || {}).forEach(function (af) {
    ...
    merged[af] = order.map(function (k) {
      ...
      var win3 = (tts3 > mts3) ? e.t : e.m;
      /* ═══ S666 — THE ITEM MUST NOT WIN ITS CHILDREN'S ARGUMENTS. ═══════════
         "Wins whole" was written for Diesel, where an item is a shallow
         reading. For FRT an item is a CONTRACTOR — a container holding every
         deficiency and observation under it, three levels of other people's
         work. So a pin device A had already SAVED TO THE CLOUD was annihilated
         the moment device B's push collided: B's contractor carried the newer
         _ts, won whole, and its deficiency list — which had never heard of
         A's pin — replaced the union merge3 had just correctly built.
         (Reproduced by pincomment.mjs check 6 / soloinputs.mjs case B; merge3
         proven innocent in isolation — this pass was the only hand left.)
         The pull door already speaks the right law: recurse into declared
         nested children, union additions, arbitrate leaves by their own
         stamps. Same law at this door now, via the SAME function — never a
         matching copy. Scoped by declaration: only arrays whose spec says
         `nested` recurse (FRT's contractors/generalDeficiencies/drawings);
         Diesel's shallow arrays and photo lists keep whole-item LWW. */
      var nSpec3 = spec.arrays[af].nested || null;
      if (nSpec3) {
        try {
          var snapArr3 = (_lastSeenSnapshot && Array.isArray(_lastSeenSnapshot[af])) ? _lastSeenSnapshot[af] : [];
          var snapIt3 = null;
          for (var si3 = 0; si3 < snapArr3.length; si3++) {
            if (_lwwKey(snapArr3[si3], keyF, si3) === k) { snapIt3 = snapArr3[si3]; break; }
          }
          var stats3 = {};
          var kids3 = _lwwMergeNestedChildren(e.m, e.t, snapIt3, nSpec3, stats3, _nowSync());
          if (kids3) Object.keys(kids3).forEach(function (cf3) { win3[cf3] = kids3[cf3]; });
        } catch (_) { /* on any doubt, the stamp-winner stands unmodified */ }
      }
      return win3;
    });
  });
...
function _persistSyncMeta(projectId, instanceId) {
  ...
  var record = {
    ...
    /* S692 — the authority mark this device has ACCEPTED. It must survive a
       relaunch: a mark held only in memory would make every cold boot look
       like a device that has never seen the correction, and the takeover
       would fire (harmlessly but noisily) on every open. */
    authSeen: _authSeen || 0,
...
/* ═══ S692 — SET THE OUTGOING COPY ASIDE BEFORE AN AUTHORITATIVE REPLACE. ═══
   An authority mark says "the cloud copy is the truth now", and this device's
   copy is about to stop existing. It is never simply discarded: it is written
   to the same IDB store the sync snapshot already uses, under its own key, so
   a wrong correction is always recoverable from the device that held the work.
   Fire-and-forget by design — a failure here must never block the takeover,
   because the alternative is a device that keeps re-imposing a superseded
   report on everyone else. */
function _setAsideLocalCopy(projectId, instanceId, toRev) {
  try {
    var proj = (model && typeof model.getProject === 'function') ? model.getProject() : null;
    if (!proj) return;
    var rec = {
      id: 'authBackup:' + _syncMetaKey(projectId, instanceId) + ':' + Date.now(),
    ...
    IDB.put('syncMeta', rec).then(function () {
      console.warn('[Sync S692] Local copy set aside as ' + rec.id);
    }).catch(function (e) {
      console.warn('[Sync S692] set-aside failed (non-fatal):', e && e.message);
    });
  } catch (e) {
    console.warn('[Sync S692] set-aside skipped (non-fatal):', e && e.message);
  }
}
 ...
 */
/* ═══ S692 FIX — THE ACCEPTED MARK GETS ITS OWN RECORD. ══════════════════════
   The mark was stored inside the sync-snapshot record, and that record is
   rejected on read unless it carries BOTH a timestamp and a full snapshot:

       if (!rec || !rec.updatedAt || !rec.snapshot) return false;

   So on any device whose snapshot had not been written yet, the accepted mark
   was thrown away together with the record, the device read zero on the next
   open, and it took the same correction again — and told the person again, and
   again. Mark saw that dialog every time he opened 7155.34.

   The two facts have nothing to do with each other. A snapshot is a merge
   ancestor that is legitimately absent sometimes; "this device has accepted
   correction N" is a permanent acknowledgement that must survive on its own,
   and must be written the instant it is true rather than as a side effect of
   persisting something else. Its own key, its own read, its own write. */
function _authSeenKey(projectId, instanceId) {
  return 'authSeen:' + _syncMetaKey(projectId, instanceId);
}
function _persistAuthSeen(projectId, instanceId) {
  try {
    return IDB.put('syncMeta', { id: _authSeenKey(projectId, instanceId),
                                 kind: 'authority-accepted',
                                 authSeen: _authSeen || 0,
                                 savedAt: new Date().toISOString() })
      .catch(function (e) { console.warn('[Sync S692] could not record accepted mark:', e && e.message); });
  } catch (e) { return Promise.resolve(null); }
}
function _restoreAuthSeen(projectId, instanceId) {
  if (!projectId) return Promise.resolve(false);
  return IDB.get('syncMeta', _authSeenKey(projectId, instanceId)).then(function (rec) {
    if (rec && typeof rec.authSeen === 'number' && rec.authSeen > _authSeen) {
      _authSeen = rec.authSeen;
      console.log('[Sync S692] accepted authority mark restored:', _authSeen);
    }
    return true;
...
function _restoreSyncMeta(projectId, instanceId) {
  ...
  return IDB.get('syncMeta', key).then(function(rec) {
    ...
    /* S692 — restore the accepted authority mark. Absent on records written
       before S692: those devices read 0, which is correct — they have accepted
       no correction, so the first raised mark reaches them. */
    if (typeof rec.authSeen === 'number' && rec.authSeen > _authSeen) _authSeen = rec.authSeen;
...
window.addEventListener('online', function() {
  _online = true;
  /* S699 — if a host registered the reconnect hook, IT is the single path and
     this engine does not also flush. If none is registered, behaviour is
     unchanged from before S699: push whatever is pending. */
  if (_reconnectAfterPull) {
    console.log('[Sync S699] Back online — running the host reconnect path (pull → repaint → push if ahead)');
    SyncEngine.reconnect();
    return;
  }
  if (_pendingSync) {
...
function _reportPartialOutcome(info) {
  _lastPartialInfo = info || null;   /* S672: consumed by _notePushMode below */
...
var _lastPartialInfo = null;   /* S672: last scoped-save outcome, for save_mode */
function _notePushMode(wasPartial) {
  /* ═══ S672 — SAVE-MODE TELEMETRY ══════════════════════════════════════════
     Change-scoped saves shipped in S566, and until now nobody could say how
     often they actually won in the field: outcomes went to a per-tool journal
     (Diesel) or the browser console (FRT), and the sync_diag table only ever
     saw anomalies. Every decision about the sync engine's future — finishing
     the scoped path, retiring the double bookkeeping, sizing tablet data use —
     needs that number. This is the ONE point every confirmed save passes,
     scoped or full, every tool, so it is the one place that can report the
     mode without any host remembering to. Fire-and-forget through the same
     _onDiag hook the pull path already uses; a tool with no hook wired loses
     telemetry, never a save. */
  try {
    if (_onDiag) {
      var d = { mode: wasPartial ? 'partial' : 'full' };
      if (_lastPartialInfo) {
        if (_lastPartialInfo.reason) d.reason = String(_lastPartialInfo.reason).slice(0, 80);
        if (_lastPartialInfo.sent)   d.sent   = _lastPartialInfo.sent;
        if (_lastPartialInfo.sentKB != null) d.sentKB = _lastPartialInfo.sentKB;
        if (_lastPartialInfo.fullKB != null) d.fullKB = _lastPartialInfo.fullKB;
      } else if (!wasPartial) {
        d.reason = _partialSaveCfg ? 'unscoped' : 'no partial config';
      }
      _onDiag('save_mode', d);
    }
  } catch (_) {}
  _lastPartialInfo = null;
...
var SyncEngine = {
  ...
  pull: function(projectId, instanceId, opts) {
    ...
    opts = opts || {};
    /* S692 — AUTHORITATIVE PULL. True when the caller demanded the cloud copy
       outright (reloadFromCloud), or when the row arrives carrying an authority
       mark higher than the one this device has accepted (decided below, once
       the row is in hand). While true, every guard and the per-item merge stand
       down: they exist to protect local work from an unexplained cloud, and an
       authority mark IS the explanation. */
    var _AUTH = !!(opts && opts.authoritative);
    /* S692 — did the takeover actually change what this device was showing?
       Only a real change is worth interrupting somebody for. */
    var _authChanged = false;
    var path;
    if (instanceId) {
      path = '/rest/v1/tool_data?select=*&id=eq.' + instanceId;
    } else {
      /* S731 — NO ROW ID, NO PULL. This used to fall back to
         project + tool ORDER BY updated_at DESC LIMIT 1 — "whichever report
         of this project was saved last", which after report #2 exists can be
         report #1. The engine would bind that row and the next push would
         write it. Every legitimate caller already carries the row uuid: the
         Hub puts ?instance= on every tool URL (S495, after 7155.51 lost
         photos to two tabs on one row), Start-next opens the uuid the insert
         returned, and boot reads it from the URL. Standalone has no cloud row
         and must not reach here at all. Refuse loudly rather than guess. */
      var _eni = new Error('SYNC_NO_INSTANCE: refusing to pull without a report row id');
      _eni.code = 'SYNC_NO_INSTANCE';
      console.error('[Sync S731] pull called without an instance id for project ' + projectId + ' — refused. No row was bound and the model was not touched.');
      return Promise.reject(_eni);
    }

    // S124 A3 — try IDB restore first. Non-blocking on failure; the network
    // pull below will overwrite anyway if it succeeds.
    return _restoreAuthSeen(projectId, instanceId).then(function () {
      return _restoreSyncMeta(projectId, instanceId);
    }).then(function() {
      ...
      /* S693 — the issued lock lifts the moment the cloud row says the report
         is no longer issued: unlocking is a deliberate, recorded status flip,
         and the next pull is how every device learns of it. */
      if (_issuedLocked && row.status !== 'issued') {
        _issuedLocked = false;
        _hideSyncBanner();
        console.warn('[Sync S693] Report unlocked (status "' + row.status +
                     '") — cloud saves resume on this device.');
      }
      /* S692 — THE AUTHORITY DOOR (pull side). Rows written before the column
         existed, and every ordinary row since, carry 0 — so this is silent on
         every normal pull in every tool. */
      var _authCloud = Number(row.authority_rev || 0);
      if (_authCloud > (_authSeen || 0)) {
        _AUTH = true;
        console.warn('[Sync S692] AUTHORITY MARK ' + _authCloud + ' > accepted ' +
                     (_authSeen || 0) + ' — taking the cloud copy whole.');
        try { if (_onDiag) _onDiag('authority_takeover', {
          from: _authSeen || 0, to: _authCloud,
          note: String(row.authority_note || '').slice(0, 200),
          at: row.authority_at || null, path: 'pull' }); } catch (_) {}
        /* Set the outgoing copy aside BEFORE it is replaced. This is the
           difference between an authorised correction and data loss: the
           device's own state is always recoverable afterwards, from the
           device, without asking anyone to clear anything. */
        _setAsideLocalCopy(projectId, _instanceId, _authCloud);

        /* ═══ S692 FIX 3 — ANNOUNCE ONLY WHAT THE PERSON NEEDS TO KNOW. ══════
           Twice I made the notice depend on this device REMEMBERING that it
           had accepted the correction, and twice it announced itself again.
           The dependency is the flaw: a device that loses its local storage,
           or a second browser profile, or a fresh tablet, has nothing to
           remember and will keep announcing forever. Remembering is not a
           foundation to build a promise on.

           What actually deserves a person's attention is whether the copy in
           front of THEM is about to change. If this device's copy is already
           identical to the corrected one — which it is for every device that
           has taken it once, and for every device that never held anything
           else — then nothing changes on their screen and there is nothing to
           announce. Taking it silently is correct.

           This is self-limiting by construction: after the first takeover the
           two copies match, so a second notice for the same correction cannot
           be produced, whether or not anything was remembered. */
        try {
          var _lcl = (model && model.getProject) ? model.getProject() : null;
          _authChanged = !_lcl || JSON.stringify(_lcl) !== JSON.stringify(row.data);
        } catch (_ac) { _authChanged = true; }   // can't tell → tell the person
      }
      ...
      // pushed. A pull passes opts.allowStaleOverwrite=true to bypass when
      // the HOST has decided the cloud copy should replace what is in memory:
      // a user-confirmed "Pull now", or a boot into an EMPTY model. S729: FRT
      // boot no longer passes it when a real saved document is already held —
      // the host decides per document, not per entry point. Auto/silent pulls
      // are gated.
      if (!opts.allowStaleOverwrite && !_AUTH) {   // S692: an authority mark overrules the stale-overwrite veto
        try {
          ...
          /* ═══ S666 — THE GUARD STANDS DOWN WHERE THE LAW ALREADY RULES. ═══
             This gate compares local `modified` (DEVICE wall clock) against
             cloud `updated_at` (SERVER clock) and, on "local newer", vetoes
             the ENTIRE pull. Cross-clock, whole-project, winner-take-all —
             the same fabrication family S600/S605/S617 removed everywhere
             else, still standing at pull's front door. Harness-proven under
             load (console captures, 23-47 fires per run): a device whose
             wall clock reads ~300ms past the server's stamp refuses EVERY
             pull — a colleague's strictly newer entry included — until it
             happens to push. Sub-second cross-clock skew is exactly what the
             S622i learner deliberately does NOT correct (its dead band says
             the device clock is the better sub-second reference FOR STAMPS),
             so this comparison can sit inverted indefinitely on a healthy
             device. The protection it was built for (S263: typed comments
             vanishing under a stale silent pull) is delivered BETTER by the
             per-item LWW merge below — dirty-vs-snapshot and entry-stamp
             arbitration, per item, one law at every door. So: a tool with an
             LWW spec skips the veto and lets the item merge rule. Tools
             WITHOUT a spec keep the guard unchanged — no better arbiter
             exists for them, and their behaviour is out of this scope. */
          if (!_LWW_SPECS[_toolKey]) {
      ...
      if (data) {
        ...
        if (!_AUTH) data = _guardEmptyArrays(data, 'pull');   // S692
        ...
        // 7155.40 wipe shape.
        if (!_AUTH) data = _guardContentCollapse(data, 'pull');   // S692
        ...
        try {
          ...
          // the per-item merge must not run against a DEFAULT SKELETON — the
          // Diesel host's boot once injected one (60 unanswered items, empty
          // rows) and the merge read those blanks as "local dirty edits",
          // preserving emptiness over the cloud's real content (checklist
          // 20→0). The contract: the merge is skipped whenever the caller
          // passes opts.allowStaleOverwrite, and it is the HOST's job to pass
          // it only when the model holds nothing real. S729: FRT boot now
          // decides that from the model itself (_isBlankSnapshot) and merges
          // whenever a real saved document is in memory; "never on boot" was
          // FRT applying Diesel's incident to a boot that injects a real
          // document. Heartbeat pulls onto a live model always merge.
          var _rawCloudForDiag = null;
          try { _rawCloudForDiag = JSON.parse(JSON.stringify(data)); } catch (_) {}
          if (_lp && !_AUTH && !(opts && opts.allowStaleOverwrite)) data = _mergeLWW(_lp, data, _lastSeenSnapshot);   // S692
        ...
        if (!_AUTH) data = _guardArrayShrinkage(data, 'pull');   // S692
        ...
        if (!_AUTH) data = _guardDeepCollapse(data, 'pull');   // S692
        ...
        /* S643 — the S583 sentence above is right about WHAT the ledger means
           and wrong about WHEN it may be written. "What this device holds" is
           not knowable until the host has painted, and the paint can fail
           silently. Keep the provisional assignment (a great deal downstream
           reads it), then correct it from the device itself immediately after
           the apply. _preApplyLedger is this device's state BEFORE the pull —
           the honest home of a stale screen's entry times. */
        var _preApplyLedger = _lastStampedLocal;
        _lastStampedLocal = _lastSeenSnapshot;
        model.setProject(data);
        if (!(opts && opts.allowStaleOverwrite)) _anchorLedgerToDevice(data, _preApplyLedger, 'pull');
        /* S626b — runs AFTER the anchor now, so the disk-restored entry times
           get the last word on the ledger, exactly as its own comment says
           ("applied ONCE, immediately after the boot pull replaces the
           ledger"). The anchor IS that replacement. */
        ...
        /* ═══ S692 — ACCEPT THE MARK ONLY AFTER THE APPLY. ══════════════════
           The mark is recorded here, not at the door, because everything
           between the two can still fail. A device that recorded the mark and
           then failed to apply would believe it holds the correction while
           showing the superseded report — and would never take it again. */
        if (_AUTH) {
          /* ═══ S692 FIX 4 — THE CORRECTION BECOMES THE NEW COMMON GROUND. ═══
             This is the bug that put 7155.34 FRT #1 back to eight pins after
             the takeover had correctly reduced it to two.

             The takeover replaces the model, but the MERGE ANCESTOR — the
             record of "what this device and the cloud last agreed on" — was
             left at the pre-correction state, because the line that updates it
             sits inside the per-item merge that an authoritative pull skips.
             So on the very next pull the engine compared the corrected cloud
             copy against an ancestor that still contained the withdrawn items,
             concluded the cloud had LOST five site records and two photos, and
             dutifully preserved them. The guards were right given what they
             were told; they were told the wrong history.

             An authoritative correction IS the new agreed state, by definition.
             Nothing about it is provisional and nothing about it is local, so
             the ancestor is set to the cloud copy outright — no dirty-key
             reversal, which exists to protect unpushed local edits that this
             takeover has deliberately set aside. Without this line the
             correction survives one render and is undone on the next pull. */
          try {
            _lastSeenSnapshot = JSON.parse(JSON.stringify(data));
            _lastStampedLocal = _lastSeenSnapshot;
          } catch (_as) {}
          if (_authCloud > (_authSeen || 0)) {
            _authSeen = _authCloud;
            /* Written HERE, on its own, the moment it becomes true — not left
               to _persistSyncMeta, which declines to write when there is no
               snapshot and took the acknowledgement down with it. */
            _persistAuthSeen(projectId, _instanceId);
          }
          /* Nothing this device was holding may now be pushed back: the whole
             point is that the cloud copy wins. The queued save is dropped, not
             merged. The set-aside copy taken at the door is the safety net. */
          _pendingSync = false;
          try { IDB.clear('syncQueue'); } catch (_) {}
          try {
            window.dispatchEvent(new CustomEvent('arencon-authority-replaced', {
              detail: { rev: _authCloud, projectId: projectId, instanceId: _instanceId,
                        changed: _authChanged,
                        note: String(row.authority_note || '').slice(0, 200),
                        at: row.authority_at || null,
                        reload: !!(opts && opts.authoritative) }
            }));
          } catch (_) {}
        }
        _persistSyncMeta(projectId, _instanceId);
  ...
  /* ═══ S692 — RELOAD FROM CLOUD (the escape hatch that must exist). ════════
     Until now the only way to make a device surrender a copy it was wrongly
     holding was to clear the browser's site data — which is not something any
     commercial application asks of the person using it, and which also throws
     away every other project on that device. Mark named this directly: "No
     commercial app requires clearing data/cache."

     This is that action, done properly: set the device's copy aside, then take
     the cloud copy whole — no guards, no merge, no negotiation. It is
     destructive by intent, so the HOST must confirm with the person first
     (custom modal, one tap) before calling it.

     Returns the applied cloud data, or null if the cloud had nothing to give —
     in which case the device keeps what it had and nothing is lost. */
  reloadFromCloud: function (projectId, instanceId) {
    var iid = instanceId || _instanceId;
    console.warn('[Sync S692] Reload from cloud requested — instance:', iid);
    try { if (_onDiag) _onDiag('reload_from_cloud', { instanceId: iid }); } catch (_) {}
    _setAsideLocalCopy(projectId, iid, _authSeen || 0);
    /* I-14 EXCEPTION 3 — user-confirmed replacement. The host shows a confirm
       that says local changes are replaced; the copy is set aside first. */
    return this.pull(projectId, iid, { allowStaleOverwrite: true, authoritative: true });
  },
  ...
  getRemoteUpdatedAt: function(projectId, instanceId) {
    ...
    if (instanceId) {
      ...
      /* S731 — same rule as pull. This is read-only, but its answer decides
         whether a pull fires and against which row; a timestamp read from
         "whichever report was saved last" is a wrong answer about a different
         document. Resolve null: callers already treat that as "don't know",
         which holds rather than acts. */
      console.error('[Sync S731] getRemoteUpdatedAt called without an instance id for project ' + projectId + ' — refused.');
      return Promise.resolve(null);
    }
  ...
  /* ═══ S673 — THE ANCHOR REACHES THE BOOT DOOR, AT THE RIGHT MOMENT.
     S643's anchor deliberately never ran on the boot pull: "there the host
     applies on its own schedule and the model is still the default skeleton".
     Correct — and it left boot as the one window where ledger and screen can
     disagree with pushes live, which is exactly where Mark's field test 3
     died (17 Aug: a relaunch pushed a blank NPSH wearing a fresh mint).
     The schedule is now ANNOUNCED: the facade calls this from its
     bootApplyComplete, after the host's paint, with the applied document.
     Same law, same helper, same detector as the pull door.

     STAMP-0 SCRUB, this door only: a boot merge keeps an unsent offline edit
     with stamp 0 ("awaiting its mint at the next save"). The anchor would
     ledger that value at 0, the next save would then see value == ledger and
     CARRY the 0 — and a zero-stamped push loses to everything, silently
     re-losing the recovered work. A value with no recorded time is an
     unminted edit and does not belong in the ledger: remove it, so the next
     save sees no previous entry and mints its honest time exactly where
     mints have always happened. No time is invented; an absent time is
     declared absent. Gated by _anchorHostReadsScreen like the pull door, so
     FRT is structurally untouched. Harness: tools/sim/bootlaunder.mjs. */
  anchorBoot: function (applied) {
    try {
      if (!_anchorHostReadsScreen) return null;
      _anchorLedgerToDevice(applied || {}, _lastStampedLocal || {}, 'boot');
      var spec = _LWW_SPECS[_toolKey];
      if (spec && _lastStampedLocal) {
        var r = (_lastStampedLocal._fts && _lastStampedLocal._fts._root) || {};
        (spec.scalars || []).forEach(function (f) {
          if (Object.prototype.hasOwnProperty.call(_lastStampedLocal, f) && !r[f]) delete _lastStampedLocal[f];
        });
        (spec.fieldMaps || []).forEach(function (fm) {
          var m = _lastStampedLocal[fm];
          if (!m || typeof m !== 'object' || Array.isArray(m)) return;
          var mf = (_lastStampedLocal._fts && _lastStampedLocal._fts[fm]) || {};
          Object.keys(m).forEach(function (k) { if (!mf[k]) delete m[k]; });
        });
      }
      if (_lastPersistPid) return _persistSyncMeta(_lastPersistPid, _instanceId).catch(function () { return null; });
    } catch (e) { console.warn('[Sync S673] boot anchor skipped:', e && e.message); }
    return null;
  },
  ...
  /* S729 — ISSUE AS ONE SERVER WRITE. The caller has already set status,
     revision, dateOfIssue and the ledger entry on the live model. This runs the
     ordinary push pipeline in issue mode (see _pushRun) so the body that lands
     is exactly the body a normal save would send, and the final request is the
     frt_issue_report RPC, which writes body and status column in one UPDATE.

     Waits for any in-flight push to settle, then takes the flight slot itself
     so no ordinary save can interleave. Never coalesces into a regular push:
     a coalesced follow-up would run as a plain PATCH, the RPC would never be
     called, and the caller would read success from a save that did not issue.

     Resolves with the returned row on success. Rejects on ANY failure — stale
     baseline, offline, refused body, photos still uploading — so the caller can
     revert the local model and say plainly that nothing was issued. */
  issue: function(projectId) {
    return this._atomicLifecycle(projectId, { issue: true });
  },

  /* S732 — REOPEN. Same discipline as issue: wait for any in-flight push, take
     the flight slot so no ordinary save interleaves, never coalesce into a
     regular push (a coalesced follow-up would run as a plain PATCH, the server
     function would never run, and the caller would read success from a save
     that reopened nothing). Rejects on any failure so the caller can put the
     lock back and say what happened. */
  reopen: function(projectId) {
    return this._atomicLifecycle(projectId, { reopen: true });
  },

  /* S732b — REVERT. Deleting the newest issued revision moves the ledger, the
     revision and the status at once, and can land on either a draft or the
     issued copy underneath. Same atomic discipline; the server derives the
     status from the revision letter rather than believing this client. */
  revert: function(projectId) {
    return this._atomicLifecycle(projectId, { revert: true });
  },

  _atomicLifecycle: function(projectId, mode) {
    var self = this;
    if (projectId) _lastPersistPid = projectId;
    var start = function () {
      if (_pushFlight) return _pushFlight.then(start, start);
      var run = self._pushRun(projectId, 0, mode);
      _pushFlight = run.then(function (r) { _pushFlight = null; return r; },
                             function (e) { _pushFlight = null; throw e; });
      return _pushFlight;
    };
    return start().then(function (row) {
      if (!row || !row.id) throw new Error('ISSUE_NO_ROW');
      return row;
    });
  },

  _pushRun: function(projectId, _attempt, opts) {
    _attempt = _attempt || 0;
    var self = this;
    /* S729 — ISSUE MODE. Same serializer, same stamps, same strips, same
       single-flight slot; only the final request differs: one RPC that writes
       body + status column + ledger together (frt_issue_report). Nothing in
       this mode is queued, merged or retried — an issue either lands as one
       row or fails loudly to the caller. */
    /* S729 — ATOMIC LIFECYCLE WRITE. S732 generalises it: issue and reopen are
       the same mechanism — serialise the body exactly as a normal save would,
       then send ONE rpc that writes body + status column together — and differ
       only in which function the server runs and what it will accept. Nothing
       in this mode is queued, merged or retried; it lands as one row or fails
       loudly to the caller. */
    var _issue = !!(opts && (opts.issue || opts.reopen || opts.revert));
    var _rpcName = (opts && opts.reopen) ? 'frt_reopen_report'
                 : (opts && opts.revert) ? 'frt_revert_report'
                 : 'frt_issue_report';
    var proj = model.getProject();
    if (!proj) return Promise.resolve(null);
    /* S729 — the two early exits below resolve null for a normal save ("kept
       locally, will retry"). An issue has no later; name the reason instead. */
    if (_issue && !_online) {
      var _eof = new Error('ISSUE_OFFLINE'); _eof.code = 'ISSUE_OFFLINE'; return Promise.reject(_eof);
    }
    if (_issue && _writeBlockedByFloor) {
      var _efl = new Error('ISSUE_BUILD_BELOW_FLOOR'); _efl.code = 'ISSUE_BUILD_BELOW_FLOOR'; return Promise.reject(_efl);
    }
    ...
    return SyncWorkerHost.serializePush(proj).then(function(serialized) {
      ...
      if (BinaryOutbox && BinaryOutbox.isEnabled && BinaryOutbox.isEnabled()) {
        ...
        rows.forEach(function(r) {
          ...
          if (r.status === 'r2_confirmed') {
            ...
            /* S735 — 'verifying' was missing. A row in the post-PUT check has
               no r2Key on its photo yet (the key is patched on confirm), so it
               was pushed as a record with neither bytes nor a key, and did not
               count toward ISSUE_PHOTOS_PENDING — an issue could lock a photo
               with no image. 'cancelled' stays OUT: the outbox marks a row
               cancelled only after it was r2_confirmed (orphaned pin), so its
               bytes and key exist and stripping would hide a good photo. */
        ...
        /* S735 — contractor-thread photos went on the outbox at S729 but were
           never stripped: mid-upload they rode the push with their dataUrl
           removed and no r2Key, and never blocked an issue. Same rule, same
           counter, as the pin pool. A separate walker because _strip returns
           early on a pin with no photos array, and a thread can still hold
           photos there. Only outbox-tracked ids are touched; legacy thread
           photos pass through unchanged. */
        function _stripThreads(d) {
          if (!d || !Array.isArray(d.observations)) return;
          function _stripList(e, field) {
            if (!e || !Array.isArray(e[field])) return;
            var n0 = e[field].length;
            e[field] = e[field].filter(function(p) {
              return !(p && p.id && stripIds[p.id]);
            });
            stripCount += (n0 - e[field].length);
          }
          d.observations.forEach(function(o) {
            if (!o) return;
            (o.responses || []).forEach(function(e) { _stripList(e, 'rectPhotos'); });
            (o.arenconReviews || []).forEach(function(e) { _stripList(e, 'followupPhotos'); });
          });
        }
        (data.contractors || []).forEach(function(c) {
          (c.deficiencies || []).forEach(_strip);
          (c.deficiencies || []).forEach(_stripThreads);
        });
        (data.generalDeficiencies || []).forEach(_strip);
        (data.generalDeficiencies || []).forEach(_stripThreads);
        if (stripCount > 0) {
          /* S729 — an ISSUE must carry every photograph. A normal save may
             strip photos still in the outbox because the next save will carry
             them; an issued row refuses content changes (PT423), so a stripped
             photo would be gone from the issued copy for good. Refuse instead.
             The caller says so in plain words; nothing is queued. */
          if (_issue) {
            var _eip = new Error('ISSUE_PHOTOS_PENDING');
            _eip.code = 'ISSUE_PHOTOS_PENDING'; _eip.count = stripCount;
            throw _eip;
          }
      ...
      if (_issue) {
        /* S729 — one request, one row. The RPC checks the baseline itself, so
           no If-Match header rides this call: two concurrency checks on one
           write would be two places to get out of step. No baseline → refuse;
           an issue from a context that never pulled has no standing to write. */
        if (!_instanceId) {
          var _eni = new Error('ISSUE_NO_INSTANCE'); _eni.code = 'ISSUE_NO_INSTANCE'; throw _eni;
        }
        if (!_lastSeenUpdatedAt) {
          var _enb = new Error('ISSUE_NO_BASELINE'); _enb.code = 'ISSUE_NO_BASELINE'; throw _enb;
        }
        method = 'POST';
        path = '/rest/v1/rpc/' + _rpcName;
        payload = { p_row_id: _instanceId, p_data: data, p_expected_updated_at: _lastSeenUpdatedAt };
      } else if (_instanceId) {
        method = 'PATCH';
      ...
      /* S729 — in issue mode every refusal is final and is handed back with
         the server's own message: 412 stale baseline, 423 already issued with
         different words, 409 wipe guard, 400 malformed body, 404 row gone,
         428 no baseline. None of them is merged, set aside or retried here —
         the caller reverts the local model and tells the person what happened. */
      if (_issue && !res.ok) {
        return res.json().catch(function() { return { message: res.statusText }; }).then(function(err) {
          var _ie = new Error(err.message || err.msg || res.statusText || ('HTTP ' + res.status));
          _ie.status = res.status; _ie.code = err.code || ('HTTP' + res.status);
          _ie.details = err.details || ''; _ie.hint = err.hint || '';
          try { if (_onDiag) _onDiag('issue_result', { outcome: 'refused', status: res.status, code: _ie.code }); } catch (_) {}
          throw _ie;
        });
      }
      ...
      if (!res.ok) {
        return res.json().catch(function() { return { message: res.statusText }; }).then(function(err) {
          ...
          /* ═══ S693 — THE ISSUED-REPORT LOCK (PT423 / HTTP 423 Locked). ═════
             The server has refused because this report is ISSUED: it went to a
             client, and its content does not move. This is a PERMANENT refusal
             — a state, not an error — so the engine answers with a state:
             the person's unsent work is set aside in this browser's storage,
             the retry machinery stands down, and the host is told so it can
             say, in plain words, what happened and where to continue. A new
             edit will attempt exactly ONE push and be refused again — bounded
             by user actions, never by a timer. Same family as the PT409 wipe
             guard above: the refusal is a SUCCESS of the safety system. */
          if (res.status === 423 || err.code === 'PT423' ||
              msg.indexOf('REPORT_ISSUED') !== -1) {
            var _lk = {};
            try { _lk = JSON.parse(err.details || '{}'); } catch (_le) { _lk = {}; }
            console.error('[Sync S693] SAVE REFUSED — this report is issued and closed to ' +
              'edits. Unsent changes are set aside on this device. ' + (err.hint || ''));
            try { _setAsideLocalCopy(projectId, _instanceId, 'issued-lock'); } catch (_ls) {}
            _issuedLocked = true;
            _pendingSync = false;             // a permanent refusal is never retried
            try { IDB.clear('syncQueue'); } catch (_lq) {}
            try { if (_onDiag) _onDiag('issued_lock_refusal', {
              instance: _instanceId, issued: _lk.issued || '' }); } catch (_ld) {}
            try {
              window.dispatchEvent(new CustomEvent('arencon-report-issued-locked', {
                detail: { projectId: projectId, instanceId: _instanceId,
                          issued: _lk.issued || '', tool: _lk.tool || '',
                          instance: _lk.instance || null }
              }));
            } catch (_lv) {}
            _showSyncBanner('This report was issued' +
              (_lk.issued ? ' on ' + _lk.issued : '') +
              ' and is closed to edits. Your changes are held safely on this device.', 'red');
            return null;
          }
      ...
      /* S729 — an issue never resolves null. null means "saved locally, will
         retry", and an issue that did not land must not be mistaken for one
         that did. Rethrow so the caller reverts and speaks. */
      if (_issue) throw err;
  ...
  _handleConflict: function(projectId, localProj, attempt) {
    ...
    return Auth.request(pullPath).then(function(rows) {
      ...
      /* ═══ S692 — THE AUTHORITY DOOR (conflict side). ═══════════════════════
         The pull door alone is not enough. A device that is mid-save when the
         correction lands arrives HERE instead, and the 3-way merge would do
         exactly what it is built to do: notice that the cloud copy is shorter
         than its own, keep its own, and push it back over the correction. That
         is the 25 Aug loop, three times over. A raised mark ends the merge —
         this push is abandoned and the cloud copy is taken whole. */
      if (Number(cloudRow.authority_rev || 0) > (_authSeen || 0)) {
        console.warn('[Sync S692] AUTHORITY MARK on the conflict path — ' +
                     'abandoning this push and taking the cloud copy whole.');
        try { if (_onDiag) _onDiag('authority_takeover', {
          from: _authSeen || 0, to: Number(cloudRow.authority_rev || 0),
          note: String(cloudRow.authority_note || '').slice(0, 200),
          at: cloudRow.authority_at || null, path: '412' }); } catch (_) {}
        /* I-14 EXCEPTION 4 — THE AUTHORITY MARK, conflict door. Only reachable
           when the SERVER's mark is higher than the mark this device accepted;
           no client can raise it. This is the door that ended the 25 Aug loop —
           a device mid-save never reaches the pull door. DO NOT REMOVE in the
           name of "412 recovery must always merge": merging here is exactly
           what pushed the stale copy back over three correct repairs. */
        return self.pull(projectId, _instanceId,
                         { allowStaleOverwrite: true, authoritative: true })
      ...
      if (mergeResult.conflicts.length === 0) {
        ...
        /* S643 — the same lie lived at this door. S625's own lesson was that
           the pull path had the honesty revert and the 412 path did not; that
           is not repeated here. applyMerged above can fail exactly as silently
           as setProject, and a narrow two-person race almost always lands on
           this side. */
        _anchorLedgerToDevice(guardedMerged, stampedMine, 'merge412');
        _persistSyncMeta(projectId, _instanceId);
      ...
      return Promise.resolve(self.onConflict(mergeResult.conflicts, mergeResult)).then(function(resolution) {
        ...
        _anchorLedgerToDevice(resolution.merged, stampedMine, 'conflict412');   // S643: same law, third door
  ...
  /**
   * S699 — register the host's reconnect steps. `afterPull` is MANDATORY and is
   * the host's repaint; `pushAhead` is optional and lets a host that keeps its
   * own ahead-ledger (Diesel/Electric compare _lastSavedJson vs _lastPushedJson)
   * do its own save instead of the engine's push. Returns false and registers
   * nothing if afterPull is missing — a host that cannot repaint must not get
   * the pull-then-push path.
   */
  onReconnect: function(cfg) {
    if (!cfg || typeof cfg.afterPull !== 'function') {
      console.error('[Sync S699] onReconnect refused: afterPull is mandatory. ' +
                    'Reconnect stays on the pre-S699 push-only path for this host.');
      return false;
    }
    _reconnectAfterPull = cfg.afterPull;
    _reconnectPushAhead = (typeof cfg.pushAhead === 'function') ? cfg.pushAhead : null;
    console.log('[Sync S699] Host reconnect path registered' +
                (_reconnectPushAhead ? ' (host owns the push)' : ' (engine owns the push)'));
    return true;
  },

  /**
   * S699 — THE ONE RECONNECT PATH. Order is fixed here so no host can get it
   * wrong: pull with the ORDINARY merge (never allowStaleOverwrite, never
   * authoritative — reconnect is not an authorised correction, see doctrine
   * I-14) → host repaints → push only if this device is still ahead.
   * Heartbeat flush is deliberately NOT routed through this; it stays push-only.
   */
  reconnect: function() {
    var self = this;
    var params = new URLSearchParams(window.location.search);
    var pid = params.get('project') || _lastPersistPid;
    if (!pid) return Promise.resolve(null);
    if (!_reconnectAfterPull) return _pendingSync ? this.flush() : Promise.resolve(null);

    var _pushStep = function () {
      if (_reconnectPushAhead) return Promise.resolve(_reconnectPushAhead());
      if (!_pendingSync) return Promise.resolve(null);   // not ahead — nothing to send
      return self.push(pid);
    };

    return this.pull(pid, _instanceId).then(function (data) {
      /* The repaint must happen BEFORE any push, because a host that collects
         from the screen would otherwise send pre-pull pixels over the merge
         that just landed. */
      try { _reconnectAfterPull(data); }
      catch (e) { console.warn('[Sync S699] host afterPull threw:', e && e.message); }
      return _pushStep();
    }).catch(function (e) {
      /* A failed pull must not strand unsent work: fall back to the pre-S699
         behaviour, which is exactly what would have happened anyway. */
      console.warn('[Sync S699] reconnect pull failed, falling back to push-only:', e && e.message);
      return _pushStep();
    });
  },
...
SyncEngine.stampSoon = function () {
  try {
    ...
    /* ═══ S673 — THE BOOT BARRIER REACHES THE STAMPER. During boot the screen
       is the default skeleton; a stamp minted against it certifies values
       nobody typed (Mark's field test 3: a blank NPSH pushed at 21:14:32
       wearing a stamp minted seconds after relaunch, killing 660 and then the
       recovered 770 on every device). Facades that hold a boot barrier set
       holdEditStamps=true at init and clear it the moment the host announces
       its boot apply — a 20s facade fallback guarantees it can never stay up.
       Default is falsy, so FRT and every host without a barrier is unchanged. */
    if (SyncEngine.holdEditStamps) return;
    ...
    _stampSoonTimer = setTimeout(function () {
      ...
      /* ═══ S674 — THE CLAIM MAY NOT OUTLIVE ITS VALUE ═══════════════════════
         MARK, 21 AUG: typed 2500 into Rated Flow, closed the app at once,
         reopened — 2000. The cloud never held 2500 at any point, and the OLD
         2000 came back wearing a stamp minted at 23:08:52, replacing one from
         08 Aug. Both symptoms, one ordering fault.
         Three clocks run after a keystroke: the CLAIM is durable at 500ms
         (stampLocal → _persistSyncMeta, just above); the host writes the value
         to its own IDB at 700ms; the value only becomes ARGUABLE at ~5500ms,
         when it reaches the facade cache and the cloud. The host's IDB copy is
         read at boot but cloud is authoritative for typed fields (correctly —
         that merge rescues photo binaries, not readings). Kill in between and
         the ledger holds a timed claim about a number nothing authoritative
         has. At the next boot the claim is an ORPHAN: ledger says changed,
         screen says the old value, they disagree, and the stamping pass mints
         NOW onto the old value — manufacturing recency that then outranks a
         real edit on any device that has not pushed yet.
         So the value rides WITH the claim: the same trigger, the same instant,
         local only, no network. Whatever survives a kill can now argue its own
         case. The 4s/1.5s batching stays exactly as it is for what it is good
         at — batching network writes; durability simply stops depending on it.
         Hook, not a call: this engine cannot reach a host's collect or its
         cache, and each tool keeps its own field-proven save path (the storage
         rule, S496). A facade that sets nothing is unchanged — FRT included.
         Harness: tools/sim/typekill.mjs (4 checks red before this). */
      try {
        if (typeof SyncEngine.onStampPersist === 'function') SyncEngine.onStampPersist();
      } catch (e) { console.warn('[Sync S674] keystroke persist skipped:', e && e.message); }
    }, STAMP_SOON_MS);
...
try {
  if (typeof document !== 'undefined' && document.addEventListener) {
    ...
    /* ═══ S675 — ANY EDIT IS AN EDIT (Mark: "throughout the tool, as a shared
       engine, for any existing or new tool — correctly, not a patch").
       S674 made a typed value durable with its claim — but only TYPED values,
       because durability rode the form-event trigger above. Everything else an
       inspector does reaches state through its own handler and fires no form
       event: a checklist YES/NO/NA answer is a TAP (saved by nothing until the
       30s background loop — six times the window that lost Rated Flow); a
       signature is POINTER STROKES on a canvas; a photo record lands in state
       seconds after the click that started it, announced by no event at all.
       Per-handler save calls are the "handler 19 forgets" failure the S488
       watchdog exists to end — so this is three FEEDERS into the ONE existing
       diff-gated pipeline (stampSoon → stampLocal + onStampPersist), never a
       second pipeline:
         • form events, above, unchanged (S635);
         • document-wide click + pointerup, capture phase, unfiltered — the
           pipeline itself is the filter: a tap that changed nothing collects,
           diffs, and writes nothing;
         • a slow idle sweep for mutations no DOM event announces (the
           photo-attach shape) — visible pages only, same diff gate, so an
           idle report costs one no-op comparison per beat and writes nothing.
       OPT-IN per facade (wideEditTriggers, default false): stamping cadence is
       merge-visible behaviour, so a tool that has not raised the flag — FRT
       today — is byte-for-byte unchanged until its own lane enables it
       deliberately. Diesel and Electric raise it at init.
       Harness: tools/sim/anyedit.mjs (tap, pen, silent arrival — all three
       red before this; idle-invents-nothing green on both sides). */
    var _wideEdit = function () {
      if (SyncEngine.wideEditTriggers) SyncEngine.stampSoon();
    };
    document.addEventListener('click', _wideEdit, true);
    document.addEventListener('pointerup', _wideEdit, true);
    var EDIT_SWEEP_MS = 5000;
    setInterval(function () {
      try {
        if (!SyncEngine.wideEditTriggers) return;

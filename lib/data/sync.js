  ...
  Object.keys(nestedSpec).forEach(function (cf) {
    ...
    cOrder.forEach(function (k) {
      ...
      if (xc && xl) {
        ...
        var xDirty = !xs || _lwwStripNested(xs, deeper) !== _lwwStripNested(xl, deeper);
    ...
    return _restoreSyncMeta(projectId, instanceId).then(function() {
      ...
      if (data) {
        ...
        try {
          ...
          try { SyncEngine.lastPullKeptLocal = !!(_rawCloudForDiag && !contentEquals(data, _rawCloudForDiag)); } catch (_) {}   // S604

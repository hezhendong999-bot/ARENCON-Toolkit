  ...
  try {
    Object.keys(spec.arrays).forEach(function (field) {
      ...
      arr.forEach(function (item, i) {
        ...
        if (p && _lwwStrip(p) === _lwwStrip(item)) item._ts = p._ts || item._ts || 0;
        else item._ts = now;
      });
    });
  ...
  try {
    ...
    Object.keys(spec.arrays).forEach(function (field) {
      ...
      order.forEach(function (k) {
        ...
        if (c && l) {
          var localDirty = !s || _lwwStrip(s) !== _lwwStrip(l);
          var localTs = localDirty ? now : ((l && l._ts) || (s && s._ts) || 0);
          var cloudTs = c._ts || 0;
          if (cloudTs > localTs) { merged.push(c); stats.replacedFromCloud++; }
          else { merged.push(l); if (localDirty) stats.keptLocalDirty++; }
        } else if (l && !c) {
...
function _ensureStaleWatch() {
  ...
  _staleWatchTimer = setInterval(function () {
    ...
    if (ageMin >= 15) {
      _showSyncBanner('YOUR WORK IS NOT REACHING THE CLOUD — last confirmed save ' +
        Math.round(ageMin) + ' min ago. Data is saved on this device only. ' +
        'Check connection; do not clear this app.', 'red');
    } else if (ageMin >= 5) {
      _showSyncBanner('Cloud saves have not been confirmed for ' +
        Math.round(ageMin) + ' min. Working from local copy.', 'amber');
    }
  }, 30000);
}
    ...
    return _restoreSyncMeta(projectId, instanceId).then(function() {
      ...
      if (data) {
        ...
        try {
          ...
          if (_lp) data = _mergeLWW(_lp, data, _lastSeenSnapshot);

...
export function createChangeJournal(config) {
  ...
  /* ── S570 — STAGE THREE, the half that is safe to ship ────────────────────
   * The journal may now RAISE A FLAG. It still may not block, alter, or delay
   * a save, and it still does not travel to the server or feed the merge —
   * those need a rule watched against real jobs, and a guard firing on an
   * unwatched threshold strands an inspector on site.
   *
   * What changes: a save that removed a lot at once no longer sits silently in
   * a panel nobody opens. It marks itself unreviewed, the tool shows a quiet
   * dot, and opening Recent Saves clears it. Worst case if the threshold is
   * wrong in either direction: a dot appears that did not need to, or does not
   * appear when it might have helped. Nobody is ever stopped from saving. */
  var SEEN_KEY = 'arencon_journal_seen_' + (config.tag || 'journal').replace(/[^a-z]/gi, '');

  function _seenAt() {
    try { return parseInt(localStorage.getItem(SEEN_KEY) || '0', 10) || 0; } catch (_) { return 0; }
  }

  /** Flagged saves the person has not looked at yet. Never throws. */
  function unreviewedLosses() {
    var since = _seenAt();
    return lossHistory(50).then(function (rows) {
      return (rows || []).filter(function (r) { return (r.at || 0) > since; });
    }).catch(function () { return []; });
  }

  /** Called when Recent Saves is opened — everything up to now is reviewed. */
  function markLossesSeen() {
  ...
  /** S571 — STAGE THREE, second half: assess two states against the SAME rule
   *  record() uses, without writing anything. Lets a caller decide before a
   *  push whether this save looks like a wipe. Returns [] on any doubt —
   *  a failure here must never manufacture a false alarm. */
  function assessLosses(beforeState, afterState) {
    try {
      if (!beforeState || !afterState) return [];
      return _diff(_measure(beforeState), _measure(afterState)).losses || [];
    } catch (_) { return []; }
  }

  return { record: record, history: history, lossHistory: lossHistory,
           rebase: rebase, note: note, storeName: STORE,
           unreviewedLosses: unreviewedLosses, markLossesSeen: markLossesSeen,
           assessLosses: assessLosses };
}

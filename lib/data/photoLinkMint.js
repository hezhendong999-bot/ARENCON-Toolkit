...
export function buildPhotoHrefs(photos){
  ...
  return mintTokens(keys).then(function(tokenByKey){
    ...
    /* S520 — THE DIAGNOSTIC WAS WRONG AND COST TWO SESSIONS.
       It probed at 10s and, on a 404, printed "this is NOT propagation". That
       conclusion was false and it sent the investigation in the wrong
       direction. The worker source (arencon-r2-worker.js) shows exactly why a
       fresh token 404s: /mintlinks does LINKS.get(token) before writing, as a
       KV-write saving. That read caches a MISS at the edge, and Cloudflare KV
       holds a cached miss for ~60s — so the token it just created reads back
       as absent for about a minute. Proven on Mark's own failing token: 404 at
       10s, HTTP 200 with a 599KB JPEG once the window passed.
       The probe now waits 90s (past the cache window) and, on failure, says
       what is actually true: still not resolving after the known propagation
       window, so worth reporting — rather than asserting a cause it cannot
       know. Nothing here affects the report; links are baked either way. */
          ...
          .then(function(r){
            try {
              if (r.status === 404 || r.status === 403){
                console.warn('[photo links] DIAGNOSTIC: a minted link still does not resolve ' +
                  '90s after minting (HTTP ' + r.status + '), which is past the ~60s KV ' +
                  'propagation window. Key: ' + firstKey + ' — worth reporting.');
              } else {
                console.info('[photo links] DIAGNOSTIC: link resolves (HTTP ' + r.status +
                  ') — links in this report are good.');
              }
            } catch(_){}
          })
          .catch(function(){});
    ...
    /* Heads-up for whoever exports: a link clicked in the first minute after a
       FIRST export of new photos can 404 for that reason alone. Cached tokens
       (any re-export) resolve immediately. */
    try { console.info('[photo links] note: newly minted links can take up to ~60s ' +
      'to resolve (Cloudflare KV). Re-exports use cached tokens and work at once.'); } catch(_){}
    return {
      href: function(p){

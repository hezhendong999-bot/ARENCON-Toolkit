...
export function createPhotoStore(config) {
   ...
   */
  /** Reconnect photos to their device files. S560 — fixes a gap in S554:
   *  _localSrc was only ever set at the MOMENT of retirement, so after an app
   *  restart a retired photo had an empty picture field and nothing pointing at
   *  the file sitting right there on disk — offline, that is a broken tile for
   *  a photo the device actually holds. This walks every byte-less photo and
   *  re-attaches its store file as a live source. Cheap (one getAll, no photo
   *  bytes touched beyond minting URLs) and idempotent. Runs at load and at the
   *  top of every retire pass, so retirement can never outrun reattachment. */
  function hydratePass() {
    var need = [];
    _photos().forEach(function (p) {
      if (p && p.id && !p[BYTES] && !p._localSrc) need.push(p);
    });
    if (!need.length) return Promise.resolve(0);
    return IDB.getAll(STORE).then(function (rows) {
      var byId = {};
      (rows || []).forEach(function (r) { if (r && r.id && r.blob) byId[r.id] = r.blob; });
      var n = 0;
      ...
      if (n) console.log(TAG + ' reconnected ' + n + ' photo(s) to their device files');
      return n;
  ...
  function retirePass(limit) {
    ...
    _photos().forEach(function (p) {
      ...
      /* S560 — the two S554 skips, revisited deliberately:
         BACKUP RECORDS (_isOrigBackup) now retire under the standard gates.
         Their inline copy IS the clean original — the same pixels the confirmed
         /original/ upload holds in cloud storage — so it is a true duplicate,
         which is exactly what retirement exists to remove.
         ANNOTATED photos retire ONLY when their stroke objects (mk) exist.
         With strokes, the marked image is a COMPUTATION: clean original +
         strokes recomposites it anywhere, so the baked inline copy is
         derivable and safe to drop. WITHOUT strokes (legacy S367b-era photos,
         markup existing only as baked pixels) the inline copy can be the last
         copy of the markup anywhere — cloud /original/ is CLEAN — and it is
         kept unconditionally. */
      if (p._annotated && !(p.mk && p.mk.length)) return;
      if (p.r2Status !== 'uploaded') return;
      todo.push(p);
    });
    if (!todo.length) return hydratePass().then(function(){ return 0; });
    var freed = 0, chain = hydratePass();

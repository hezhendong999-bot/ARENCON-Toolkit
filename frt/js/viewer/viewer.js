...
function _renderPins() {
  ...
  if (_useGLPins && window.PinsGL){
    ...
    var _showRings = (function() {    /* S729: per-device preference; default OFF */
      try { var v = localStorage.getItem('frt_show_insp_rings'); return v !== null ? v === '1' : false; } catch (_) { return false; }
    }());
...
document.addEventListener('change', function(e) {
  ...
  /* S729 — inspector rings: per-device localStorage preference; default OFF.
     No longer written to the project — this is a view preference, not data.
     The per-person hide list remains per-project (see _saveHiddenInspectors). */
  if (e.target.id === 'dv-insp-master') {
    try { localStorage.setItem('frt_show_insp_rings', e.target.checked ? '1' : '0'); } catch (_) {}
    _renderPins();
  }
...
function _populateInspectors() {
  ...
  var proj = Model.getProject() || {};
  var ui = proj.ui || {};
  var ringsOn = (function() {    /* S729: per-device preference; default OFF */
    try { var v = localStorage.getItem('frt_show_insp_rings'); return v !== null ? v === '1' : false; } catch (_) { return false; }
  }());

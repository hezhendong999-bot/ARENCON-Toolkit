...
// ── S464: activity[] → CRB thread migration (SHIPS DORMANT) ──
// Follows the S217 discipline exactly: this REWRITES REAL PROJECT DATA, so it
// does NOTHING on load until Mark flips it while watching.
//   Model.auditActivityMigration()    // read-only dry run — changes nothing
//   Model.enableActivityMigration()   // flip ON + re-run load
//   Model.revertActivityMigration()   // undo via the _s464Backup manifest
//
// WHY A QUARANTINE (Mark's decision, option A):
// activity[] ties to an obs by obsRef = a POSITIONAL LETTER ('A','B','C'…,
// derived at render time from the obs array INDEX). The thread ties to an obs
// by index + stable obs.id. Positional letters are NOT stable — deficiencies.js
// itself notes a stale obsRef (pointing at a since-removed obs) silently falls
// back to pin level. So a comment written about Obs C may now point at whatever
// slid into slot C, or at nothing.
//   • obsRef resolves to a live obs → migrate. Provable.
//   • obsRef null (pin-level entry)  → migrate to obs 0. This is ALREADY what
//     the report does (pdf.js folds no-obsRef activity onto the first obs), so
//     it changes nothing a reader sees.
//   • obsRef STALE (no such obs)     → DO NOT MIGRATE. Left in activity[],
//     counted, surfaced. A machine cannot recover intent here, and guessing
//     would launder a mis-attribution into the permanent record — which is
//     exactly what LOCKED_CRB_THREAD_UI.md §0 forbids ("History is never
//     rewritten as something it wasn't").
var _S464_MIGRATE_ENABLED = false;  // DORMANT until Mark runs it with eyes on.
...
/**
 * S464 — activity[] → CRB thread migration engine (option A: migrate-provable,
 * quarantine-stale). ONE engine, used by BOTH the dry-run audit and the real
 * run, so the two can never disagree about what would happen.
 *
 * @param {object}  proj    the project to walk
 * @param {boolean} dryRun  true = inspect only, mutate NOTHING
 * @returns {{migrated:number, quarantined:number, skipped:number, details:Array}}
 *
 * Mapping (see the _S464_MIGRATE_ENABLED comment for the reasoning):
 *   obsRef → live obs   : migrate onto that obs               (provable)
 *   obsRef null         : migrate onto obs[0]                 (matches pdf.js today)
 *   obsRef stale        : QUARANTINE — leave in activity[], never guess
 *
 * Invariants held:
 *   • Migrated entries land as source:'sitelog' — kept, dated, attributed,
 *     visually distinct. History is not rewritten as a formal round.
 *   • Text, date and author are copied VERBATIM. Nothing is reworded.
 *   • Entries are ADDED to the thread; activity[] is NOT emptied. The old
 *     entry is stamped _s464MigratedTo so it renders once (thread), not twice,
 *     and so revert is a pure inverse.
 *   • One-shot + idempotent: an entry already carrying _s464MigratedTo is skipped.
 *   • autoGenerated entries are ignored (system noise, never user speech).
 */
export function _s464Run(proj, dryRun) {
  var res = { migrated: 0, quarantined: 0, skipped: 0, details: [] };
  if (!proj) return res;

  var manifest = [];   // { deficId, actId, obsId, entryId } — exact inverse for revert

  function walk(d) {
    if (!d || !Array.isArray(d.activity) || !d.activity.length) return;
    var obs = d.observations || [];
    if (!obs.length) return;   // nothing to migrate onto

    // Positional letter → obs index, exactly as deficiencies.js derives it.
    var letterToIdx = {};
    for (var i = 0; i < obs.length; i++) letterToIdx[String.fromCharCode(65 + i)] = i;

    for (var a = 0; a < d.activity.length; a++) {
      var act = d.activity[a];
      if (!act || act.autoGenerated) continue;
      if (act._s464MigratedTo) { res.skipped++; continue; }   // already done

      var targetIdx = null, why = '';
      if (!act.obsRef) {
        targetIdx = 0;                       // pin-level → obs 0 (what the PDF already does)
        why = 'pin-level';
      } else if (letterToIdx.hasOwnProperty(act.obsRef)) {
        targetIdx = letterToIdx[act.obsRef]; // resolves to a live obs
        why = 'obs ' + act.obsRef;
      } else {
        // STALE: the obs this pointed at no longer exists. Do not guess.
        res.quarantined++;
        res.details.push({
          deficId: d.id, deficNum: d.num, actId: act.id, date: act.date,
          obsRef: act.obsRef, text: (act.text || '').slice(0, 80),
          outcome: 'QUARANTINED', why: 'obsRef "' + act.obsRef + '" points at no existing observation'
        });
        continue;
      }

      var target = obs[targetIdx];
      if (!target) { res.quarantined++; continue; }

      res.details.push({
        deficId: d.id, deficNum: d.num, actId: act.id, date: act.date,
        obsRef: act.obsRef || null, text: (act.text || '').slice(0, 80),
        outcome: 'MIGRATE', why: why
      });
      res.migrated++;

      if (dryRun) continue;   // ── inspect only, mutate nothing ──

      if (!Array.isArray(target.arenconReviews)) target.arenconReviews = [];
      var entry = {
        id: _uid('ar'),                                  // merge-safe stable id (S461 law)
        round: 0,                                        // 0 = pre-thread history, not a formal round
        frtInstance: act.instance || 1,
        status: null,                                    // a site log never sets the item's pill
        text: act.text || '',                            // VERBATIM
        date: act.date || _todayStr(),                   // VERBATIM
        source: 'sitelog',                               // grey row — distinct from formal rounds
        author: act.label || 'ARENCON',                  // VERBATIM attribution
        migratedFromActivity: act.id,                    // provenance, both directions
        followupPhotos: (Array.isArray(act.photos) ? act.photos : []).map(function(ph) {
          ph = ph || {};
          return {
            id: ph.id || _uid('fph'),                    // id-stamped or merge DROPS it (S461 probe)
            r2Key: ph.r2Key || null, r2Url: ph.r2Url || null, caption: ph.caption || ''
          };
        })
      };
      target.arenconReviews.push(entry);
      act._s464MigratedTo = entry.id;                    // render once; also the revert key
      manifest.push({ deficId: d.id, actId: act.id, obsId: target.id, entryId: entry.id });
    }
  }

  (proj.contractors || []).forEach(function(c) { (c.deficiencies || []).forEach(walk); });
  (proj.generalDeficiencies || []).forEach(walk);

  if (!dryRun && manifest.length) {
    proj._s464Backup = { ts: new Date().toISOString(), moved: manifest };
  }
  return res;
}
...
export var Model = {
  ...
  setProject: function(proj) {
    ...
    // ── S464: activity[] → CRB thread (dormant unless _S464_MIGRATE_ENABLED) ──
    // The engine lives in _s464Run (shared by the dry-run audit), so the audit
    // and the real run can never drift apart. dryRun:true touches nothing.
    if (_S464_MIGRATE_ENABLED) {
      try {
        var _s464 = _s464Run(proj, false);
        if (_s464.migrated > 0 || _s464.quarantined > 0) {
          console.log('[Migrate S464] ' + _s464.migrated + ' activity entr(ies) \u2192 thread as sitelog; '
            + _s464.quarantined + ' quarantined (stale obsRef, left untouched in activity[]). '
            + 'Reversible: Model.revertActivityMigration()');
        }
      } catch (_e464) { console.error('[Migrate S464] aborted, data untouched:', _e464); }
    }
  ...
  // ── S464: activity[] → CRB thread migration controls ──────────────────
  // Run the audit FIRST, on real project data, before enabling anything.

  // auditActivityMigration(): READ-ONLY dry run. Mutates nothing, saves nothing.
  // Prints exactly what WOULD migrate and what WOULD be quarantined.
  auditActivityMigration: function() {
    if (!_project) { console.warn('[S464] No project loaded.'); return null; }
    var r = _s464Run(_project, true);   // dryRun — nothing is written
    console.log('%c[S464 AUDIT] dry run — nothing was changed', 'font-weight:bold');
    console.log('  would migrate : ' + r.migrated);
    console.log('  would QUARANTINE: ' + r.quarantined + (r.quarantined ? '  \u2190 stale obsRef; these stay in activity[] for you to place by hand' : ''));
    console.log('  already done  : ' + r.skipped);
    if (r.details.length && console.table) console.table(r.details);
    return r;
  },

  // enableActivityMigration(): flip the flag ON and re-run the load path so the
  // migration executes against the loaded project. Reversible (see below).
  enableActivityMigration: function() {
    if (!_project) { console.warn('[S464] No project loaded.'); return null; }
    _S464_MIGRATE_ENABLED = true;
    this.setProject(_project);          // re-runs the load-path migration
    _dirty = true;
    _queueSave();
    this._notify('project', _project);
    var bk = _project._s464Backup;
    var n = (bk && bk.moved) ? bk.moved.length : 0;
    console.log('[S464] Migration run. ' + n + ' entr(ies) moved into the thread as sitelog rows. '
      + 'Revert with Model.revertActivityMigration().');
    return n;
  },

  // revertActivityMigration(): exact inverse via the _s464Backup manifest —
  // pulls each created thread entry back out and unstamps the activity row.
  // Nothing was ever deleted, so the revert is lossless.
  revertActivityMigration: function() {
    if (!_project) { console.warn('[S464] No project loaded.'); return 0; }
    var bk = _project._s464Backup;
    if (!bk || !bk.moved || !bk.moved.length) { console.warn('[S464] Nothing to revert.'); return 0; }
    var self = this, restored = 0;
    bk.moved.forEach(function(m) {
      var f = self.findDeficiency(m.deficId);
      if (!f) return;
      var d = f.defic;
      // Remove the thread entry we created.
      var obs = (d.observations || []).find(function(o) { return o && o.id === m.obsId; });
      if (obs && Array.isArray(obs.arenconReviews)) {
        var i = obs.arenconReviews.findIndex(function(e) { return e && e.id === m.entryId; });
        if (i >= 0) obs.arenconReviews.splice(i, 1);
      }
      // Un-stamp the original activity row so it renders normally again.
      var act = (d.activity || []).find(function(x) { return x && x.id === m.actId; });
      if (act) delete act._s464MigratedTo;
      restored++;
    });
    delete _project._s464Backup;
    _S464_MIGRATE_ENABLED = false;   // back to dormant
    _dirty = true;
    _queueSave();
    this._notify('project', _project);
    console.log('[S464] Reverted ' + restored + ' entr(ies). Migration flag back OFF.');
    return restored;
  },
  ...
  addContractorResponse: function(deficId, obsIdx, data) {
    ...
    var entry = {
      ...
      issuedOnInstance: null,                          // set when printed → THE ISSUED LINE (frozen)
      removed: false, removedBy: null, removedAt: null,// soft removal only — never a hard delete
      withdrawn: false,                                // set by re-import; withdrawn \u2260 deleted
  ...
  addArenconReview: function(deficId, obsIdx, data) {
    ...
    var entry = {
      ...
      author: data.author || null,                     // who wrote it (any signed-in inspector)
      // ── S464 Phase 2 fields ──
      replyTo: data.replyTo || null,                   // id of the comment this answers (one level deep)
      issuedOnInstance: null,                          // set when printed → THE ISSUED LINE (frozen)
      edited: false, editedBy: null, editedAt: null,   // edit attribution (ARENCON's own words only)
      removed: false, removedBy: null, removedAt: null,// soft removal only — never a hard delete
  ...
  // ══ S464 — PHASE 2 THREAD MUTATIONS ═══════════════════════════════════
  // Governed by THE ISSUED LINE (LOCKED_CRB_THREAD_UI.md §1):
  //   not yet printed  → DRAFT  → edit / remove freely
  //   printed already  → THE RECORD → frozen. Reply is the only way forward.
  // One rule replaces all versioning/diff/staleness logic.

  // Locate any thread comment by id across both arrays. Returns
  // { obs, entry, kind:'response'|'review' } or null.
  _findThreadEntry: function(deficId, obsIdx, entryId) {
    var f = this.findDeficiency(deficId);
    if (!f) return null;
    var obs = (f.defic.observations || [])[obsIdx];
    if (!obs) return null;
    var i;
    for (i = 0; i < (obs.responses || []).length; i++) {
      if (obs.responses[i] && obs.responses[i].id === entryId) {
        return { obs: obs, entry: obs.responses[i], kind: 'response' };
      }
    }
    for (i = 0; i < (obs.arenconReviews || []).length; i++) {
      if (obs.arenconReviews[i] && obs.arenconReviews[i].id === entryId) {
        return { obs: obs, entry: obs.arenconReviews[i], kind: 'review' };
      }
    }
    return null;
  },

  // THE ISSUED LINE. A comment is frozen once it has been printed in an
  // issued report. This is the single gate every mutation asks.
  isThreadEntryFrozen: function(entry) {
  ...
  // Soft-remove a comment. NOTHING in CRB hard-deletes.
  //
  // Removal NEVER cascades (locked §3): removing a contractor's claim leaves
  // ARENCON's review standing — you looked at the site and formed a judgment,
  // and that happened regardless of what became of their claim. The review is
  // merely TOLD its subject is gone (orphaned) so the thread can't quietly
  ...
  removeThreadEntry: function(deficId, obsIdx, entryId, who) {
    ...
    ['responses', 'arenconReviews'].forEach(function(arrName) {
      (hit.obs[arrName] || []).forEach(function(e) {
        if (e && e.replyTo === entryId && !e.removed) { e.orphaned = true; orphaned++; }
      });
    });
    _dirty = true;
    _queueSave();
    this._notify('crb', { action: 'remove', deficId: deficId, obsIdx: obsIdx, entry: hit.entry, orphaned: orphaned });
    return hit.entry;
  },

  // Undo a soft removal. Always available — removal is reversible by design.
  restoreThreadEntry: function(deficId, obsIdx, entryId) {
    var hit = this._findThreadEntry(deficId, obsIdx, entryId);
    if (!hit) return null;
    ...
    ['responses', 'arenconReviews'].forEach(function(arrName) {
      (hit.obs[arrName] || []).forEach(function(e) {
        if (e && e.replyTo === entryId) delete e.orphaned;
      });
    });
    _dirty = true;
    _queueSave();
  ...
  withdrawThreadEntry: function(deficId, obsIdx, entryId) {
    var hit = this._findThreadEntry(deficId, obsIdx, entryId);
    if (!hit) return null;
    hit.entry.withdrawn = true;
    _dirty = true;
    _queueSave();
    this._notify('crb', { action: 'withdraw', deficId: deficId, obsIdx: obsIdx, entry: hit.entry });
    return hit.entry;
  },

  // Stamp every unissued thread comment in the project as printed on `instance`.
  ...
  // Record that an import stamp (export id) has been seen, so a re-import of
  // the same sheet is caught rather than duplicated. Returns false if already seen.
  registerExportId: function(exportId) {
    if (!_project || !exportId) return false;
    if (!Array.isArray(_project.exportIds)) _project.exportIds = [];
    if (_project.exportIds.indexOf(exportId) >= 0) return false;   // already imported
    _project.exportIds.push(exportId);
    _dirty = true;
    _queueSave();
    return true;
  },

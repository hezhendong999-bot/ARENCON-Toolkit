...
function _peRenderObsTabs(d) {
  ...
  obs.forEach(function(o, i) {
    ...
    var custMark = (Array.isArray(o.photoSelection))
      ...
      : '';
    // S205c (Mark): per-obs delete ✕ — hover-reveal on desktop, always
    // shown on touch (CSS). Hidden when only one obs remains (a pin must
    // keep at least one observation).
    var delX = _multi
      ? '<span class="pe-obs-del" data-pe-obs-del="' + i + '" role="button" aria-label="Delete observation ' + (i + 1) + '" title="Delete this observation">\u2715</span>'
      : '';
    html += '<button class="pe-obs-tab' + (i === _peObsIdx ? ' active' : '') + '" data-pe-obs="' + i + '">Obs. #' + (i + 1) + custMark + delX + '</button>';
  });
...
document.addEventListener('click', function(e) {
  ...
  // Obs tab click
  // S205c (Mark): per-obs delete ✕ (inside the tab; must precede the
  // tab-switch branch). Confirms, then Model.removeObservation. Clamps the
  // active index so the editor stays on a valid tab.
  var obsDel = e.target.closest && e.target.closest('[data-pe-obs-del]');
  if (obsDel) {
    var fdel = Model.findDeficiency(_peDeficId);
    if (!fdel) return;
    var dIdx = parseInt(obsDel.getAttribute('data-pe-obs-del'), 10);
    var obsArr = (fdel.defic.observations || []);
    if (obsArr.length <= 1) { toast('A pin must keep at least one observation'); return; }
    showConfirm('Delete observation', 'Delete Obs. #' + (dIdx + 1) + ' from this pin? Its text and photo selection are removed. This cannot be undone.').then(function(yes) {
      if (!yes) return;
      Model.removeObservation(_peDeficId, dIdx);
      var f7 = Model.findDeficiency(_peDeficId);
      if (!f7) return;
      var n = (f7.defic.observations || []).length;
      if (_peObsIdx >= n) _peObsIdx = Math.max(0, n - 1);
      _peRenderObsTabs(f7.defic);
      _peRenderObsContent(f7.defic, _peObsIdx);

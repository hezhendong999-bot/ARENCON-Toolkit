export var PDF_PIPELINE_BUILD='S501'; // S499e: module-level so app.js imports the RESIDENT engine build for the boot line
/**
...
function _buildCSS(fontB64){
  ...
  // S501: .rep now rides the grey meta line as a quiet, subordinate chip — it
  // reports what the CONTRACTOR claimed, and must never read louder than the
  // ARENCON pill (the row\u2019s only coloured status verdict). Meta-grey, weight
  // 600, no colour of its own.
  c+='.tr-meta .rep{color:#928E9C;font-weight:600;letter-spacing:0;}';
  ...
  c+='.arv{border-left:3px solid #9C2742;padding-left:20px;}';
...
function _buildDefCard(r,hdrExtra){
  ...
  // S467: entries stamped _s464MigratedTo now live in the thread (crbRender
  // sitelog rows) — skip them here or they print twice. GATED on _frtCrbLive:
  // when the thread box isn't rendering, the FOLLOW-UP row is the entry's ONLY
  // surface and must still print (an unconditional skip would vanish it).
  var actArr=d.activity&&d.activity.length?d.activity.slice().filter(function(a){return !a.autoGenerated&&!(window._frtCrbLive&&a._s464MigratedTo);}).sort(function(a,b){return(b.date||'').localeCompare(a.date||'');}):[];
  var fuActs;
  if(po.id){

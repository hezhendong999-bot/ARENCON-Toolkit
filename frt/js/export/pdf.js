...
function _buildDefCard(r,hdrExtra){
  ...
  // The pill now carries the recommendation state, so the separate REC chip
  // would be redundant ("REC" + "Recommendation"). Show the REC chip ONLY when
  // the pill itself does NOT already say Recommendation (i.e. a closed rec,
  // which reads "Closed" — the REC chip then preserves the rec identity).
  var _showRecChip=_isRec && pillCls!=='pill-rec';

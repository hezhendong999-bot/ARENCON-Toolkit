...
function _styleOnce() {
  ...
  st.textContent = [
    ...
    '.exv-multi{border:1.5px solid #2A3344;border-radius:8px;overflow:hidden;background:#161C28;}',
    '.exv-mrow{display:flex;align-items:center;gap:10px;padding:10px 12px;border-bottom:1px solid #232C3C;cursor:pointer;user-select:none;transition:background .12s;}',
    '.exv-mrow:last-of-type{border-bottom:none;}',
    '.exv-mrow.on{background:rgba(156,39,66,.18);}',
    '.exv-mall{background:#1E2636;font-weight:700;}',
    '.exv-msep{height:1px;background:#232C3C;}',
    '.exv-mbox{width:18px;height:18px;flex:0 0 auto;border:2px solid #6B7686;border-radius:4px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:900;line-height:1;background:transparent;}',
    '.exv-mrow.on .exv-mbox{background:#9C2742;border-color:#9C2742;}',
    '.exv-mdot{width:9px;height:9px;border-radius:50%;flex:0 0 auto;}',
    '.exv-mname{flex:1;font-size:calc(14px + var(--ts,0px));font-weight:600;color:#EAEDF2;}',
    '.exv-mct{font-size:calc(12px + var(--ts,0px));color:#9AA3B2;font-variant-numeric:tabular-nums;}',
    '.exv-mhint{font-size:calc(11px + var(--ts,0px));color:#9AA3B2;padding:8px 12px;background:#11161F;}',
...
export var initExportView = {
  ...
  open: function() {
    ...
    ov.querySelector('#exv-go').addEventListener('click', function() {
      ...
      // Do NOT close the modal here. The PDF opens in its own window; keeping the
      // export modal open means closing the PDF doesn't wipe the modal (or the
      // user's scope selections), so they can tweak and re-export. The modal is
      // dismissed only via its own X / Cancel.

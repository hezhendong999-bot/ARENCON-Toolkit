...
function _obsPhotoSyncBadge(ph) {
  ...
  if (ph.r2Status === 'failed') {
    ...
    var photoTs = 0;
    // S482: match the first 13-digit run after any underscore (sph_orig_ ids
    // never matched the old anchored regex -> permanent false yellow).
    ...
    // S482: unparseable id (photoTs 0) = same unknown rule -> synced.
...
// ── Activity Modal (v1-style) — MOVED to ./activityModal.js (S566, stage 1
//    of the deficiency split). Imported back under its own name, so every
//    call site in this file is untouched. ──────────────────────────────
import { _showActivityModal } from './activityModal.js';
...
function _buildPinGroupCard(d, ctrId) {
  ...
  obs.forEach(function(o, oi) {
    ...
    // S471: entries the S464 migration moved into the thread render THERE —
    // skip them here or they show twice. (Unconditional on the card: unlike
    // the PDF, the thread below always renders when it has content.)
    var _obsActs = (d.activity || []).filter(function(a) {
      return a && !a.autoGenerated && !a._s464MigratedTo && a.obsRef === _obsLetter;
    }).sort(function(a, b) { return (b.date || '').localeCompare(a.date || ''); });
    // S471: the CRB thread — responses[], arenconReviews[] and migrated site
    // log for THIS obs, rendered per the locked demo grammar (read-only;
    // actions arrive with step 3).
    var _obsObj = (d.observations || [])[oi];
  ...
  var _generalActivity = activity.filter(function(a) {
    ...
    if (a._s464MigratedTo) return false; // S471: migrated → renders in the obs thread, not here
...
function _buildObsEditor(d, oi, ctrId, opts) {
  ...
  var _obsLetter = String.fromCharCode(65 + oi);
  // S471: skip entries migrated into the thread (they render below) — see site 1.
  var _obsActs = (d.activity || []).filter(function(a) {
    return a && !a.autoGenerated && !a._s464MigratedTo && a.obsRef === _obsLetter;
  }).sort(function(a, b) { return (b.date || '').localeCompare(a.date || ''); });
  // S471: CRB thread render (read-only, locked demo grammar).

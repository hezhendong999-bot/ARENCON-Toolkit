...
function _buildActEntryHtml(a, deficId) {
  ...
  if (actId) {
    ...
    h += '<button data-action="delete-activity" data-defic-id="' + esc(deficId) + '" data-act-id="' + esc(actId) + '" title="Delete" style="background:none;border:none;cursor:pointer;font-size:calc(11px + var(--ts));color:#C0392B;padding:2px 4px;line-height:1;">\u2715</button>';
    h += '</div>';
  }
  h += '</div>';
  return h;
...
function _buildPinGroupCard(d, ctrId) {
  ...
  var _generalActivity = activity.filter(function(a) {
    ...
    if (a.obsRef && _validObsLetters[a.obsRef] && multiObsPin) return false; // threaded
    return true;
  });

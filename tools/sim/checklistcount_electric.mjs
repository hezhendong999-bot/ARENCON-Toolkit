/* ELECTRIC copy of the Diesel checklist-count probe. Identical logic; the section map carries
   s3gen (generator + automatic transfer switch), which Electric has and Diesel does not.
   Keep the two files in lockstep apart from that map and the app path. */
...
const p06  = grab('electric-app/js/part06.js', PRE);
const p06c = grab('electric-app/js/part06c.js', PRE);
const pdf  = grab('electric-app/js/pdfExport.js', PRE);
...
const SZ = { s1: arrLen(p06, 'S1'), s2: arrLen(p06, 'S2'), s3: arrLen(p06, 'S3'),
             ...
             s3gen: arrLen(p06, 'S3_gen') };   // ELECTRIC: generator + ATS section
SZ.s4pld = SZ.s4;
...
{
  const engineSecs = (p06.match(/\b(s1|s2|s3gen|s3|s4pld|s4|s5m|s5):\(typeof/g) || [])
    .map(s => s.split(':')[0]);
...
/* 5 — arithmetic on the real manifest: screen total vs cover total */
{
  ...
  if (gm) {
    ...
    screenTotal = SZ.s1 + SZ.s2 + SZ.s3gen + SZ.s3 + SZ.s4 + SZ.s4pld + SZ.s5m + SZ.s5;
  }
...
/* 6 — a No on a mandatory FA item reaches the verdict */
{
  ...
  const secs = ['s1','s2','s3gen','s3','s4','s4pld','s5m','s5'];

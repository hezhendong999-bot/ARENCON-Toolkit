...
function _realExportPDF() {
  ...
  // The page-break rules now live in lib/export/reportPdf.js (S686, Phase 4
  // resident 4). Diesel supplies only what is its own: the wrapper divs whose
  // children must be promoted before section grouping, the running header's
  // TEXT, and the node new pages are inserted before.
  setTimeout(function(){
    try {
      var wd = w.document;
      ...
      window.ReportPdf.paginate({
        ...
        header: {
          ...
          title:  'Diesel Fire Pump Commissioning Report #' + _pdfInstNum + (_chProjName ? ' - ' + _chProjName : ''),
          projNo: proj.projno||'',

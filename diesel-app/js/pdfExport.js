...
function _appendixEligible(it){
  /* S685 — the MECHANISM is shared (lib/export/reportPdf.js); this SCOPE is
     ...
     photos, flow chart photos, placard + PLD placard photos ONLY. No site
...
function _exportModalOpen(){
  ...
  window.ReportPdf.exportPanel({
    ...
    sub: ((proj.projno?proj.projno+' ':'')+(proj.projname||'')).trim()+' \u00B7 Diesel Fire Pump Commissioning Report',
  ...
  setTimeout(() => {
    try {
      ...
      /* S721 — TWO faults in one block. (1) Only the 3-point array ever printed:
         a 7-point (PLD) test's flow-chart and gauge photos were eligible in the
  ...
  setTimeout(function(){
    ...
    window.ReportPdf.paginate(w, {
      header: {
        client: proj.client||'',
        addr: proj.addr||'',
        title: 'Diesel Fire Pump Commissioning Report #' + _pdfInstNum + (_pgProjName ? ' - ' + _pgProjName : ''),
        projNo: proj.projno||'',

...
function _collectAllPhotos(opts){
  var all = [];
  // S341 (Bug 2 fix): by DEFAULT this returns LIVE photos only — ~28 callers
  // (gallery view, badge filenames, reassign, counts, PDF appendix) rely on that.
  // The deletion-management consumers (Recently Deleted list, Restore, Purge,
  // auto-purge) pass {includeDeleted:true} to get the FULL walk including
  // soft-deleted + original-backup rows. S367 had hard-coded the live-only filter
  // here, which silently starved the trash gatherer → Recently Deleted was always
  // empty and Restore could never find a photo. The flag restores the S354
  // contract ("_collectAllPhotos stays the full walk") for the consumers that need it.
  var _includeDeleted = !!(opts && opts.includeDeleted);
  var _includeBackups = !!(opts && opts.includeBackups);
  // S367: a photo is "live" (eligible to appear in the gallery/appendix) only
  // if it is NOT soft-deleted (S337 `deleted` flag) and NOT an internal
  // original-backup placeholder (`_isOrigBackup`, kept only to restore markup).
  // Every on-screen renderer already skips these; the appendix must too, or
  // deleted placards and backup duplicates leak into the client PDF.
  // S372 (never-bake): the GALLERY now shows clean-original backups as their own
  // Site Record tiles (FRT S363 parity — mark a photo → see the marked one + a
  // clean copy). Callers that want them pass includeBackups:true (gallery only).
  // The PDF appendix never sets it, and _appendixEligible already excludes site
  // records, so client reports never gain backup duplicates.
  function _live(p){
    if(_includeDeleted) return !!p;
    if(!p || p.deleted) return false;
    if(p._isOrigBackup) return _includeBackups;
    return true;
  }
  ...
  (flowTestPhotos||[]).forEach(function(p,i){
    ...
    all.push({photo:p, type:'flowtest', cat:'flow', badge:'3·'+sh, label:_floweqLabel?_floweqLabel(_floweqTag(p)):'Flow Test (3-pt)', section:'flowtest', idx:i, src:_photoSrc(p)});
  });
  // 2. Flow test photos (PLD) — S355: badge by category tag
  (flowTestPhotosPld||[]).forEach(function(p,i){
    if(!_live(p)) return;
    var sh=(typeof _floweqShort==='function')?_floweqShort((typeof _floweqTag==='function')?_floweqTag(p):'flow_chart'):'Flow';
    all.push({photo:p, type:'flowtest-pld', cat:'flow', badge:'7·'+sh, label:_floweqLabel?_floweqLabel(_floweqTag(p)):'Flow Test (PLD)', section:'flowtestpld', idx:i, src:_photoSrc(p)});
  });
  ...
  Object.keys(clState||{}).forEach(function(id){
    ...
    st.photos.forEach(function(p,pi){
      if(!_live(p)) return;
      all.push({photo:p, type:'checklist', cat:'checklist', badge:_clItemNum(id), label:'Checklist '+_clItemNum(id), section:'cl_'+id, idx:pi, src:_photoSrc(p)});
    });
  });
  // 4. Deficiency photos (per contractor)
  Object.keys(deficiencies||{}).forEach(function(ctr){
    (deficiencies[ctr]||[]).forEach(function(d,di){
      (d.photos||[]).forEach(function(p,pi){
        if(!_live(p)) return;
        all.push({photo:p, type:'deficiency', cat:'deficiency', badge:'D'+(di+1), label:ctr+' #'+(di+1), section:'def_'+ctr+'_'+di, idx:pi, src:_photoSrc(p)});
      });
      // Response photos
      (d.responses||[]).forEach(function(r,ri){
        (r.photos||[]).forEach(function(p,pi){
          if(!_live(p)) return;
          all.push({photo:p, type:'response', cat:'deficiency', badge:'D'+(di+1)+'R', label:ctr+' #'+(di+1)+' Response', section:'resp_'+ctr+'_'+di+'_'+ri, idx:pi, src:_photoSrc(p)});
        });
      });
    });
  });
  ...
  (generalDeficiencies||[]).forEach(function(d,di){
    (d.photos||[]).forEach(function(p,pi){
      if(!_live(p)) return;
      all.push({photo:p, type:'general-defic', cat:'general', badge:'G'+(di+1), label:'General #'+(di+1), section:'gdef_'+di, idx:pi, src:_photoSrc(p)});
    });
  });
  ...
  (typeof recordPhotos==='undefined'?[]:recordPhotos).forEach(function(p,i){
    ...
    else if(_k==='placard-pld'){ kl='PLD Placard'; badge='7·Placard'; }
  ...
  (typeof stdData==='undefined'?[]:stdData).forEach(function(row,ri){
    (row && row.photos ? row.photos : []).forEach(function(p,pi){
      if(!_live(p)) return;
      all.push({photo:p, type:'gauge', cat:'flow', badge:('3·'+(row.pct||(ri+''))+(p.tag&&_GAUGE_TAG_SHORT[p.tag]?('·'+_GAUGE_TAG_SHORT[p.tag]):'')), label:'Gauge (3-pt) '+(row.pct||'')+(p.tag&&_GAUGE_TAG_LABEL[p.tag]?(' · '+_GAUGE_TAG_LABEL[p.tag]):''), section:'gauge_std_'+ri, idx:pi, src:_photoSrc(p)});
    });
  });
  (typeof pldData==='undefined'?[]:pldData).forEach(function(row,ri){
    (row && row.photos ? row.photos : []).forEach(function(p,pi){
      if(!_live(p)) return;
      all.push({photo:p, type:'gauge-pld', cat:'flow', badge:('7·'+(row.pct||(ri+''))+(p.tag&&_GAUGE_TAG_SHORT[p.tag]?('·'+_GAUGE_TAG_SHORT[p.tag]):'')+(p.mode==='pld'?'·PLD':'')), label:'Gauge (PLD) '+(row.pct||'')+(p.tag&&_GAUGE_TAG_LABEL[p.tag]?(' · '+_GAUGE_TAG_LABEL[p.tag]):'')+(p.mode==='direct'?' · without PLD':p.mode==='pld'?' · with PLD':''), section:'gauge_pld_'+ri, idx:pi, src:_photoSrc(p)});
    });
  });
  return all;
}
...
function _pgDayKey(p){
  var d = p.addedDate || p.date;
  if(!d && p.id){ var m=(''+p.id).match(/_(\d{10,})_/); if(m) d = parseInt(m[1],10); }
  ...
  var label = dd.toLocaleDateString('en-CA', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
...
function _isPhotoDeleted(p){
  if(!p) return false;
  if(p.delState === 'deleted') return true;
  if(p.delState === 'live') return false;
  // legacy fallback (pre-S354 data not yet normalized): any deleted flag = deleted
  return !!p.deleted;
}
...
function _photoCreatedTs(ph){
  // id pattern: ph_<ms-timestamp>_<rand>. Fall back to 0 if unparseable.
  try{ var m=String(ph&&ph.id||'').match(/^ph_(\d{10,})_/); return m?parseInt(m[1],10):0; }catch(e){ return 0; }
}
function _markPhotoDeleted(ph, opts){
  if(!ph) return false;
  if(_isPhotoDeleted(ph)) return false;           // idempotent
  // opts.force === true means an explicit user Delete action (a real Delete button
  // tap). Any caller WITHOUT force is treated as programmatic (merge propagation,
  // cleanup, etc.) and is NOT allowed to delete a freshly-captured photo — that is
  // the phantom-delete signature that lost field photos on 7155.51.
  var _force = !!(opts && opts.force);
  // ── diagnostic capture (before flags are written) ──
  var _ageMs = -1, _entry = null;
  try{
    var created=_photoCreatedTs(ph), now=Date.now();
    _ageMs = created?(now-created):-1;
    var stack='';
    try{ stack=(new Error('del')).stack||''; }catch(_e){}
    // trim stack to the 3 frames after this function for readability
    var caller=stack.split('\n').slice(2,5).join(' « ').replace(/https?:\/\/[^ )]+\//g,'');
    _entry={ id:ph.id||'(no id)', name:ph.n||'', ageMs:_ageMs, at:new Date(now).toISOString(), caller:caller, forced:_force };
    window._dslDelDiag.push(_entry);
    if(window._dslDelDiag.length>50) window._dslDelDiag.shift();
    console.warn('[del-diag]', _entry);
  }catch(_e){}
  // ── PHANTOM GUARD (P0-1) ──
  // A non-forced (programmatic) delete of a photo younger than 10s is blocked. The
  // photo stays live; the ring-buffer entry above records the caller stack so the
  // exact offending code path is captured for the field.
  if(!_force && _ageMs>=0 && _ageMs<10000){
    console.error('[del-diag] PHANTOM DELETE BLOCKED — fresh photo kept', _entry);
    if(_entry) _entry.blocked = true;
    if(typeof showToast==='function') showToast('\u26A0 Phantom delete blocked: kept '+(ph.n||ph.id||'photo')+' ('+(_ageMs/1000).toFixed(1)+'s after capture). See Photo Delete Log.', 6000);
    return false;
  }
  // ── canonical soft-delete ──
  ph.delState = 'deleted';
  ph.delAt = new Date().toISOString();
  ph.deleted = true; ph.deletedDate = ph.delAt; ph.deletedBy = _force ? 'user' : 'system'; // legacy mirror
  return true;
}
...
function _markPhotoLive(ph){
  if(!ph) return false;
  ph.delState = 'live';
  delete ph.delAt;
  delete ph.deleted; delete ph.deletedDate; delete ph.deletedBy; // clear legacy mirror
  return true;
}
// Migration A: normalize one photo's old flags into the canonical field, in place.
// Called for every photo on load. Idempotent.
function _normalizePhotoDel(p){
  if(!p || typeof p !== 'object') return p;
  if(p.delState === 'deleted' || p.delState === 'live') return p; // already canonical
  if(p.deleted){
    p.delState = 'deleted';
    p.delAt = p.deletedDate || p.delAt || new Date().toISOString();
...
function _normalizeAllPhotoDel(s){
  if(!s || typeof s !== 'object') return s;
  function arr(a){ if(Array.isArray(a)) a.forEach(_normalizePhotoDel); }
  arr(s.recordPhotos); arr(s.flowTestPhotos); arr(s.flowTestPhotosPld);
  if(Array.isArray(s.stdData)) s.stdData.forEach(function(r){ if(r&&Array.isArray(r.photos)) arr(r.photos); });
  if(Array.isArray(s.pldData)) s.pldData.forEach(function(r){ if(r&&Array.isArray(r.photos)) arr(r.photos); });
  if(s.clState) Object.keys(s.clState).forEach(function(k){ var v=s.clState[k]; if(v&&Array.isArray(v.photos)) arr(v.photos); });
  if(s.deficiencies) Object.keys(s.deficiencies).forEach(function(ctr){ (s.deficiencies[ctr]||[]).forEach(function(d){ if(Array.isArray(d.photos))arr(d.photos); (d.responses||[]).forEach(function(r){ if(Array.isArray(r.photos))arr(r.photos); }); }); });
  if(Array.isArray(s.generalDeficiencies)) s.generalDeficiencies.forEach(function(d){ if(Array.isArray(d.photos))arr(d.photos); (d.responses||[]).forEach(function(r){ if(Array.isArray(r.photos))arr(r.photos); }); });
  return s;
}
function _pgVisible(){
...
function _pgTrashDaysLeft(iso){
  if(!iso) return _TRASH_RETENTION_DAYS;
  var t = new Date(iso).getTime();
  if(!t) return _TRASH_RETENTION_DAYS;
  var elapsedDays = Math.floor((Date.now() - t) / 86400000);
...
function _pgGroupView(view){
  var groups={};
  view.forEach(function(a){
    var g=_pgDayKey(a.photo); var key=g.key, label=g.label, ts=g.ts;
    if(!groups[key]) groups[key]={key:key,label:label,ts:ts,items:[]};
...
function _evTileHtml(kind){
  ...
  var map={pump:['📷','Pump','tap / drop · camera'],'pump-pld':['📷','Pump','tap / drop · camera'],placard:['🏷','Placard','tap / drop · camera'],'placard-pld':['🏷','PLD Placard','engine placard · control pressure'],flow:['📊','Flow Chart & Equipment','charts · gauges · calibration'],'flow-pld':['📊','Flow Chart & Equipment','charts · gauges · calibration']};
...
function _salvagePlacardJson(payload){
  ...
  var candidates = [];
  // 1) fenced ```json ... ``` (or plain ``` ... ```) block — take the LAST one,
  //    since the model often shows a draft then a final.
  var fence = /```(?:json)?\s*([\s\S]*?)```/gi, m;
  while((m = fence.exec(raw))) candidates.push(m[1]);
  // 2) fallback: the widest {...} span in the text.
  ...
  for(var i = candidates.length - 1; i >= 0; i--){
    ...
    try{
      ...
      // sanity: must look like a placard result
      if(obj && typeof obj === 'object' &&
         ('nameplate' in obj || 'rated_flow_gpm' in obj || 'rated_pressure_psi' in obj || 'rated_speed_rpm' in obj)){
        return obj;
      }
...
function _downscaleForVision(img){
  // Vision API accepts ONLY image/jpeg|png|gif|webp. A photo whose media_type is
  // 'image/jpg' (note: NOT a real MIME type) or 'application/octet-stream' — which
  // is exactly what an R2-fetched blob can resolve to — is rejected by the worker
  // with a bare HTTP 400. This is why a reloaded 3pt placard (bytes from R2, bad
  // media_type) 400'd at 1 image / 90KB while a freshly-captured 7pt (clean
  // image/jpeg from the camera) passed. Normalize the type; re-encode if needed.
      ...
      reader.onload = function(e){
        try {
          ...
          while (offset < len) {
            ...
            var app1 = offset + 4;
            if (view.getUint32(app1) !== 0x45786966) { resolve(null); return; }
            var tiff = app1 + 6;
...
function _photoDateFromExif(file){
  return _readExifCaptureDate(file).then(function(cap){

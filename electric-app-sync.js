/* ============================================================================
 * ARENCON Electric (electric-app/ rebuild) — CloudSync facade over the SHARED sync engine
 * ----------------------------------------------------------------------------
 * TEMPORARY SECOND COPY of diesel-sync.js with exactly four literals changed
 * (TOOL_KEY, toolKey default, R2P.upload path, R2P.getUrl path: 'diesel' -> 'electric').
 * NOT electric-sync.js — that file is the S567 facade the OLD single-file Electric
 * monolith loads and must stay untouched until that monolith is retired.
 * The one-implementation fix is to make diesel-sync.js read every tool literal
 * from its init opts so both tools load ONE file; that touches live Diesel's sync
 * path and ships with the Owner at a tablet. Until then keep this in lockstep.
 * ----------------------------------------------------------------------------
 * (original header follows) — ARENCON Diesel — CloudSync facade (S492)
 * ----------------------------------------------------------------------------

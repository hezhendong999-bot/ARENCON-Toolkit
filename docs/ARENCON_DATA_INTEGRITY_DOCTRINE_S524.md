...
**I-12. Devices must update themselves — automatically, always (Mark, S525).** A protection that has not reached a device does not exist. Update prompts are ignored in the field and nobody will audit devices. Required for every tool: version check on every launch and periodically while open; background download with **automatic activation** (a prompt may exist as a fallback, never as the only path); and a **version floor** that refuses to open a report when the build is below the minimum safe build. Constraints: never reload underneath an inspector mid-entry, and never sacrifice offline capability — the cached copy exists so the tool works in a basement. **Evidence for this invariant:** at S525 close, after a night of shipping wipe protections, one inspector's device was still stamping saves with a build ~60 sessions old and carried none of them.

**I-13. A merge may only run against a populated model, and blankness is never an edit (S525, learned in the field).** At boot the model is a default skeleton — empty rows, unanswered items. Any merge that treats that skeleton's blankness as "local dirty edits" will preserve emptiness over real cloud content: on 7155.40 instance 2 this hollowed a loaded report from 20 checklist answers to 0 and was stopped only by the server wipe guard. Rules: (a) per-item merging is skipped on boot and manual pulls — the host's own boot block owns IDB-vs-cloud reconciliation; (b) a local item carrying no content can never beat real cloud content unless it carries delete evidence, and it loses **outright** rather than on timestamps, since a tie would let the blank win; (c) **every merge/sync test suite MUST include a boot-state case (empty model + full cloud + full snapshot)** — this defect shipped through ten passing tests because every one started from a populated model, and the field found it in four hours.
...
- **Build: `S525`** (Diesel stamp). Latest engine HEAD `506dde1` (Lane C hotfix) + `4ca07c50` (Lane A: FRT `_intentionalClear` in model delete methods, engine-side flag strip, Issue-button concurrency token, nested deep-collapse guard `_guardDeepCollapse` at `792cee3`). Server: `_intentionalClear` single-use (`wipe_guard_intentional_clear_single_use_s524c`), independently verified as `authenticated` — undeclared delete-all refused, declared delete-all lands, flag consumed, guard re-arms.
...
- **NOT yet done:** MANDATORY AUTO-UPDATE + version floor (I-12 — highest priority, a protection that hasn't reached a device does not exist); two-device acceptance test (gates FRT merge activation), photos out of the save payload (I-7), 3pt/7pt clarity design, review-numbering model, placard-scan false "overwrite someone else's work" modal, Hub blindness to Diesel photos (Lane B).

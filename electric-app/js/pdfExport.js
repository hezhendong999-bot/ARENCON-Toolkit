/* ARENCON — PDF Export / Photo Appendix engine (Diesel-owned: electric-app/js/)
   Extracted VERBATIM from electric-app/js/part06.js at S503 (Lane C).
   ...
   only references anywhere in the repo were electric-app/index.html, sw.js and one comment
   in the monolith.
...
function _appendixEligible(it){
  /* S685 — the MECHANISM is shared (lib/export/reportPdf.js); this SCOPE is
     Diesel's, and it is an Owner decision (S316): gauge & RPM photos, pump
     photos, flow chart photos, placard + VFD placard photos ONLY. No site
     records, no checklist, no deficiencies. Electric will state its own scope
     against the same engine. */
  return window.ReportPdf.appendixEligible(it, {
...
function _appendixHTML(){
  /* S685b — the assembly is the engine's (the report look is toolkit canon);
     what Diesel supplies is Diesel's data: its photo walk, its S316 appendix
     scope, the user's pre-print exclusions, and its gauge-reading order.
     Output is byte-identical to the pre-extraction exporter, pinned by probe. */
  return window.ReportPdf.appendixHTML({
    collect: function(){ return _collectAllPhotos(); },
    eligible: function(it){ return _appendixEligible(it); },
    isExcluded: function(k){ return _appendixExcl.has(k); },
    key: function(it){ return _ppxKey(it); },
    gaugeReadings: (typeof _GAUGE_READINGS!=='undefined'?_GAUGE_READINGS:[]),
    link: function(p, cell){ return _lnk(p, cell); },
    onEmitted: function(){ window._apxBandEmitted = true; }
  });
}
...
/* S696 — THE EXPORT PANEL IS RESIDENT 5 OF lib/export/reportPdf.js. The rules
   about WHO a report goes to (owner always, the vendor the report is about,
   the remembered choice, S691 normalised matching) are mechanism and live in
   the engine now. What stays here is Diesel's: where the names come from,
   what its photos are, and — deliberately unshared — the commit that WRITES
   the distribution and saves. Changing how recipients are PICKED is cosmetic;
   changing how the distribution is STORED is how a client report goes to the
   wrong people. */
  ...
  try{
    if(typeof deficiencies==='object' && deficiencies){
      Object.keys(deficiencies).forEach(function(k){
        ...
        if(Array.isArray(v) && v.length) _forNames.push(k);
      });
    }
  }catch(_){}
  var proj=getProjInfo();
  window.ReportPdf.exportPanel({
    dialog: window.ArenconDlg,
    owner: ((document.getElementById('pi-client')||{}).value||'').trim(),
    contractors: _exportContractorNames(),
    saved: (distribution&&distribution.length)?distribution.slice():null,
    forNames: _forNames,
    photos: {
      included: elig.filter(function(it){ return !_appendixExcl.has(_ppxKey(it)); }).length,
      total: elig.length
    },
    sub: ((proj.projno?proj.projno+' ':'')+(proj.projname||'')).trim()+' \u00B7 Electric Fire Pump Commissioning Report',
    onReviewPhotos: function(){ _prePrintFromMenu(); },
    onCommit: function(names){ _exportModalCommit(names); },
    onGenerate: function(){ _exportPDFGo(); },
    /* S498 asymmetry, preserved: blocking a delete costs nothing, but blocking
       an export strands an inspector with a dead button — so a missing dialog
       engine still produces a report, using the last SAVED distribution. */
    onFallback: function(){ _exportPDFGo(); }
  });
}
function _exportModalClose(){
  /* S498: the engine owns dismissal now. Kept because the name is referenced
     ...
     S696: the open-panel flag moved into the engine with the panel itself. */
  try{ document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape'})); }catch(_){}
}
function _exportModalCommit(names){
  /* S696 — the ticked names arrive from the engine's panel. The FALLBACK read
     is kept for any caller that still commits with no argument: ask the engine
     what is ticked, exactly as this used to ask its own DOM. Storage is
     deliberately NOT shared — writing the distribution and saving stays here. */
  var sel = Array.isArray(names) ? names
          : ((window.ReportPdf && window.ReportPdf.exportPanelSelected) ? window.ReportPdf.exportPanelSelected() : []);
  distribution.length=0;
  sel.forEach(function(n){ distribution.push(n); });
...
function _sizeChartsForPrint(){
  /* S685 — the print-resolution knowledge (716px stage, DPR 3 ≈ 308 dpi,
     aspect preserved) and the restore choreography live in the shared engine.
     Which charts, and how labels are re-placed for the screen, stay Diesel's. */
  return window.ReportPdf.sizeChartsForPrint([chart3pt,netChart3pt,pldChart,pldNetChart], {
    annotate: function(c, id){ if(typeof renderChartAnnotations==='function') renderChartAnnotations(c, id); }
  });
}
...
function _exportPDFGo() {
  ...
  try{ console.info('[pdf] export running ' + (typeof ELECTRIC_BUILD!=='undefined' ? ELECTRIC_BUILD : 'UNKNOWN BUILD')); }catch(_){}
...
function _realExportPDF() {
  ...
  /* ELECTRIC — start-up test records power source; no duration totals. */
  const _pwr = v => v ? v.charAt(0).toUpperCase()+v.slice(1) : '—';
  const _pwrColor = v => v==='normal' ? '#1A7A4A' : v==='emergency' ? '#E67E22' : '#999';
  ...
  var _pdfSfn = window._csHubSfn || (proj.projnum||'') + ' ' + (proj.projname||'Electric Pump Report');
  ...
  var _pdfTitle = _pdfSfn.trim() + ' Electric Pump Commissioning Report' + (_pdfInstNum>1?' #'+_pdfInstNum:'') + ' ' + _pdfRev;
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    /* S732 (audit F23) — Calibri is REQUESTED by name everywhere below but was never
       embedded, and the Android tablets this tool is built for do not have it: every
       field PDF fell back to Roboto, with different glyph widths, so line wraps and
       table cells moved. Carlito is metric-identical to Calibri and freely licensed.
       local() first: a Windows desktop that has the real Calibri keeps using it. */
    @font-face{font-family:Calibri;font-weight:400;font-style:normal;src:local('Calibri'),url(/lib/assets/fonts/Carlito-Regular.woff2) format('woff2');}
    @font-face{font-family:Calibri;font-weight:700;font-style:normal;src:local('Calibri Bold'),local('Calibri-Bold'),url(/lib/assets/fonts/Carlito-Bold.woff2) format('woff2');}
    @font-face{font-family:Calibri;font-weight:400;font-style:italic;src:local('Calibri Italic'),local('Calibri-Italic'),url(/lib/assets/fonts/Carlito-Italic.woff2) format('woff2');}
    @font-face{font-family:Calibri;font-weight:700;font-style:italic;src:local('Calibri Bold Italic'),local('Calibri-BoldItalic'),url(/lib/assets/fonts/Carlito-BoldItalic.woff2) format('woff2');}
    .ph-missing{display:flex;align-items:center;justify-content:center;text-align:center;font-size:7.5pt;line-height:1.3;color:#8A8F98;background:#F4F6F7;border:1px dashed #C4CBD6;padding:6px;}
    ...
    .ph-addr{text-align:left;font-family:Calibri,sans-serif;font-size:6pt;color:#1C2333;line-height:1.26;border-left:2px solid #9C2742;padding-left:10px;}
  ...
  <script>
  /* S732 (audit F23) — a photo that cannot be fetched used to print as an EMPTY grey box,
     so a deficiency read as having no evidence rather than evidence that failed to load.
     One capturing listener catches every image error in this document, present and
     future, and replaces the box with a caption of the same size. The logo is inline
     base64 and never fails; it is excluded so a caption can never appear in its place. */
  document.addEventListener('error',function(e){var t=e.target;if(!t||t.tagName!=='IMG'||t.getAttribute('alt')==='ARENCON Inc.')return;
    var d=document.createElement('div');d.className='ph-missing';d.style.cssText=t.getAttribute('style')||'';
    d.textContent='Photo not available in this copy — see the report record';t.replaceWith(d);},true);
  </script>
  </head><body>
  <div class="page">
  <div class="cover">
  ...
  <div class="sb"><div class="sh3">Fire Pump Start-Up Test — Power Source</div>
  <table class="dt"><thead><tr><th>Start-Up Test #1</th><th>Power Source</th><th>Start-Up Test #2</th><th>Power Source</th></tr></thead>
  <tbody>${BAT_TESTS.map((t,i)=>`<tr><td class="left" style="text-align:left">${t}</td><td style="font-weight:700;color:${_pwrColor(batPower.b1[i])}">${_pwr(batPower.b1[i])}</td><td style="text-align:left">${t}</td><td style="font-weight:700;color:${_pwrColor(batPower.b2[i])}">${_pwr(batPower.b2[i])}</td></tr>`).join('')}
  </tbody></table>
  ...
  ${(function(){
    ...
    if(_isPld) out+='<tr><td style="text-align:left">VFD Setting (psi)</td><td>'+gv(_np.pld)+'</td></tr>';
    ...
    if(testType==='std'){
      out+='<div class="subhd">Standard Fire Pump Performance (w/o VFD) — 3-Point Test</div>';
      ...
      out+='</tbody></table>';
      /* ELECTRIC — three-phase voltage & amperage (3-point), ported verbatim */
      out+='<div style="font-size:9.5pt;font-weight:bold;background:#4A5568;color:white;padding:4px 12px;margin:14px 0 6px;border-radius:3px;">Voltage &amp; Amperage — 3-Point (Standard)</div>';
      out+='<table class="dt"><thead><tr><th>Flow %</th><th>V L1-L2</th><th>V L2-L3</th><th>V L1-L3</th><th>A L1</th><th>A L2</th><th>A L3</th></tr></thead><tbody>';
      vaStdData.forEach(function(r){out+='<tr><td>'+r.pct+'</td><td>'+(r.va||'—')+'</td><td>'+(r.vb||'—')+'</td><td>'+(r.vc||'—')+'</td><td>'+(r.aa||'—')+'</td><td>'+(r.ab||'—')+'</td><td>'+(r.ac||'—')+'</td></tr>';});
      out+='</tbody></table>';
      /* ELECTRIC — auto-transfer test at peak load (3-point), ported verbatim */
out+='<div style="font-size:9.5pt;font-weight:bold;background:#4A5568;color:white;padding:4px 12px;margin:14px 0 6px;border-radius:3px;">Auto-Transfer Test at Peak Load</div>';
      out+='<table class="dt" style="max-width:600px;"><thead><tr><th></th><th>w/o PRV &amp; VFD</th><th>w/ PRV &amp; VFD</th></tr></thead><tbody>';
      out+='<tr><td style="text-align:left;font-weight:600;">Peak Load Pressure (psi)</td><td>'+(document.getElementById('at-std-peak')?.value||'—')+'</td><td>'+(document.getElementById('at-vfd-peak')?.value||'—')+'</td></tr>';
      out+='<tr><td style="text-align:left;font-weight:600;">Time to Restore (s)</td><td>'+(document.getElementById('at-std-restore')?.value||'—')+'</td><td>'+(document.getElementById('at-vfd-restore')?.value||'—')+'</td></tr>';
      out+='<tr><td style="text-align:left;font-weight:600;">Transfer ≤10s</td><td colspan="2" style="font-weight:700;color:'+(at4a1Status==='yes'?'#1A7A4A':at4a1Status==='no'?'#C0392B':'#999')+'">'+(at4a1Status==='yes'?'✓ YES':at4a1Status==='no'?'✗ NO':at4a1Status==='na'?'N/A':'—')+'</td></tr>';
      out+='</tbody></table>';
      ...
      out+='</tbody></table>';
      // Chart
      if(chartImgA) out+='<div style="margin-top:14px;text-align:center;"><div style="font-size:9pt;font-weight:700;color:#2C3E50;margin-bottom:4px;">3-Point Fire Pump Performance Curve (w/o PRV &amp; VFD)</div><img src="'+chartImgA+'" style="width:100%;max-width:850px;border:1px solid #DDD;border-radius:6px;display:block;margin:0 auto;">'+_buildReadoutStripHtml3pt()+_safetyMarginPdf('')+'</div>';
    ...
    // VFD 7-Point table (only if pld)
    if(testType==='pld'){
      out+='<div class="subhd">7-Point Performance Test — Fire Pump with VFD</div>';
      out+='<table class="dt"><thead><tr><th>Flow %</th><th>Flow (gpm)</th><th>Cutsheet (psi)</th><th>Placard (psi)</th><th colspan="2">w/o PRV &amp; VFD</th><th>RPM</th><th colspan="2">w/ VFD Enabled</th><th>RPM</th><th>Pass/Fail</th></tr><tr><th></th><th></th><th></th><th></th><th>Suction</th><th>Discharge</th><th></th><th>Suction</th><th>Discharge</th><th></th><th></th></tr></thead><tbody>';
      ...
      out+='</tbody></table>';
      /* ELECTRIC — three-phase voltage & amperage (7-point, w/o + w/ VFD legs), ported verbatim */
      out+='<div style="font-size:9.5pt;font-weight:bold;background:#4A5568;color:white;padding:4px 12px;margin:14px 0 6px;border-radius:3px;">Voltage &amp; Amperage — 7-Point (VFD)</div>';
      out+='<table class="dt"><thead><tr><th>Flow %</th><th colspan="3">w/o VFD — Voltage</th><th colspan="3">w/o VFD — Amperage</th><th colspan="3">w/ VFD — Voltage</th><th colspan="3">w/ VFD — Amperage</th></tr><tr><th></th><th>L1-2</th><th>L2-3</th><th>L1-3</th><th>L1</th><th>L2</th><th>L3</th><th>L1-2</th><th>L2-3</th><th>L1-3</th><th>L1</th><th>L2</th><th>L3</th></tr></thead><tbody>';
      (typeof vaPldData!=='undefined'?vaPldData:[]).forEach(function(r){out+='<tr><td>'+r.pct+'</td><td>'+(r.va_no||'—')+'</td><td>'+(r.vb_no||'—')+'</td><td>'+(r.vc_no||'—')+'</td><td>'+(r.aa_no||'—')+'</td><td>'+(r.ab_no||'—')+'</td><td>'+(r.ac_no||'—')+'</td><td>'+(r.va_w||'—')+'</td><td>'+(r.vb_w||'—')+'</td><td>'+(r.vc_w||'—')+'</td><td>'+(r.aa_w||'—')+'</td><td>'+(r.ab_w||'—')+'</td><td>'+(r.ac_w||'—')+'</td></tr>';});
      out+='</tbody></table>';
      // Water Supply & Demand — VFD
      out+='<div class="subhd lite">Water Supply &amp; Demand Data (7-Point VFD)</div>';
      out+='<table class="dt" style="max-width:500px;"><thead><tr><th style="text-align:left">Point</th><th>Flow (US gpm)</th><th>Pressure (psi)</th></tr></thead><tbody>';
  ...
  try{
    w.document.addEventListener('click', function(ev){
      ...
      if (t && t.tagName === 'A') {
        /* S522 — OPEN IN A NEW TAB RATHER THAN SWALLOW THE CLICK.
           S513 blocked photo-link clicks in the preview because a stray tap
           replaced the whole preview with the raw image and the report had to
           be regenerated — a real tablet hazard. But blocking outright made
           the preview look broken: the photos are visibly links and clicking
           them did nothing, which cost a round of "the links don't work" when
           the exported PDF was fine all along. Blocking was the right
           instinct, the wrong remedy. Now the preview survives AND the link is
           verifiable before the report ever leaves the building: same-window
           navigation is cancelled, the photo opens in a new tab. This is a
           user-gesture window.open, so it is not treated as a popup. */
        ev.preventDefault(); ev.stopPropagation();
        var _href = t.getAttribute('href');
        if (_href) { try { w.open(_href, '_blank', 'noopener'); } catch(_o){} }
      }
    }, true);
  }catch(_na){}
  ...
  setTimeout(() => {
    try {
      ...
      /* S721 — TWO faults in one block. (1) Only the 3-point array ever printed:
         a 7-point (VFD) test's flow-chart and gauge photos were eligible in the
         pre-print picker, counted as "included", and then never drawn anywhere
         in the client PDF. (2) The picker's exclusions were ignored here, and
         "deleted" was the legacy flag rather than the canonical state. Both
         arrays now print, each under its own sub-header, each photo passing the
         same three tests the picker promised: live, eligible, not excluded. */
      var _ftIsLive = function(p){ return p && !(typeof _isPhotoDeleted==='function' ? _isPhotoDeleted(p) : String(p.deleted)==='true') && !(p.id && _appendixExcl.has(p.id)); };
      var _ftGroups = [
        { label:'Flow Chart &amp; Equipment (3-Point)', items:(flowTestPhotos||[]).filter(_ftIsLive) },
        { label:'Flow Chart &amp; Equipment (7-Point)', items:(typeof flowTestPhotosPld!=='undefined' ? (flowTestPhotosPld||[]) : []).filter(_ftIsLive) }
      ].filter(function(G){ return G.items.length; });
      const _ftLive = _ftGroups.reduce(function(a,G){ return a.concat(G.items); }, []);
      if (ftPrint && _ftLive.length > 0) {
        ...
        var _ftSub = function(lbl){ return '<div class="apx-subhead" data-subhead="'+lbl+'" style="display:flex;align-items:center;gap:9px;padding:16px 0 6px;margin:0 0 11px;border-bottom:1px solid #D8DCE3;">'
          + '<span style="width:4px;height:15px;background:#9C2742;border-radius:2px;display:inline-block;flex:0 0 auto;"></span>'
          + '<span style="font:700 14px Calibri,sans-serif;color:#1C2333;letter-spacing:.3px;">'+lbl+'</span></div>'; };
        ...
        _ftGroups.forEach(function(G){
          ftHtml += '<div class="apx-keep">'+_ftSub(G.label)+'<div style="display:flex;flex-wrap:wrap;gap:12px;">'+_ftCard(G.items[0])+'</div></div>';
          for(var _fi=1; _fi<G.items.length; _fi+=3){
            var _grp = G.items.slice(_fi,_fi+3).map(_ftCard).join('');
  ...
  try {
    ...
    if (typeof _mec === 'function') {
      ...
      try{ console.warn('[Electric] shared export chrome unavailable — fallback cluster mounted'); }catch(_lw){}
    }
  ...
  // ── PAGINATION — resident 4 of lib/export/reportPdf.js (S694). The geometry
  // (page budget, section units, keep rules, S687 photo flow, S688 settle
  // gate, S693 fill/bleed fixes) lives in the ENGINE; this host states only
  // its header content and wrapper ids. The tool name in the title is the
  // host's line to write — Electric will write its own.
  setTimeout(function(){
    var _pgProjName = (proj.projname||'');
    window.ReportPdf.paginate(w, {
      header: {
        client: proj.client||'',
        addr: proj.addr||'',
        title: 'Electric Fire Pump Commissioning Report #' + _pdfInstNum + (_pgProjName ? ' - ' + _pgProjName : ''),
        projNo: proj.projno||'',
        rev: (proj.revision||formRevision||'')
      },
      unwrapIds: ['flow-test-photos-print','sketches-print'],
      insertBeforeId: 'mobile-page-nav'
    });
  }, 1200);

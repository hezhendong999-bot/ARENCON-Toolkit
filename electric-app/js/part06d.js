/* ═══════════════════════════════════════════════════════════════════════════
   electric-app/js/part06d.js — CONTINUATION OF part06.js (S559 split)
   ---------------------------------------------------------------------------
...
function _applyLoadedState(raw) {
  ...
  try {
    ...
    /* ═══ S679 — ONE DECLARED LIST, NOT TWO HAND-WRITTEN ONES ═════════════
       ...
       electric-app/js/reportManifest.js, and walked by lib/data/reportState.js.
    ...
    if (s.batData || s.batPower) { renderBatTable('bat1-body','b1'); renderBatTable('bat2-body','b2'); updateBatTotals(); }
...
function _resetInventory(active){
  ...
  try{
    if(active === 's4' || active === 's4pld'){
      ...
      push(liveRows(typeof pldData!=='undefined'?pldData:[], ['suc_no','dis_no','rpm_no','suc_w','dis_w','rpm_w','flow']), 'entered VFD test row', 'entered VFD test rows');
          ...
          if(_D && _D.panel){
            _D.panel({
              ...
              build:function(bd){
                ...
                d.textContent='This link is missing a specific report. To keep each inspector\u2019s report separate, open the Electric report from its project in the Hub \u2014 pick an existing report or create a new one.';
                bd.appendChild(d);
              },
            ...
            _ov.innerHTML='<div style="max-width:420px;background:#fff;border-radius:16px;padding:28px 26px;box-shadow:0 20px 60px rgba(0,0,0,.4);text-align:center;">'
              ...
              +'<div style="font-size:13px;line-height:1.5;color:#5E5B68;margin-bottom:18px;">This link is missing a specific report. To keep each inspector\u2019s report separate, open the Electric report from its project in the Hub \u2014 pick an existing report or create a new one.</div>'
...
function openHelp(){
  ...
  D.panel({
    ...
    build:function(bd){
      ...
      if (window._helpHasCards && window._helpHasCards('Electric')){
        ...
        window._helpMount(bd, { scope:'Electric', tab:'wn' });
        try { if (window._helpMarkSeen) window._helpMarkSeen('Electric'); } catch(_){}
        _helpSetDot(false);
      } else {
...
function _stateHasContent(s){
  ...
  try {
    ...
    if(s.batPower && ((Array.isArray(s.batPower.b1)&&s.batPower.b1.some(function(x){return !!x;}))||(Array.isArray(s.batPower.b2)&&s.batPower.b2.some(function(x){return !!x;})))) return true;   /* ELECTRIC */
...
function _mergeCloudLocal(cloud, local){
  ...
  if(cloud && local){
    ...
    /* ═══ S639 — ONE PHOTO WALK, NOT EIGHT ═════════════════════════════════
       ...
       existed (silent no-op, binaries wiped on every apply); VFD flow photos
    ...
    /* S639 — the six preserve passes above were six copies of one idea, each
       with its own list of surfaces. Two of them (VFD flow photos, general

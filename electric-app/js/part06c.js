/* ═══════════════════════════════════════════════════════════════════════════
   electric-app/js/part06c.js — CONTINUATION OF part06.js (S559 split)
   ---------------------------------------------------------------------------
...
function renderChartAnnotations(chartInstance, canvasId) {
  ...
  if(!chart) return;
  // S369 SELF-HEAL: the VFD (7-point) charts can finish initialising AFTER
  ...
  chart.data.datasets.forEach(function(ds, dsIdx) {
    ...
    ds.data.forEach(function(pt, ptIdx) {
      ...
      // (Cutsheet, Placard, PRV, VFD, Net-pressure overlays, Available Supply line) are drawn
      ...
      var _ANNOTATE_PRIMARY = {
        ...
        'Measured Discharge (w/ VFD)':1,     // 7-Point discharge primary
        'Net Pressure (w/ VFD)':1,           // 7-Point net primary
      ...
      // S226: cap lines (PRV/PRdV/VFD) — device+setpoint label only (no flow), right endpoint only.
      ...
      var _CAP_LABEL = { 'Pressure Relief':'PRV', 'Pressure Reducing':'PRdV', 'VFD Setting':'VFD' };
...
renderChecklist(S3_gen,'cl-s3gen','s3gen');   /* ELECTRIC */
...
function _ttLabel(t){ return t==='pld' ? '7-Point with VFD' : '3-Point Standard'; }
...
function _applyStateInPlace(jsonStr) {
  try {
    ...
    if(s.batPower) {   /* ELECTRIC */
      if(Array.isArray(s.batPower.b1)) batPower.b1=s.batPower.b1.slice();
      if(Array.isArray(s.batPower.b2)) batPower.b2=s.batPower.b2.slice();
      renderBatTable('bat1-body','b1'); renderBatTable('bat2-body','b2');
    }
...
renderVaStdTable();   /* ELECTRIC */
renderVaPldTable();   /* ELECTRIC */
...
function syncAllFields() {
  ...
  pairs.forEach(([a, b]) => {
    ...
    if (va === vb) return;
    if (va !== '' && vb === '') { eb.value = ea.value; }       // 3pt → VFD (fill empty)
    else if (va === '' && vb !== '') { ea.value = eb.value; }  // VFD → 3pt (fill empty)
...
/* ═══ S548 — THE LOCAL PHOTO STORE NOW LIVES IN lib/data/photoStore.js ═══
   ...
   Names still available globally, unchanged for every caller:
     ...
     _photoStoreReport
   See electric-app/js/part02.js for the wiring. */
...
/* ═══ S636 — ONE PHOTO SERIALISER. THE ADDRESS WAS BEING DROPPED ═══════════
   ...
   older records carry no address at all. Read live from the database today:
     ...
     flow-test VFD      25 saved, 23 with no address
...
function collectState() {
  /* S679 — THE HOST READS THE SCREEN; THE MANIFEST SAYS WHAT A REPORT IS.
     ...
     back in. The list now lives in electric-app/js/reportManifest.js, declared
...
function _collectCloudState(){
  var s=collectState();
  s._build=(typeof ELECTRIC_BUILD!=='undefined')?ELECTRIC_BUILD:'unknown';   // S302: which build wrote this row
...
// "7·25%·D·VFD·(originalname).jpg". Badge read from _collectAllPhotos() — single
...
function _dslRefreshPhotoSurfaces(){
  ...
  try{
    if(typeof renderChecklist==='function'){
      ...
      if(typeof S3_gen!=='undefined') renderChecklist(S3_gen,'cl-s3gen','s3gen');   /* ELECTRIC */

/* ═══════════════════════════════════════════════════════════════════════════
   electric-app/js/part06b.js — CONTINUATION OF part06.js (S559 split)
   ---------------------------------------------------------------------------
   ...
   CONSEQUENCES, both worth knowing:
     - LOAD ORDER IN electric-app/index.html IS LOAD-BEARING. Change it and
...
// Chart handles
let chart3pt = null;        // Chart A: 3-pt discharge w/o PRV & VFD
...
// VFD row indices that are skipped in w/o PRV & VFD (only 0%,100%,150% = indices 0,4,6)
...
// 4b — pldChart pill legend (datasets: 0 supply,1 pump,2 measured,3 SD line,4 SD pt,5 OHL,6 demand,7 static,8 residual,9 PRV,10 VFD)
function renderFigLegendPld(){
  ...
  if(has(2)) defs.push({i:2, c:'#5F8068', kind:'line', dash:[],          t:'Measured Discharge (w/ VFD)', primary:true});
  if(has(1)) defs.push({i:1, c:'#9C2742', kind:'line', dash:[10,5],      t:'Pump Curve'});
  if(has(0)) defs.push({i:0, c:'#6E86B8', kind:'line', dash:[],          t:'Available Supply'});
  ...
  if(has(10))defs.push({i:10,c:'#7D6F9C', kind:'line', dash:[2,3],       t:'VFD Setting', cap:'pld'});
...
// 4b — pldNetChart pill legend (datasets: 0 measured net w/o VFD, 1 adjusted net w/o VFD, 2 cutsheet, 3 placard)
function renderFigLegendPldNet(){
  ...
  function has(i){return ds[i]&&ds[i].data&&ds[i].data.length;}
  var defs=[];
  if(has(0)) defs.push({i:0, c:'#6366F1', kind:'line', dash:[],          t:'Measured Net (w/o VFD)'});
  if(has(1)) defs.push({i:1, c:'#21D3ED', kind:'line', dash:[8,4,2,4],   t:'Adjusted Net (w/o VFD)'});
...
// ── S221: 7-Point NET-pressure chart readout (shows net pressure w/ VFD + PASS/FAIL per point) ──
...
//
// Cap inputs differ by chart. VFD only exists on 7-Point. Relief/Reducing on both.
//   3-Point ('') : pm-relief, pm-reducing            (no VFD field)
...
// ══════════════════════════════════════════════════
// CHART B: 7-Point VFD Discharge Pressure Curve
...
function initPldChart() {
  ...
  pldChart = new Chart(canvas.getContext('2d'), {
    ...
    data:{ datasets:[
      ...
      // 2: Measured Discharge w/ VFD — sage, solid bold (Scheme A measured)
      {label:'Measured Discharge (w/ VFD)',data:[],borderColor:'#5F8068',backgroundColor:'transparent',
      ...
      // 10: VFD Setting — muted plum, dotted (Scheme A)
      {label:'VFD Setting',data:[],borderColor:'#7D6F9C',backgroundColor:'transparent',showLine:true,fill:false,tension:0,pointRadius:0,borderWidth:1.6,borderDash:[2,3],datalabels:labelOff},
...
function updatePldChart() {
  ...
  // PRV / VFD / cap lines — extend to 150% rated flow (S226). Fallback to max measured flow.
  // S227: PRV Rating (Rated Press. ref) line REMOVED — rated pressure is a design reference,
  //        not a limiter (golden curve already caps to relief/reducing/VFD). Slot 9 inert.
  ...
  // 13: golden actual-output curve = min(measured discharge, lowest active cap incl. VFD)
...
// ══════════════════════════════════════════════════
// CHART D: 7-Point Fire Pump Net Pressure Curve (w/ VFD + w/o VFD + PRV line)
...
function initPldNetChart() {
  ...
  pldNetChart = new Chart(canvas.getContext('2d'), {
    ...
    plugins:[],
    data:{ datasets:[
      // 0: Measured Net w/o VFD — Ledger indigo, solid bold
      {label:'Measured Net (w/o VFD)',data:[],borderColor:'#6366F1',backgroundColor:'transparent',
        showLine:true,fill:false,tension:0,pointRadius:6,pointBorderColor:'#6366F1',pointBorderWidth:2.5,pointBackgroundColor:'#fff',borderWidth:2.6,datalabels:labelOff},
      // 1: Adjusted Net w/o VFD (RPM-corrected) — Ledger cyan, dash-dot
      {label:'Adjusted Net (w/o VFD)',data:[],borderColor:'#21D3ED',backgroundColor:'transparent',
        showLine:true,fill:false,tension:0,pointRadius:0,borderWidth:1.8,borderDash:[8,4,2,4],datalabels:labelOff},
...
function updatePldNetChart() {
  ...
  // Measured + Adjusted net w/o PRV & VFD (only on 25/50/75/125% rows — PLD_NO_SKIP holds the skips)
  var measured=[], adjusted=[];

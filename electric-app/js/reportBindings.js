/* ══════════════════════════════════════════════════════════════════════════
   DIESEL — REPORT STATE BINDINGS          electric-app/js/reportBindings.js
   ──────────────────────────────────────────────────────────────────────────
...
function dieselStateEnv(opts) {
  ...
  return {
    ...
    custom: {
      ...
      /* ELECTRIC — power source per start-up */
      collectBatPower: function () { return { b1: batPower.b1.slice(), b2: batPower.b2.slice() }; },
      applyBatPower:   function (v) {
        if (!v) return;
        if (Array.isArray(v.b1)) batPower.b1 = v.b1.map(function(x){ return x==='normal'||x==='emergency' ? x : ''; });
        if (Array.isArray(v.b2)) batPower.b2 = v.b2.map(function(x){ return x==='normal'||x==='emergency' ? x : ''; });
        try { renderBatTable('bat1-body','b1'); renderBatTable('bat2-body','b2'); } catch (e) {}
      },
      /* ELECTRIC — three-phase V/A: merge by row so a shorter saved list never truncates */
      collectVaStd: function () { return vaStdData.map(function(r){ return Object.assign({}, r); }); },
      applyVaStd:   function (v) {
        if (!Array.isArray(v)) return;
        v.forEach(function(r,i){ if (vaStdData[i] && r) Object.assign(vaStdData[i], r); });
        try { renderVaStdTable(); } catch (e) {}
      },
      collectVaPld: function () { return vaPldData.map(function(r){ return Object.assign({}, r); }); },
      applyVaPld:   function (v) {
        if (!Array.isArray(v)) return;
        v.forEach(function(r,i){ if (vaPldData[i] && r) Object.assign(vaPldData[i], r); });
        try { renderVaPldTable(); } catch (e) {}
      },
      /* ELECTRIC — auto-transfer test (shape identical to the single-file tool's autoTransfer block) */
      collectAutoTransfer: function () {
        return { at4a1Status: at4a1Status, at4b1Status: at4b1Status, swElapsed: _atElapsed, swPldElapsed: _atPldElapsed };
      },
      applyAutoTransfer: function (v) {
        if (!v) return;
        /* setAt4a1/setAt4b1 TOGGLE — calling them with the saved value while the
           variable already holds it turns the answer OFF. The single-file tool had
           exactly that load bug; clear first so the toggle lands ON. */
        if (v.at4a1Status) { at4a1Status = ''; try { setAt4a1(v.at4a1Status); } catch (e) { at4a1Status = v.at4a1Status; } }
        if (v.at4b1Status) { at4b1Status = ''; try { setAt4b1(v.at4b1Status); } catch (e) { at4b1Status = v.at4b1Status; } }
        if (typeof v.swElapsed === 'number') _atElapsed = v.swElapsed;
        if (typeof v.swPldElapsed === 'number') _atPldElapsed = v.swPldElapsed;
        try { updateAtPassFail(); updateAtPldPassFail(); } catch (e) {}
      },

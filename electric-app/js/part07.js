...
function _collectAllPhotos(opts){
  ...
  return window.PhotoInventory.collect([
    ...
    // 2. Flow test photos (VFD) — S355: badge by category tag
    { name:'flowtest-pld', each:function(emit){
      (flowTestPhotosPld||[]).forEach(function(p,i){
        var sh=(typeof _floweqShort==='function')?_floweqShort((typeof _floweqTag==='function')?_floweqTag(p):'flow_chart'):'Flow';
        emit(p,{type:'flowtest-pld', cat:'flow', badge:'7\u00b7'+sh, label:_floweqLabel?_floweqLabel(_floweqTag(p)):'Flow Test (VFD)', section:'flowtestpld', idx:i});
      });
    }},
    ...
    { name:'record', each:function(emit){
      (typeof recordPhotos==='undefined'?[]:recordPhotos).forEach(function(p,i){
        // S339: short, distinct badges — 3-pt photos are plain; 7-pt (VFD) photos
        // carry a "VFD" tag so the gallery tells the two pumps apart at a glance.
        var _k=p.kind||'site', kl, badge;
        if(_k==='pump'){ kl='General Pump Photos'; badge='3\u00b7Pump'; }
        else if(_k==='pump-pld'){ kl='General Pump Photos (VFD)'; badge='7\u00b7Pump'; }
        else if(_k==='placard'){ kl='Pump Placard & VFD Placard'; badge='3\u00b7Placard'; }
        else if(_k==='placard-pld'){ kl='Pump Placard & VFD Placard'; badge='7\u00b7Placard'; }
    ...
    { name:'gauge-pld', each:function(emit){
      (typeof pldData==='undefined'?[]:pldData).forEach(function(row,ri){
        (row && row.photos ? row.photos : []).forEach(function(p,pi){
          emit(p,{type:'gauge-pld', cat:'flow', badge:('7\u00b7'+(row.pct||(ri+''))+(p.tag&&_GAUGE_TAG_SHORT[p.tag]?('\u00b7'+_GAUGE_TAG_SHORT[p.tag]):'')+(p.mode==='pld'?'\u00b7VFD':'')), label:'Gauge (VFD) '+(row.pct||'')+(p.tag&&_GAUGE_TAG_LABEL[p.tag]?(' \u00b7 '+_GAUGE_TAG_LABEL[p.tag]):'')+(p.mode==='direct'?' \u00b7 without VFD':p.mode==='pld'?' \u00b7 with VFD':''), section:'gauge_pld_'+ri, idx:pi});
        });
      });
    }}
  ], {
...
function _evTileHtml(kind){
  ...
  var map={pump:['📷','General Pump Photos','tap / drop · camera'],'pump-pld':['📷','General Pump Photos','tap / drop · camera'],placard:['🏷','Pump Placard & VFD Placard','tap / drop · camera'],'placard-pld':['🏷','Pump Placard & VFD Placard','engine placard · control pressure'],flow:['📊','Flow Chart & Equipment','charts · gauges · calibration'],'flow-pld':['📊','Flow Chart & Equipment','charts · gauges · calibration']};   // S584 (Mark): card retitles
...
function _placardScan(btn, mode){
  // S338: 3-pt and 7-pt are SEPARATE PUMPS (3-pt=constant-speed, 7-pt=VFD/variable).
  ...
  // 7-pt engine/VFD placard. Each tab scans ONLY its own kind.
  ...
  if(!take.length){ showToast(mode==='pld' ? 'Capture a VFD/motor placard photo first' : 'Capture a placard photo first'); return; }
...
function _placardPreview(btn, d, nPhotos, mode){
  ...
  var fields=[
    ...
    { key:'pldcp', label:'VFD Control Pressure (psi)',val:d.pld_control_pressure_psi, cur:inputCur('pm-pld-setting'),
      apply:function(v){ var el=document.getElementById('pm-pld-setting'); if(el) el.value=v; } }
  ];
  // S338: VFD control pressure is a 7-pt-only concept — hide it on a 3-pt scan.
  ...
  html+='<button id="pscan-cancel" style="background:#455A64;color:#fff;border:none;border-radius:7px;padding:7px 16px;font:600 13px Calibri,sans-serif;cursor:pointer;">'+(any?'Cancel':'Close')+'</button></div>'
    +'<div style="margin-top:7px;color:#928E9C;font-size:11px;">AI-read values \u2014 verify against the physical placard before applying. Fills only the '+(mode==='pld'?'7-pt (VFD)':'3-pt')+' tab; churn/100%/150% land in this tab\u2019s Placard column rows.</div>';
  card.innerHTML=html;
...
function _pgReassignSelected(mode){
  ...
  var dests = ['Site Photos', 'General Pump Photos', 'Pump Placard & VFD Placard', 'VFD Placard (7-pt)',
               'Flow Test (3-pt)', 'Flow Test (VFD)'];
  var _RECDEST = { 'Site Photos':'site', 'General Pump Photos':'pump', 'Pump Placard & VFD Placard':'placard', 'VFD Placard (7-pt)':'placard-pld' };   // S584: labels follow the card retitles; VFD bucket disambiguated
  ...
  _aConfirmHtml(html, function(){
    ...
    if(_copyMode){
      ...
      var _placeCopy = function(cp){
        ...
        if(dest === 'Flow Test (VFD)'){ flowTestPhotosPld.push(cp); return true; }
        if(_CLDEST[dest]){
    ...
    selected.forEach(function(item){
      ...
      else if(dest === 'Flow Test (VFD)') flowTestPhotosPld.push(ph);

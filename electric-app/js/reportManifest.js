/* ══════════════════════════════════════════════════════════════════════════
   DIESEL — REPORT MANIFEST                 electric-app/js/reportManifest.js
   ──────────────────────────────────────────────────────────────────────────
...
var PROJ_FIELD_IDS = [
  ...
  /* ELECTRIC — auto-transfer test inputs */
  'at-std-peak','at-std-restore',
  'at-pld-std-peak','at-pld-std-restore','at-pld-vfd-peak','at-pld-vfd-restore',
...
var KEYS = [
  ...
  /* ELECTRIC — which power source each start-up ran on ('normal'|'emergency'|'') */
  { key: 'batPower',
    collect: { kind: 'custom', fn: 'collectBatPower' },
    apply:   { kind: 'custom', fn: 'applyBatPower' } },

  /* ELECTRIC — three-phase voltage & amperage readings */
  { key: 'vaStdData',
    collect: { kind: 'custom', fn: 'collectVaStd' },
    apply:   { kind: 'custom', fn: 'applyVaStd' } },
  { key: 'vaPldData',
    collect: { kind: 'custom', fn: 'collectVaPld' },
    apply:   { kind: 'custom', fn: 'applyVaPld' } },

  /* ELECTRIC — auto-transfer test: YES/NO/NA status + stopwatch elapsed */
  { key: 'autoTransfer',
    collect: { kind: 'custom', fn: 'collectAutoTransfer' },
    apply:   { kind: 'custom', fn: 'applyAutoTransfer' } },

...
function _aConfirm(msg,onOk,okText){
  ...
  if(!D){ try{ console.error('[Electric] dialog engine not loaded \u2014 action blocked for safety:', msg); }catch(_){} return; }
...
function _aConfirmHtml(htmlMsg, onOk, okText){
  ...
  var D=window.ArenconDlg;
  if(!D||!D.panel){ try{ console.error('[Electric] dialog engine not loaded \u2014 diagnostics panel unavailable'); }catch(_){} return; }
  var buttons = onOk
...
function _aPrompt(msg,defVal,onOk){
  ...
  var D=window.ArenconDlg;
  if(!D){ try{ console.error('[Electric] dialog engine not loaded \u2014 prompt unavailable'); }catch(_){} return; }
...
function _aTypeConfirm(msg, requiredWord, onOk, okText){
  /* S497: sealed-engine typeToConfirm (guards the reset-all / reset-page mass
     ...
     handoff regardless. */
  var D=window.ArenconDlg;
  if(!D){ try{ console.error('[Electric] dialog engine not loaded \u2014 action blocked for safety'); }catch(_){} return; }
...
function _aAlert(msg){
  ...
  var D=window.ArenconDlg;
  if(!D){ try{ console.error('[Electric] dialog engine not loaded \u2014 alert:', msg); }catch(_){} return; }
